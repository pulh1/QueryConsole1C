...
Функция ТекстВыраженияОтбора(ПараметрыВыполнения, ПсевдонимИсточника, ВыводитьПсевдонимИсточника = Ложь) Экспорт
	Если Не ЗначениеЗаполнено(ПараметрыВыполнения.Отборы) Тогда
		Возврат "";
	КонецЕсли;
...
Функция УдалитьБлокПоместитьИзПоследнегоЗапроса(ТекстЗапроса) Экспорт
	...
	Схема.ПакетЗапросов[Схема.ПакетЗапросов.Количество() - 1].ТаблицаДляПомещения = "";
