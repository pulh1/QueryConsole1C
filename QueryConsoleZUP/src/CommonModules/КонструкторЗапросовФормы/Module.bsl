...
//  Структура:
//   * Реестр - ОбработкаОбъект.РеестрПоставщиковМетаданныхТаблиц - Реестр поставщиков метаданных.
//   * СтандартныйПоставщик - ОбработкаОбъект.ПоставщикМетаданныхСтандартныхТаблиц - Поставщик стандартных таблиц.
//   * ПоставщикПредставлений - ОбработкаОбъект.ПоставщикМетаданныхИсполняемыхПредставлений - Поставщик исполняемых представлений.
...
//
// Параметры:
//  КонтекстМетаданныхUI - Структура - Контекст поставщиков формы.
//
// Возвращаемое значение:
//  Массив из Структура - Описания строк корневого каталога.
...
// Заполняет корень дерева ленивого каталога стандартных таблиц и представлений.
//
// Параметры:
//  УзелДереваТаблиц - ДанныеФормыДерево - приемник строк каталога.
//  КонтекстМетаданныхUI - см. НовыйКонтекстМетаданныхUI.
Процедура ЗаполнитьКорневыеСтрокиКаталога(
	УзелДереваТаблиц,
	...
	ДобавитьОписанияСтрокВДеревоТаблиц(
		УзелДереваТаблиц,
		ОписанияКорневыхСтрокКаталога(КонтекстМетаданныхUI));
...
// Параметры:
//  КонтекстМетаданныхUI - Структура - Контекст поставщиков формы.
//  ОписаниеРодителя - Структура - Описание раскрываемой строки каталога.
//
// Возвращаемое значение:
//  Массив из Структура - Описания дочерних строк каталога.
...
//
// Возвращаемое значение:
//  Массив из Структура - Описания строк полей таблицы.
...
//
// Параметры:
//  ЭлементКаталога - Структура - Элемент каталога поставщика.
//
// Возвращаемое значение:
//  Структура - Описание строки дерева формы.
Функция ОписаниеСтрокиКаталога(ЭлементКаталога) Экспорт
	...
	Если ЭлементКаталога.ВидИсточника = "СтандартнаяГруппа" Тогда
		Описание.ТипЗначения = Новый ОписаниеТипов(
...
//
// Параметры:
//  ОписаниеПоля - Структура - Нейтральное описание поля поставщика.
//
// Возвращаемое значение:
//  Структура - Описание строки дерева формы.
Функция ОписаниеСтрокиПоля(ОписаниеПоля) Экспорт
	...
	Описание.ЕстьДочерние = ОписаниеПоля.Поля.Количество() > 0
		Или ОписаниеТипаСодержитМетаданные(ОписаниеПоля.ТипЗначения);
	Возврат Описание;

КонецФункции

Функция ОписаниеТипаСодержитМетаданные(ОписаниеТипа)

	Если ОписаниеТипа = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;

	Для Каждого ТипДанных Из ОписаниеТипа.Типы() Цикл
		Если Метаданные.НайтиПоТипу(ТипДанных) <> Неопределено Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
// Заполняет дочерние строки раскрываемого узла доступных таблиц.
//
// Параметры:
//  Идентификатор - Произвольный - Идентификатор строки дерева.
//  ДеревоТаблиц - ДанныеФормыДерево - Дерево доступных таблиц.
//  ОператорЗапроса - Структура - Текущий оператор модели запроса.
//  КонтекстМетаданныхUI - Структура - Контекст поставщиков формы.
//  ОписаниеДоступныхВТ - Произвольный - Описания доступных временных таблиц.
Процедура ЗаполнитьДочерниеЭлементыДереваДоступныхТаблиц(
	...
	Если КонструкторЗапросовКлиентСерверФормы.СодержитТип(
		Строка.ТипЗначения,
		Тип("ГруппаДоступныхТаблицСхемыЗапроса")) Тогда
