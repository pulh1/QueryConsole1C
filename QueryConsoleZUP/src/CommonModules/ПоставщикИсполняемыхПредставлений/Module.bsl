...
Функция СоответствиеИменОбработокИменамТаблицПредставлений() Экспорт
	Возврат ПоставщикИсполняемыхПредставленийПовтИсп.СоответствиеИменОбработокИменамТаблицПредставлений();
КонецФункции
...
Функция ИменаТаблицПредставленийВходящихВГруппуСлужебный(ИмяГруппы) Экспорт
	...
	ИначеЕсли ИменаГруппОбъектовМетаданныхПоддерживаюшихПредставления().Получить(ВРег(ЧастиИмени[0])) <> Неопределено Тогда
		ИменаПредставлений = ИменаПредставленийРегистра(ИмяГруппы, ЧастиИмени[1]);
	КонецЕсли;
...
Функция СоответствиеИменОбработокИменамТаблицПредставленийСлужебный() Экспорт
	СоответствиеИменОбработокИменамПредставлений = Новый Соответствие();
	
	ИменаОбработок = ИменаОбработокИсполняемыхПредставлений();
	Для Каждого ИмяОбработки Из ИменаОбработок Цикл
		СоответствиеИменОбработокИменамПредставлений.Вставить(
			ВРег("ИсполняемоеПредставление." + Обработки[ИмяОбработки].ИмяПредставления()), 
			ИмяОбработки);
	КонецЦикла;
