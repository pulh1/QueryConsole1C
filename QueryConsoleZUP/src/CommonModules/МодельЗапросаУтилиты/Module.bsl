...
Функция ИспользуемыеПоляИсточников(ЗапросВыбора, ИндексОператора) Экспорт
	...
	Для Каждого Источник Из ВсеИсточникиОператора(Оператор) Цикл
...
Функция ИспользуемыеПоляИсточникаВВыбираемыхПолях(ОператорЗапроса, ИдентификаторИсточника) Экспорт
	ИспользуемыеПоля = Новый Соответствие();	
	Для Каждого Поле Из ОператорЗапроса.ВыбираемыеПоля Цикл
		ЗаполнитьИспользуемыеПоляИзВыражения(ИспользуемыеПоля, Поле.Выражение);		
	КонецЦикла;
	
	Если ИспользуемыеПоля.Получить(ИдентификаторИсточника) = Неопределено Тогда
		Возврат Новый Соответствие();
	Иначе
		Возврат ИспользуемыеПоля.Получить(ИдентификаторИсточника);
	КонецЕсли;
КонецФункции
...
Функция ПолучитьИспользуемыеПараметры(ТекстЗапроса, Сортировать = Истина) Экспорт
	...
	Для Каждого КлючЗначение Из ИспользуемыеПараметрыЗапроса Цикл
	 	Результат.Добавить(КлючЗначение.Значение, КлючЗначение.Значение.Имя);
	КонецЦикла;
...
Функция НеобходимаГруппировка(ОператорЗапроса) Экспорт
	...
	КоличествоАгрегируемыхПолей = КоличествоАгрегируемыхПолей(ОператорЗапроса.ВыбираемыеПоля); 
	Возврат КоличествоАгрегируемыхПолей > 0
		И КоличествоАгрегируемыхПолей <> ОператорЗапроса.ВыбираемыеПоля.Количество();
КонецФункции	
...
Функция ДополнительныеПоляГруппировки(ОператорЗапроса) Экспорт 
	...
	Для Каждого ЭлементГруппировки Из ОператорЗапроса.Группировка.Элементы Цикл
		ПолеИсточника = ОбработкаМоделиЗапроса.ПолеИсточникаИзВыражения(ЭлементГруппировки);
	...
	Для Каждого Поле Из ОператорЗапроса.ВыбираемыеПоля Цикл
		...
		ПолеИсточника = ОбработкаМоделиЗапроса.ПолеИсточникаИзВыражения(Поле.Выражение);
		Если ПолеИсточника = Неопределено Тогда
...
Функция КонтекстОбработкиВыражения(
	...
	РеестрПоставщиков = Неопределено) Экспорт

	Контекст = ОбработкаМоделиЗапроса.КонтекстОбработкиВыражения(РеестрПоставщиков);
	Контекст.ИндексЗапросаПакета = ИндексПакетаЗапроса;
	
	Для Каждого Источник Из ВсеИсточникиОператора(ОператорЗапроса) Цикл
...
Функция АгрегатнаяФункцияДляВыражения(Выражение, Контекст) Экспорт 
	...
	Возврат АгрегатнаяФункцияПоТипуЗначения(КопияВыражения.ТипЗначения);
КонецФункции	

Функция АгрегатнаяФункцияПоТипуЗначения(ТипЗначения) Экспорт
...
Функция СписокВыбораАгрегируемогоПоля(Выражение, ОператорЗапроса, ИндексЗапроса) Экспорт
	...
	ГенераторТекстов.УстановитьИсточники(ВсеИсточникиОператора(ОператорЗапроса));
...
Функция ИндексИсточникаЗапросаПоПсевдониму(ОператорЗапроса, ПсевдлонимИсточника) Экспорт
	Источники = ВсеИсточникиОператора(ОператорЗапроса);
	Для Индекс = 0 По Источники.ВГраница() Цикл
		СтрокаИсточника = Источники[Индекс];
...
// Источник запроса по псевдониму.
// 
// Параметры:
//  ОператорЗапроса - см. ЭлементыМоделиЗапроса.НовыйОператорЗапроса
...
Функция ИсточникЗапросаПоПсевдониму(ОператорЗапроса, ПсевдлонимИсточника) Экспорт
	Индекс = ИндексИсточникаЗапросаПоПсевдониму(ОператорЗапроса, ПсевдлонимИсточника);
	Если Индекс = Неопределено Тогда
		Возврат Неопределено;
	Иначе
		Возврат ВсеИсточникиОператора(ОператорЗапроса)[Индекс];
	КонецЕсли;	
КонецФункции

// Источники оператора по идентификаторам.
// 
// Параметры:
...
Функция ИсточникиОператораПоИдентификаторам(Оператор) Экспорт
...
// Все источники оператора в порядке depth-first обхода дерева JOIN.
//
// Параметры:
//  Оператор - см. ЭлементыМоделиЗапроса.НовыйОператорЗапроса
//
...
Функция ВсеИсточникиОператора(Оператор) Экспорт
	Возврат ВсеИсточники(Оператор.Источники);
КонецФункции

// Все источники в порядке depth-first обхода дерева JOIN.
//
// Параметры:
//  КорневыеИсточники - Массив из см. ЭлементыМоделиЗапроса.НовыйИсточник
//
// Возвращаемое значение:
//  Массив из см. ЭлементыМоделиЗапроса.НовыйИсточник
Функция ВсеИсточники(КорневыеИсточники) Экспорт
	Результат = Новый Массив();
	Посещенные = Новый Соответствие();
	Для Каждого Источник Из КорневыеИсточники Цикл
		ДобавитьИсточникИДочерние(Источник, Результат, Посещенные);
	КонецЦикла;
	Возврат Результат;
КонецФункции

// Удаляет источник из корней оператора или из JOIN владельца.
//
// Параметры:
//  Оператор - см. ЭлементыМоделиЗапроса.НовыйОператорЗапроса
//  УдаляемыйИсточник - см. ЭлементыМоделиЗапроса.НовыйИсточник
//
// Возвращаемое значение:
//  Булево - Истина, если источник найден и удален
Функция УдалитьИсточникОператора(Оператор, УдаляемыйИсточник) Экспорт
	Индекс = Оператор.Источники.Найти(УдаляемыйИсточник);
	Если Индекс <> Неопределено Тогда
		Оператор.Источники.Удалить(Индекс);
		Возврат Истина;
	КонецЕсли;

	Для Каждого Источник Из ВсеИсточникиОператора(Оператор) Цикл
		Для ИндексСоединения = 0 По Источник.Соединения.ВГраница() Цикл
			Если Источник.Соединения[ИндексСоединения].Источник.ИдентификаторИсточника
				= УдаляемыйИсточник.ИдентификаторИсточника Тогда
...
Процедура ДобавитьИсточникИДочерние(Источник, Результат, Посещенные)
	Если Посещенные.Получить(Источник.ИдентификаторИсточника) = Истина Тогда
		ВызватьИсключение "Модель источников должна быть деревом без циклов и повторных узлов";
	КонецЕсли;
	Посещенные.Вставить(Источник.ИдентификаторИсточника, Истина);
	Результат.Добавить(Источник);
	Для Каждого Соединение Из Источник.Соединения Цикл
		ДобавитьИсточникИДочерние(Соединение.Источник, Результат, Посещенные);
	КонецЦикла;
КонецПроцедуры

Функция ИндексСоединения(ИсточникЗапроса, ИдентификаторПрисоединяемойТаблицы) Экспорт
	Для Индекс = 0 По ИсточникЗапроса.Соединения.ВГраница() Цикл
		Соединение = ИсточникЗапроса.Соединения[Индекс];
		Если Соединение.Источник.ИдентификаторИсточника = ИдентификаторПрисоединяемойТаблицы Тогда
			Возврат Индекс;
		КонецЕсли;	
	КонецЦикла;
...
Функция НайтиСоединение(ОператорЗапроса, ПсевдонимПрисоединяемойТаблицы, ПсевдонимВладельцаСвязи) Экспорт
	Источник = ИсточникЗапросаПоПсевдониму(ОператорЗапроса, ПсевдонимВладельцаСвязи); 
	Для Каждого Соединение Из Источник.Соединения Цикл
		Если ВРег(Соединение.Источник.Источник.Псевдоним) = ВРег(ПсевдонимПрисоединяемойТаблицы) Тогда
			Возврат Соединение;
		КонецЕсли;
	КонецЦикла;
...
Функция НайтиСредиВсехСоединений(ОператорЗапроса, ПсевдонимПрисоединяемойТаблицы) Экспорт
	Для Каждого Источник Из ВсеИсточникиОператора(ОператорЗапроса) Цикл
		Для Каждого Соединение Из Источник.Соединения Цикл
			Если ВРег(Соединение.Источник.Источник.Псевдоним) = ВРег(ПсевдонимПрисоединяемойТаблицы) Тогда
				Возврат Соединение;
...
Функция СгенерироватьПсевдонимИсточника(ИмяТаблицы, ОператорЗапроса) Экспорт
	...
	Для Каждого ИсточникОператора Из ВсеИсточникиОператора(ОператорЗапроса) Цикл
...
Функция СгенерироватьПсевдонимВыбираемогоПоля(Выражение, ОператорЗапроса) Экспорт
	...
	Если Выражение.Значение.Тип = "АгрегатнаяФункцияКоличество"
		И ТекущееВыражение = "*" Тогда
		ПсевдонимБезПостфикса = "Количество";
	ИначеЕсли ТекущееВыражение.Тип = "Разыменование" Тогда  
...
Функция СортироватьИсточникиПоИерархии(Источники) Экспорт
	Возврат ВсеИсточники(Источники);
КонецФункции
...
Функция ОписаниеВременнойТаблицы(Запрос, ИндексЗапроса) Экспорт 
	...
	Для Каждого Колонка Из Запрос.Колонки Цикл
		ДобавитьКолнкуВОписаниеВТ(ОписаниеВременнойТаблицы, Колонка);
	КонецЦикла;	
	
	Возврат ОписаниеВременнойТаблицы;
КонецФункции	

Процедура ДобавитьКолнкуВОписаниеВТ(ОписаниеВТ, Колонка) Экспорт
...
Функция КоличествоАгрегируемыхПолей(ВыбираемыеПоля)
	Количество = 0;
...
Процедура ЗаполнитьИспользуемыеВОператореПараметры(ИспользуемыеПараметры, ОператорЗапроса)
	Для Каждого Источник Из ВсеИсточникиОператора(ОператорЗапроса) Цикл
		...
		ИначеЕсли Источник.Источник.Тип = "ИсполняемоеПредставление" Тогда 	
			ЗаполнитьИспользуемыеВИсполняемомПредставленииПараметры(ИспользуемыеПараметры, Источник.Источник);		
		КонецЕсли;
...
// Заполняет используемые в исполняемом представлении параметры.
// 
// Параметры:
//  ИспользуемыеПараметры - Соответствие из КлючИЗначение:
//  * Ключ - Строка
...
Процедура ЗаполнитьИспользуемыеВИсполняемомПредставленииПараметры(ИспользуемыеПараметры, Источник)
	...
	Для Каждого Параметр Из Источник.Параметры Цикл
		Если Параметр.ЭтоПараметрЗапроса Тогда
			...
			ОписаниеПараметраЗапроса.Имя = Параметр.Значение;
			ОписаниеПараметраЗапроса.ТипЗначения = ОписаниеПараметраПредставления.ТипКонстанты;
			
			ИспользуемыеПараметры.Вставить(ВРег(ОписаниеПараметраЗапроса.Имя ), ОписаниеПараметраЗапроса);
		КонецЕсли;
	КонецЦикла;
	Если Источник.Отбор <> Неопределено Тогда
		ЗаполнитьИспользуемыеВВыраженииПараметры(ИспользуемыеПараметры, Источник.Отбор.Выражение);			
	КонецЕсли;
КонецПроцедуры
...
// Добавить поле в описание ВТ.
// 
// Параметры:
//  ОписаниеВТ - см. ЭлементыМоделиЗапроса.НовыйОписаниеВременнойТаблицы
//  ИмяПоля - Строка
//  ТипПоля - ОписаниеТипов
Процедура ДобавитьПолеВОписаниеВТ(ОписаниеВТ, ИмяПоля, ТипПоля) Экспорт
	Если ОписаниеВТ.Колонки.Получить(ВРег(ИмяПоля)) = Неопределено Тогда
		ОписаниеВТ.Колонки.Вставить(ВРег(ИмяПоля), ТипПоля);
		ОписаниеВТ.ПорядокКолонок.Добавить(ИмяПоля);
	КонецЕсли;	
КонецПроцедуры
