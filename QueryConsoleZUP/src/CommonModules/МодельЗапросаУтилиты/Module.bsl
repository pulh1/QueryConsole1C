...
Функция ИндексСоединения(ИсточникЗапроса, ИдентификаторПрисоединяемойТаблицы) Экспорт
	Для Индекс = 0 По ИсточникЗапроса.Соединения.ВГраница() Цикл
		...
		Если Соединение.Источник = ИдентификаторПрисоединяемойТаблицы Тогда
			Возврат Индекс;
		КонецЕсли;	
	КонецЦикла;
...
Функция НайтиСредиВсехСоединений(ОператорЗапроса, ПсевдонимПрисоединяемойТаблицы, ИсточникиПоИдентификаторам) Экспорт 
	Для Каждого Источник Из ОператорЗапроса.Источники.Элементы Цикл
		Для Каждого Соединение Из Источник.Соединения Цикл
			Если ВРег(ИсточникиПоИдентификаторам[Соединение.Источник].Источник.Псевдоним) = ВРег(ПсевдонимПрисоединяемойТаблицы) Тогда
				Возврат Соединение;
...
Функция СортироватьИсточникиПоИерархии(Источники) Экспорт
	...
	Для Каждого Источник Из Источники Цикл
		УзелИсточника = Новый Структура("Идентификатор, ДочерниеЭлементы", Источник.ИдентификаторИсточника, Новый Массив());
	...
	Для Каждого Источник Из Источники Цикл
		УзелИсточника = УзлыПоИд.Получить(Источник.ИдентификаторИсточника);
		ЗаполнитьУровниИсточников(УзелИсточника, УровниИсточников);
	КонецЦикла;
...
Процедура ЗаполнитьУровниИсточников(УзелИсточника, УровниИсточников, Знач ТекущийУровень = 0)
	СтрокаТЗ = УровниИсточников.Найти(УзелИсточника.Идентификатор, "Источник");
	...
	Для Каждого ПодчиненныйУзел Из УзелИсточника.ДочерниеЭлементы Цикл
		ЗаполнитьУровниИсточников(ПодчиненныйУзел, УровниИсточников, ТекущийУровень);
	КонецЦикла;	
КонецПроцедуры
