...
Функция ИспользуемыеПоляИсточникаВВыбираемыхПолях(ОператорЗапроса, ИдентифкаторИсточника) Экспорт
	ИспользуемыеПоля = Новый Соответствие();	
	Для Каждого Поле Из ОператорЗапроса.ВыбираемыеПоля Цикл
		ЗаполнитьИспользуемыеПоляИзВыражения(ИспользуемыеПоля, Поле.Выражение);		
	КонецЦикла;
	
	Если ИспользуемыеПоля.Получить(ИдентифкаторИсточника) = Неопределено Тогда
		Возврат Новый Соответствие();
	Иначе
		Возврат ИспользуемыеПоля.Получить(ИдентифкаторИсточника);
	КонецЕсли;
КонецФункции
...
// Источники оператора по идентификаторма.
// 
// Параметры:
