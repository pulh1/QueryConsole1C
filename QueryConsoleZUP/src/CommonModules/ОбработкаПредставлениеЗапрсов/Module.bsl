...
Процедура ОбработатьИсточникЗапроса(Источник, Контекст) Экспорт 
	...
	Для Каждого Параметр Из Параметры Цикл
		...
		ИначеЕсли ЭтоПараметрВТФильтр(Параметр) Тогда
			ВТФильтр = ОбработатьВТФильтр(ИсполняемоеПредставление, ОписаниеПредставления, Параметр, Источник.Источник, Контекст);
			УстанавливаемыеПараметры.Добавить(ВТФильтр);
		Иначе
			Параметр = ОбработатьПараметрПредставления(ИсполняемоеПредставление, Параметр, Источник, Контекст);	
			УстанавливаемыеПараметры.Добавить(Параметр);
		КонецЕсли;	
	КонецЦикла;	
	 
 	УстановитьПараметрыИсполняемогоПредтсавления(ИсполняемоеПредставление, ОписаниеПредставления, УстанавливаемыеПараметры);
	 
	Источник.Источник = ИсполняемоеПредставление;
КонецПроцедуры	

Процедура УстановитьПараметрыИсполняемогоПредтсавления(ИсполняемоеПредставление, ОписаниеПредставления, Параметры) Экспорт
	...
	Для Каждого ТекущийПараметр Из Параметры Цикл
		Если ТекущийПараметр.Тип = "ПарараметрИсполянегоПредставленияКонстанта" Тогда
		...
		ИначеЕсли ТекущийПараметр.Тип = "ПарараметрИсполянегоПредставленияВТФильтр" Тогда
...
Функция ОбработатьВТФильтр(ИсполняемоеПредставление, ОписаниеПредтсавления, Параметр, Источник, Контекст)
	...
	ВложенныейКонтекст = ОбработкаМоделиЗапроса.КонтекстОбработкиВложенногоЗапроса(Контекст);
	ОбработкаМоделиЗапроса.ОбработатьЗапрос(ЗапросФильтра, ВложенныейКонтекст); 
	
	ВТФильтр = ЭлементыМоделиЗапроса.НовыйПарараметрИсполянегоПредставленияВТФильтр();
...
Процедура ПроверитьОперандДляПараметраВТФильтр(ИсполняемоеПредставление, Псевдоним, ПараметрВТФильтр)
	...
	ЭлементРазымеования = ПараметрВТФильтр.Операнд.Элементы[0];
	
	Если ТипЗнч(ЭлементРазымеования) = Тип("Строка") Тогда
		Возврат; 
	КонецЕсли;
	
	Если ЭлементРазымеования.Тип <> "СписокВыражений" Тогда
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;	
	
	Для Каждого ЭлементСписка Из ЭлементРазымеования.Элементы Цикл
...
Функция РазобратьСписокПараметров(Операнд)
	...
	ЭлементРазымеования = Операнд.Элементы[0];
	
	Если ТипЗнч(ЭлементРазымеования) = Тип("Строка") Тогда
		Результат.Добавить(ЭлементРазымеования);
		Возврат Результат; 
	КонецЕсли;
...
Функция ЗаполнитьЭлементыОтбора(ЭлементыОтбора, ПараметрОтбора, ИмяПредставления, ПсевдонимПредставления)
	Если ПараметрОтбора.Тип = "БинарнаяОперация" Тогда
		Если ПараметрОтбора.Операция = "И" Тогда 
			ЗаполнитьЭлементыОтбора(ЭлементыОтбора, ПараметрОтбора.ЛеваяЧасть, ИмяПредставления, ПсевдонимПредставления);  
			ЗаполнитьЭлементыОтбора(ЭлементыОтбора, ПараметрОтбора.ПраваяЧасть, ИмяПредставления, ПсевдонимПредставления);	
		ИначеЕсли ПараметрОтбора.Операция = "="
			Или ПараметрОтбора.Операция = "<>"
			Или ПараметрОтбора.Операция = ">"
			Или ПараметрОтбора.Операция = "<" Тогда  
			
			ПроверитьВыражениеПоляОбора(ПараметрОтбора.ЛеваяЧасть, ИмяПредставления, ПсевдонимПредставления);
			ПроверитьВыражениеЗначенияОтбора(ПараметрОтбора.ЛеваяЧасть, ИмяПредставления, ПсевдонимПредставления);
			
			ЭлементОтбора = НовыйЭлементаОтбора(); 
			ЭлементОтбора.Поле = ПолеОбора(ПараметрОтбора.ЛеваяЧасть);  
			ЭлементОтбора.ВидСравнения = ПараметрОтбора.Операция; 
			ЭлементОтбора.ПравоеЗначение = ЗначениеОтбора(ПараметрОтбора.ПраваяЧасть);	  
			
			ЭлементыОтбора.Добавить(ЭлементОтбора);
		Иначе	
			ВызватьИсключение "Не корректное описание отбора для представления " + ИмяПредставления+ " КАК " + ПсевдонимПредставления + ". 
				| Операция " + ПараметрОтбора.Операция + " не поддерживается";
		КонецЕсли;
	ИначеЕсли ПараметрОтбора.Тип = "ОператорВ" Тогда
		Если ПараметрОтбора.Инверсия Тогда
			ВызватьИсключение "Не корректное описание отбора для представления " + ИмяПредставления+ " КАК " + ПсевдонимПредставления + ". 
				|Отричание не поддерживается";
		КонецЕсли;
		
		Если ПараметрОтбора.Иерахия Тогда
			ВызватьИсключение "Не корректное описание отбора для представления " + ИмяПредставления + " КАК " + ПсевдонимПредставления + ". 
				|Отбор по иерархии не поддерживается";
		КонецЕсли;  
		
		Если ПараметрОтбора.Список.Элементы.Количество() > 1 Тогда
			ВызватьИсключение "Не корректное описание отбора для представления " + ИмяПредставления + " КАК " + ПсевдонимПредставления + ". 
				|Список выражений не поддерживается";
		КонецЕсли; 
		
		ПроверитьВыражениеПоляОбора(ПараметрОтбора.Операндь, ИмяПредставления, ПсевдонимПредставления);
		ПроверитьВыражениеЗначенияОтбора(ПараметрОтбора.Список.Элементы[0], ИмяПредставления, ПсевдонимПредставления);

		
		ЭлементОтбора = НовыйЭлементаОтбора();
		ЭлементОтбора.Поле = ПолеОбора(ПараметрОтбора.Операнд);  
		ЭлементОтбора.ВидСравнения = "В"; 
		ЭлементОтбора.ПравоеЗначение = ЗначениеОтбора(ПараметрОтбора.Список.Элементы[0]);
...
Процедура ПроверитьВыраженийПараметрКонстанта(ИсполняемоеПредставление, ПараметрКонстанта)		
	Если ПараметрКонстанта.Тип = "БинарнаяОперация"	Тогда
		...
		ПроверитьВыраженеЗначенияКонстанта(ИсполняемоеПредставление, ПараметрКонстанта.ПраваяЧасть); 
	Иначе
		ВызватьИсключение "Не корректное выражения для параметра представления " 
			+ ИсполняемоеПредставление.ИмяТаблицы + ".";	
	КонецЕсли;	
...
Процедура УстновитьПараметрКонстанта(ПараметрыПредставление, Параметр, ОписаниеПараметровПоИменам, ИмяПредставления) Экспорт 		
	
	ОписаниеПараметра = ОписаниеПараметровПоИменам.Получить(ВРег(Параметр.Имя));
	Если ОписаниеПараметра = Неопределено Тогда
		ВызватьИсключение "Не известное имя параметра " + ОписаниеПараметра.Имя + " для представления " 
				+ ИмяПредставления + ". 
...
Функция ПараметрКонстантаИзВыражения(ВыражениеПараметраКонстанты)		
	Параметр = ЭлементыМоделиЗапроса.НовыйПарараметрИсполянегоПредставленияКонстанта();
...
Процедура ПроверитьВыраженеЗначенияКонстанта(ИсполняемоеПредставление, ПараметрКонстанта)
...
Процедура ПроверитьВыражениеПоляОбора(Выражение, ИмяПредставления, ПсевдонимПредставления)  
	ТекстИсключения = "Не корректное описание отбора для представления " + ИмяПредставления + " КАК " + ПсевдонимПредставления + ". 
				|Для полей отбора не поддердиваются дополнительная обработка, функции, оператор ВЫБОР и.п.";
...
Процедура ПроверитьВыражениеЗначенияОтбора(Выражение,  ИмяПредставления, ПсевдонимПредставления)
	Если Выражение.Тип <> "ПараметрЗапроса"
		И Выражение.Тип <> "Константа" Тогда
		
		ТекстИсключения = "Не корректное описание отбора для представления " + ИмяПредставления + " КАК " + ПсевдонимПредставления + ". 
				|Для значений отбора поддердиваются только параметры запроса и константы";
	КонецЕсли;	 
КонецПроцедуры	

Функция ПолеОбора(Выражение)  	
	Возврат СтрСоединить(Выражение.Элементы, ".");
КонецФункции
...
Функция НовыйЭлементаОтбора()  
	ЭлементаОтбора = Новый Структура("Тип", "ЭлементаОтбора");
	ЭлементаОтбора.Вставить("Поле", ""); 
	ЭлементаОтбора.Вставить("ВидСравнения");  
	ЭлементаОтбора.Вставить("ПравоеЗначение");
	
	Возврат ЭлементаОтбора;
КонецФункции	
...
Функция ПарараметрКонстантаПоОписанию(ОписаниеПараметра) Экспорт
	Парараметр = ЭлементыМоделиЗапроса.НовыйПарараметрИсполянегоПредставленияКонстанта();  
	Парараметр.Имя = ОписаниеПараметра.Имя;
