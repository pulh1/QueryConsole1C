...
Функция МодельЗапросаДляСКД(ПараметрыВыполнения, ОписаниеПредставления) Экспорт
	...
	Для Каждого Поле Из ТипыИспользуемыхПолей Цикл
		ДобавитьПустоеПолеВЗапрос(Построитель, Поле.Поле, Поле.Типы);
	КонецЦикла;
	...
	Для Каждого Отбор Из ПараметрыВыполнения.ОтборыВнешнегоЗапроса Цикл
...
Процедура ДобавитьПустоеПолеВЗапрос(Построитель, ИмяПоля, ТипыПоля) Экспорт
	...
	Если ВыраженияПоля.Количество() = 1 Тогда
		Построитель.ДобавитьПолеВЗапрос(ВыраженияПоля[0], ИмяПоля);	
	Иначе
		...
		Построитель.ДобавитьПолеВЗапрос(Выражение, ИмяПоля);
	КонецЕсли;	
КонецПроцедуры
