...
Функция СтрокаПрисовенияПараметрыПеременной(ИмяПеременной, ИмяПараметра, ПараметрыВыполнения, Знач Табуляции = "", УчитыватьЗначениеПоУмолчанию = Ложь) Экспорт
...
Функция СтрокаПрисовенияЗначения(ИмяПеременной, Значение, Знач Табуляции = "") Экспорт
	Строка = ИмяПеременной + " = " + ПримитивноеЗначениеВСтроку(Значение) + ";";
	
	Возврат Табуляции + Строка;
КонецФункции

Процедура КодПрисвоенияПараметровВТекстовыйДокумент(ТекстовыйДокумент, ПараметрыВыполнения, ОписаниеПредставления, СоответствиеИмен, Знач Таблцляции = "") Экспорт
	Для Каждого ОписаниеПараметра Из ОписаниеПредставления.ОписаниеПараметров Цикл
		...
		Строка = СтрокаПрисовенияПараметрыПеременной(
...
Функция ПустоеЗначениеТипаВВыражениеСтрокой(Тип) Экспорт
	...
	МетададанныеТипа =  Метаданные.НайтиПоТипу(Тип); 
	Если МетададанныеТипа <> Неопределено Тогда
		Возврат "ЗНАЧЕНИЕ("  +МетададанныеТипа.ПолноеИмя() + ".ПУСТАЯССЫЛКА)";
	КонецЕсли;
...
Функция МодельЗапросаДляСКД(ПараметрыВыполнения, ОписаниеПредставления) Экспорт
	...
	Модель = МодельЗапросаУтилиты.СоздатьПакетЗапрсов();
