...
Функция ИсполнитьУзелПакетЗапросов(ТекстЗапроса, Контекст) 	
	...
	Для Каждого ЗапросПакета Из МодельЗапроса.Элементы Цикл
		Если ЗапросПакета.Тип = "ЗапросУничтожения" Тогда
			...
			КонецЕсли;	
			Если Контекст.ДостпныеДляУдаленияВТ.Получить(ВРег(ИмяТаблицы)) <> Неопределено Тогда
				Контекст.ДостпныеДляУдаленияВТ.Удалить(ВРег(ИмяТаблицы));
			КонецЕсли;	
		Иначе
...
Функция ВыполнитьЗапрос(ЗапросПакета, Контекст,  ВыполнятьБезусловно = Ложь)
	Если Не ВыполнятьЗапрос(ЗапросПакета, Контекст) И Не ВыполнятьБезусловно Тогда
		...
		ИспользуемыеПоляПоИстчоникам = ИспользуемыеПоляИсполняемыхПредставлений(ЗапросПакета.Операторы[0]);
		ПараметрыВыполнения.ИспользуемыеПоля = 
			ИспользуемыеПоляПоИстчоникам.Получить(ЗапросПакета.Операторы[0].Источники.Элементы[0].ИдентификаторИсточника);
			
		Если ПараметрыВыполнения.ИспользуемыеПоля = Неопределено Тогда
			ПараметрыВыполнения.ИспользуемыеПоля = Новый Массив();
		КонецЕсли;	
		...
		ПараметрыВыполнения.ПсевдонисыПолей = ПсевдонимыПолей(ЗапросПакета.Операторы[0]);	
...
Процедура ОбработатьОператор(ЗапросПакета, ИндексОператора, Контекст)
	...
	ИспользуемыеПоляПоИстчоникам = Неопределено;
	Для Каждого Источник Из Оператор.Источники.Элементы Цикл
		...
		ИначеЕсли Источник.Источник.Тип = "ИсполняемоеПредставление" Тогда 
			Если ИспользуемыеПоляПоИстчоникам = Неопределено Тогда
				ИспользуемыеПоляПоИстчоникам = ИспользуемыеПоляИсполняемыхПредставлений(Оператор);
			КонецЕсли;
			
			ПараметрыВыполнения = ЭлементыМоделиИсполненияПредставлений.НовыйПараметрыВыполненияПредставления();
			ПараметрыВыполнения.ТолькоРазрешенные = ЗапросПакета.ВыбиратьРазрешенные;
			ПараметрыВыполнения.ИспользуемыеПоля = ИспользуемыеПоляПоИстчоникам.Получить(Источник.ИдентификаторИсточника);
...
Функция ОбработатьПараметрВТФильтр(ИсполняемоеПредставление, ОписаниеПредставления, Контекст)
	...
	Если Не СоздавтьВТФильтр(ВТФильтр.ЗапросВыбора, ВТФильтр.Поля, ОписаниеПараметраВТФильтр, Контекст.ГенерацияЗапросаДляСКД) Тогда	
		Если ОписаниеПараметраВТФильтр.ПоддерживаютсяПсевдонимы Тогда
			ОписаниеВТФильтр.ПсевдонимыПолей = ПсведонимыПолейВТФильтр(ВТФильтр.ЗапросВыбора, ВТФильтр.Поля, Ложь);
		КонецЕсли;
	...
	Если ОписаниеПараметраВТФильтр.ПоддерживаетсяИмяВТФильтр Тогда
		...
		Контекст.ДостпныеДляУдаленияВТ.Вставить(ВРег(ОписаниеВТФильтр.ИмяВТ), Истина);	
	Иначе
		Если Контекст.ИспользуемыеИменаВТ.Получить(ВРег(ОписаниеПараметраВТФильтр.ИмяВТПоУмолчанию)) <> Неопределено 
			И Контекст.ДостпныеДляУдаленияВТ.Получить(ВРег(ОписаниеПараметраВТФильтр.ИмяВТПоУмолчанию)) = Неопределено Тогда
				
			ВызватьИсключение "Не возможно создать ВТФильтр для исполняемого представления " + ИсполняемоеПредставление.ИмяТаблицы + "
			|ВТ с именем " + ОписаниеПараметраВТФильтр.ИмяВТПоУмолчанию + " была ранее создана в пакете запрсов";	
		КонецЕсли;
		
		Если Контекст.ДостпныеДляУдаленияВТ.Получить(ВРег(ОписаниеПараметраВТФильтр.ИмяВТПоУмолчанию)) <> Неопределено Тогда
			...
			Контекст.ДостпныеДляУдаленияВТ.Удалить(ВРег(ОписаниеПараметраВТФильтр.ИмяВТПоУмолчанию));	
		КонецЕсли;
	...
	Модель = ЭлементыМоделиЗапроса.НовыйПакетЗапрсов();
	...
	Если ОписаниеПараметраВТФильтр.ПоддерживаютсяПсевдонимы
		...
		ОписаниеВТФильтр.ПсевдонимыПолей = ПсведонимыПолейВТФильтр(ВТФильтр.ЗапросВыбора, ВТФильтр.Поля);	
	Иначе
...
Функция ПсведонимыПолейВТФильтр(ЗапросВТФильтр, ПоляФильтра, УчитыватьПсевдонимыПолейЗапроса = Истина)
	ПсведонимыПолей = Новый Соответствие(); 
	
	Для Индекс = 0 По ПоляФильтра.ВГраница() Цикл
		Если УчитыватьПсевдонимыПолейЗапроса Тогда
			ПсведонимыПолей.Вставить(ПоляФильтра[Индекс], ЗапросВТФильтр.Колонки[Индекс].Имя);	
		Иначе
			ПолеВТ = ЗапросВТФильтр.Операторы[0].ВыбираемыеПоля[Индекс].Выражение.Значение.Элементы[0].ИмяПоля;
			ПсведонимыПолей.Вставить(ПоляФильтра[Индекс], ПолеВТ);
		КонецЕсли;
	КонецЦикла;
		
	Возврат ПсведонимыПолей;
КонецФункции
...
// Возвращаемое значение:
//  Булево - Выполнять запрос
Функция СоздавтьВТФильтр(ЗапросВТФильтр, ПоляФильтра, ОписаниеВТФильтр, ДляСКД)
...
// * ДостпныеДляУдаленияВТ - Соответствие из Строка, Булево  
...
Функция КонтекстИсполнения()
	...
	Контекст.Вставить("ДостпныеДляУдаленияВТ", Новый Соответствие);
...
Функция ИспользуемыеПоляИсполняемыхПредставлений(Оператор)
...
Процедура ЗаполнитьИспользуемыеПоляИзВыражения(ИспользуемыеПоля, ВыражениеМодели)
	...
	Для	Каждого КлючЗначение Из ВыражениеМодели.ПоляИсточников Цикл
		...
		КонецЕсли;
		ИспользуемыеПоляИсточника.Вставить(ВРег(ЧастиИмениПоля[1]), Истина);
	КонецЦикла;
КонецПроцедуры
...
Функция ИспользуемыеИндексы(ЗапросВыбора)
	...
	Для Каждого ИндексируемоеПоле Из ЗапросВыбора.Индекс.Элементы Цикл
		Индексы.Добавить(КолонкиПоИдентифкаторам[ИндексируемоеПоле.Идентификатор].Имя);
	КонецЦикла;	
