...
Процедура СформироватьУниверсальныйОтчет(Запрос, МакетКомпоновки, НастройкиСКД, ДанныеРасщифроки, ДокументРезультат) Экспорт
	Контекст = КонтекстИсполнения();

	Контекст.Исполнитель = Обработки.ИсполнительУниверсальногоОтчета.Создать();
	Контекст.Исполнитель.Инициализировать(МакетКомпоновки, НастройкиСКД, ДанныеРасщифроки, ДокументРезультат);
	
	ИсполнитьУзелПакетЗапросов(Запрос, Контекст);		
КонецПроцедуры

Функция ИсполнитьУзелПакетЗапросов(Запрос, Контекст) 	
	Если ТипЗнч(Запрос) = Тип("Строка") Тогда
		МодельЗапроса = ОбработкаМоделиЗапроса.РазобратьЗапрос(Запрос);
	Иначе
...
Функция ВыполнитьЗапрос(ЗапросПакета, Контекст,  ВыполнятьБезусловно = Ложь)
	Если Не ВыполнятьЗапрос(ЗапросПакета, Контекст) И Не ВыполнятьБезусловно Тогда
		...
		ПервыйИсточник = МодельЗапросаУтилиты.ВсеИсточникиОператора(ЗапросПакета.Операторы[0])[0];
		ПараметрыВыполнения.ИдентификаторИсточника = ПервыйИсточник.ИдентификаторИсточника;
		ИспользуемыеПоляПоИсточникам = МодельЗапросаУтилиты.ИспользуемыеПоляИсточников(
			ЗапросПакета, 
			0);
		ПараметрыВыполнения.ИспользуемыеПоля = 
			ИспользуемыеПоляПоИсточникам.Получить(ПервыйИсточник.ИдентификаторИсточника);
...
Процедура ОбработатьОператор(ЗапросПакета, ИндексОператора, Контекст)
	...
	Для Каждого Источник Из МодельЗапросаУтилиты.ВсеИсточникиОператора(Оператор) Цикл
		...
		ИначеЕсли Источник.Источник.Тип = "ИсполняемоеПредставление" Тогда 
			...
			ПараметрыВыполнения = ЭлементыМоделиИсполненияПредставлений.НовыйПараметрыВыполненияПредставления();
			ПараметрыВыполнения.ИдентификаторИсточника = Источник.ИдентификаторИсточника;
			ПараметрыВыполнения.ТолькоРазрешенные = ЗапросПакета.ВыбиратьРазрешенные;
...
Процедура ОбработатьИсполняемоеПредставление(ИсполняемоеПредставление, ПараметрыВыполненияПредставления, Контекст)	
	...
	ПараметрыВыполненияПредставления.УдалятьПромежуточныеВТ = ОписаниеПредставления.УдалятьПромежуточныеВТ;
	...
	Если ИсполняемоеПредставление.Отбор <> Неопределено Тогда
		ВыражениеОтбора = ЭлементыМоделиЗапроса.НовыйСписокВыражений();
		ВыражениеОтбора.Элементы.Добавить(ИсполняемоеПредставление.Отбор.Выражение.Значение);
		ПараметрыВыполненияПредставления.Отборы.Добавить(ВыражениеОтбора);
	КонецЕсли;	
КонецПроцедуры
...
Функция ОбработатьПараметрВТФильтр(ИсполняемоеПредставление, ОписаниеПредставления, Контекст)
	...
	Если Не СоздатьВТФильтр(ВТФильтр.ЗапросВыбора, ВТФильтр.Поля, ОписаниеПараметраВТФильтр, Контекст.ГенерацияЗапросаДляСКД) Тогда	
		...
		ОписаниеВТФильтр.ИмяВТ = МодельЗапросаУтилиты.ВсеИсточникиОператора(
			ВТФильтр.ЗапросВыбора.Операторы[0])[0].Источник.ИмяТаблицы;
...
Функция ВыполнятьЗапрос(ЗапросВыбора, Контекст)
	...
	ИсточникиОператора = МодельЗапросаУтилиты.ВсеИсточникиОператора(Оператор);
	Если ИсточникиОператора.Количество() <> 1 Тогда
		Возврат Истина;
	КонецЕсли;	
	
	Если Не ИсточникиОператора[0].Источник.Тип = "ИсполняемоеПредставление" Тогда
		Возврат Истина;
	КонецЕсли;
	...
	ИсполняемоеПредставление = ИсточникиОператора[0].Источник;
	ОписаниеПредставления = ПоставщикИсполняемыхПредставлений.ОписаниеПредставленияПоИмениТаблицы(ИсполняемоеПредставление.ИмяТаблицы);
	...
	Если ЗначениеЗаполнено(Оператор.Отбор)
		И Не ПредставлениеПоддерживаетВсеОтборы(
			...
			Оператор.Отбор, 
			ИсточникиОператора[0].ИдентификаторИсточника,
...
Функция ПредставлениеПоддерживаетВсеОтборы(ОписаниеПредставления, ПроверяемыеОтборы, ИдентификаторИсточника, ДляСКД)
	...
	Если ОписаниеПредставления.ОписаниеОтбора = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция СоздатьВТФильтр(ЗапросВТФильтр, ПоляФильтра, ОписаниеВТФильтр, ДляСКД)
	...
	Если Не ИсточникиОператора[0].Источник.Тип = "ИсточникДанныхВременнаяТаблица" Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если Не ОписаниеВТФильтр.ПоддерживаетсяИмяВТФильтр 
		И ВРег(ИсточникиОператора[0].Источник.ИмяТаблицы) <> ВРег(ОписаниеВТФильтр.ИмяВТПоУмолчанию) Тогда
		Возврат Истина;
	КонецЕсли;
