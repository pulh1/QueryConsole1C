...
Процедура ОбработатьЭлементыПорядка(ЗапросВыбора, ЭлементыПорядка, Контекст, ЭтоВложенныйЗапрос) Экспорт  
	Если (ЗначениеЗаполнено(ЗапросВыбора.ТаблицаДляПомещения) или ЭтоВложенныйЗапрос)
		...
		ВызватьИсключение "Во вложенных запросах и при создании временных таблиц упорядочивание можно использовать только совместно с выбрать ПЕРВЫЕ N записей";
	КонецЕсли;
	...
	Для Каждого ЭлементПорядка Из ЭлементыПорядка Цикл
		...
		Иначе
			Если Не ЕдинственныйОператор Тогда
				ТекстИсключения = "При объединении нескольких запросов, в блоке УПОРЯДОЧИТЬ необходимо указывать псевдонимы выбираемых полей";
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;		
