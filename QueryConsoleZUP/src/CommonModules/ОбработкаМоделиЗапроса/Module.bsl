...
Процедура ОбработатьМодельЗапроса(ПакетЗапросов) Экспорт 
	...
	Для Каждого Запрос Из ПакетЗапросов.Элементы Цикл
		...
		Если Запрос.Тип = "ЗапросУничтожения" Тогда 
			...
			КонецЕсли;	  
		Иначе
			ЭтоЗапросСозданияВТ = ЗначениеЗаполнено(Запрос.ТаблицаДляПомещения);
			Контекст.РазрешеныИтоги = Ложь;
		...
		Если ЭтоПервыйОператор Тогда 
			...
			Для Каждого Поле Из Оператор.ВыбираемыеПоля Цикл
				...
				Если Не ЗначениеЗаполнено(Поле.Псевдоним) 
					И Поле.Выражение.Значение.Тип <> "ВыражиениеВсеПоля" Тогда
...
Процедура ОбработатьПараметрыВиртуальнойТаблицы(Источник, Контекст)
	...
	Для Каждого ВыражениеПараметра Из Источник.Источник.Параметры Цикл
...
Процедура ОбработатьВыбираемыеПоляОператора(Оператор, Контекст)
	Для Каждого Поле Из Оператор.ВыбираемыеПоля Цикл     
		...
		Если Поле.Выражение.Значение.Тип = "ВыражиениеВсеПоля"
			...
			И Поле.Выражение.Значение.Элементы[Поле.Выражение.Значение.Элементы.ВГраница()].Тип = "НовыйВыражиениеВсеПоляИсточника") Тогда
...
Процедура ОбработатьВыражение(Выражение, Контекст, РассчитываемыеСвойства) Экспорт 
	...
	ИначеЕсли Выражение.Тип = "ФункцияРазностьДат" Тогда   
		...
		РассчитываемыеСвойства.ТипЗначения = Новый ОписаниеТипов("Число");		
	ИначеЕсли Выражение.Тип = "ВыражиениеВсеПоля" Тогда 
...
Функция ТипПоляПоИмени(ИмяПоля, Источник, Контекст)
	...
	ИначеЕсли Источник.Тип = "ИсточникДанныхВременнаяТаблица" Тогда	 
		Если Контекст.ЗаполнятьОписаниеВременныхТаблиц
			...
			ТипПоля = МодельЗапросаТипы.ОписаниеПроивзольногоТипа();
			ДобавитьПолеВОписаниеВТ(Источник.ОписаниеВТ, ИмяПоля, ТипПоля);
...
Процедура ЗаполнитьИспользуемыеОператоромПоля(Приемник, Оператор)
	Для Каждого ВыбираемоеПоле Из Оператор.ВыбираемыеПоля Цикл
		Для Каждого КлючЗнаение Из ВыбираемоеПоле.Выражение.ПоляИсточников Цикл
			Приемник.Вставить(КлючЗнаение.Ключ, КлючЗнаение.Значение);
		КонецЦикла;
	КонецЦикла;
...
Процедура ДобавитьПолеВОписаниеВТ(ОписаниеВТ, ИмяПоля, ТипПоля)
