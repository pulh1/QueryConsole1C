...
Функция КонтекстОбработкиВыражения(РеестрПоставщиков = Неопределено) Экспорт
	Если РеестрПоставщиков = Неопределено Тогда
		РеестрПоставщиков = Обработки.РеестрПоставщиковМетаданныхТаблиц.СоздатьРеестр();
	КонецЕсли;
	Возврат НовыйКонтекстОбработкиВыражения(РеестрПоставщиков);
КонецФункции

Функция КонтекстОбработкиЗапроса(РеестрПоставщиков = Неопределено) Экспорт
	Если РеестрПоставщиков = Неопределено Тогда
		РеестрПоставщиков = Обработки.РеестрПоставщиковМетаданныхТаблиц.СоздатьРеестр();
	КонецЕсли;
	Возврат НовыйКонтекстОбработкиЗапроса(РеестрПоставщиков);
КонецФункции

Функция НовыйКонтекстОбработкиВыражения(РеестрПоставщиков)

	Контекст = Новый Структура;
	
	Контекст.Вставить("ИндексЗапросаПакета", -1);
	Контекст.Вставить("РеестрПоставщиковМетаданныхТаблиц", РеестрПоставщиков);
...
Функция НовыйКонтекстОбработкиЗапроса(РеестрПоставщиков)
	Контекст = НовыйКонтекстОбработкиВыражения(РеестрПоставщиков);
...
Функция КонтекстОбработкиВложенногоЗапроса(ТекущийКонтекст, ДоступныИсточникиВнешнегоЗапроса = Ложь) Экспорт  
	Контекст = НовыйКонтекстОбработкиЗапроса(
...
Процедура ОбработатьКонтрольныеТочкиИтогов(ЗапросВыбора, КонтрольныеТочки, Контекст) Экспорт
	...
	Для Каждого КонтрольнаяТочка Из КонтрольныеТочки Цикл
		Если КонтрольнаяТочка.Тип = "ОбщиеИтоги" Тогда
			Если КонтрольныеТочки.Найти(КонтрольнаяТочка) > 0 Тогда
				ТекстИсключения = "Общий итог может указываться только в первой позиции итогов";
		...
		ОбработатьДополнениеПериодамиКонтрольнойТочкиИтогов(КонтрольнаяТочка, Контекст);
	КонецЦикла;
	
	Если ЗначениеЗаполнено(КонтрольныеТочки) Тогда
		КонтрольнаяТочка = КонтрольныеТочки[0];
		Если КонтрольнаяТочка.Тип = "ОбщиеИтоги" Тогда
			КонтрольныеТочки.Удалить(0);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Процедура ОбработатьОбщиеИтоги(ЗапросВыбора, ОбщиеИтоги, Контекст) Экспорт
	Если Не Контекст.РазрешеныИтоги
		И ОбщиеИтоги Тогда
		ВызватьИсключение "Итоги не могут использоваться во вложенных запросах и при создании временных таблиц";
	КонецЕсли;
...
Процедура ОбработатьДополнениеПериодамиКонтрольнойТочкиИтогов(
	КонтрольнаяТочка, ТекущийКонтекст = Неопределено) Экспорт
	...
	Если ТекущийКонтекст = Неопределено Тогда
		ТекущийКонтекст = КонтекстОбработкиВыражения();
	КонецЕсли;

	ОбработатьПериодДополненияКонтрольнойТочкиИтогов(КонтрольнаяТочка, ТекущийКонтекст);
КонецПроцедуры

Процедура ОбработатьПериодДополненияКонтрольнойТочкиИтогов(КонтрольнаяТочка, ТекущийКонтекст)
	Контекст = НовыйКонтекстОбработкиВыражения(
		ТекущийКонтекст.РеестрПоставщиковМетаданныхТаблиц);
...
Процедура ОбработатьОператор(Оператор, Контекст, РассчитываемыеСвойства = Неопределено)
	...
	ОбработатьОтборСКД(Оператор, Контекст);
	
	Если РассчитываемыеСвойства <> Неопределено Тогда
		ЗаполнитьРасчитываемыеСвойстваПоОператору(РассчитываемыеСвойства, Оператор);
	КонецЕсли;
КонецПроцедуры	
...
Процедура ОбработатьОтборСКД(Оператор, Контекст)
	Для Каждого ЭлементОтбора Из Оператор.ОтборыСКД Цикл     
		ОбработатьЭлементОтбораСКД(ЭлементОтбора, Контекст);
	КонецЦикла;
КонецПроцедуры	

Процедура ОбработатьЭлементОтбораСКД(ЭлементОтбора, Контекст) Экспорт
	ОбработатьВыражениеВыбираемогоПоля(ЭлементОтбора.Выражение,  ЭлементОтбора.Псевдоним, Контекст);	
	Если ЭлементОтбора.Выражение.Значение.Тип = "ВыражениеВсеПоля"
		Или (ЭлементОтбора.Выражение.Значение.Тип = "Разыменование"
		И ТипЗнч(ЭлементОтбора.Выражение.Значение.Элементы[ЭлементОтбора.Выражение.Значение.Элементы.ВГраница()]) = Тип("Структура")
		И ЭлементОтбора.Выражение.Значение.Элементы[ЭлементОтбора.Выражение.Значение.Элементы.ВГраница()].Тип = "ВыражениеВсеПоляИсточника") Тогда
		
		ЭлементОтбора.СодержитСимволВсеПоля = Истина;
	КонецЕсли;		
КонецПроцедуры
...
Процедура ОбработатьВыбираемыеПоляОператора(Оператор, Контекст)
	...
	Для Каждого Поле Из Оператор.ВыбираемыеПоля Цикл     
		...
		Если Поле.Выражение.Значение.Тип = "ВыражениеВсеПоля"
			...
			И Поле.Выражение.Значение.Элементы[Поле.Выражение.Значение.Элементы.ВГраница()].Тип = "ВыражениеВсеПоляИсточника") Тогда
...
//  Оператор - см. ЭлементыМоделиЗапроса.НовыйОператорЗапроса
Процедура ЗаполнитьРасчитываемыеСвойстваПоОператору(РасчитываемыеСвойства, Оператор)
	Для Каждого ВыбираемоеПоле Из Оператор.ВыбираемыеПоля Цикл
		Для Каждого КлючЗначение Из ВыбираемоеПоле.Выражение.ПоляИсточников Цикл
			РасчитываемыеСвойства.ПоляИсточников.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
		КонецЦикла;
		Для Каждого КлючЗначение Из ВыбираемоеПоле.Выражение.ПараметрыЗапроса Цикл
			РасчитываемыеСвойства.ПараметрыЗапроса.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
		КонецЦикла;
	...
	Для Каждого ВыражениеГруппировки Из Оператор.Группировка.Элементы Цикл
		Для Каждого КлючЗначение Из ВыражениеГруппировки.ПоляИсточников Цикл
			РасчитываемыеСвойства.ПоляИсточников.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
		КонецЦикла;
		Для Каждого КлючЗначение Из ВыражениеГруппировки.ПараметрыЗапроса Цикл
			РасчитываемыеСвойства.ПараметрыЗапроса.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
		КонецЦикла; 
		РасчитываемыеСвойства.ИспользуютсяИсточникиВнешнегоЗапроса = РасчитываемыеСвойства.ИспользуютсяИсточникиВнешнегоЗапроса
	...
	Для Каждого ВыражениеМодели Из Оператор.Отбор Цикл
		Для Каждого КлючЗначение Из ВыражениеМодели.ПоляИсточников Цикл
			РасчитываемыеСвойства.ПоляИсточников.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
		КонецЦикла;
		Для Каждого КлючЗначение Из ВыражениеМодели.ПараметрыЗапроса Цикл
			РасчитываемыеСвойства.ПараметрыЗапроса.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
		КонецЦикла;
		РасчитываемыеСвойства.ИспользуютсяИсточникиВнешнегоЗапроса = РасчитываемыеСвойства.ИспользуютсяИсточникиВнешнегоЗапроса
	...
	Для Каждого ЭлементОтбора Из Оператор.ОтборыСКД Цикл
		ВыражениеМодели = ЭлементОтбора.Выражение;
		Для Каждого КлючЗначение Из ВыражениеМодели.ПоляИсточников Цикл
			РасчитываемыеСвойства.ПоляИсточников.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
		КонецЦикла;
		Для Каждого КлючЗначение Из ВыражениеМодели.ПараметрыЗапроса Цикл
	...
	Для Каждого Источник Из МодельЗапросаУтилиты.ВсеИсточникиОператора(Оператор) Цикл
		Для Каждого Соединение Из Источник.Соединения Цикл
			Для Каждого КлючЗначение Из Соединение.Условие.ПоляИсточников Цикл
				РасчитываемыеСвойства.ПоляИсточников.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
			КонецЦикла;
			Для Каждого КлючЗначение Из Соединение.Условие.ПараметрыЗапроса Цикл
				РасчитываемыеСвойства.ПараметрыЗапроса.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
			КонецЦикла;
			РасчитываемыеСвойства.ИспользуютсяИсточникиВнешнегоЗапроса = РасчитываемыеСвойства.ИспользуютсяИсточникиВнешнегоЗапроса
