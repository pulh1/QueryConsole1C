...
Функция КонтекстОбработкиВыражения() Экспорт
	...
	Контекст.Вставить("РеестрПоставщиковМетаданныхТаблиц");
	ОбновитьРеестрПоставщиковМетаданныхТаблиц(Контекст);
...
Процедура ОбновитьРеестрПоставщиковМетаданныхТаблиц(Контекст)
	Контекст.РеестрПоставщиковМетаданныхТаблиц = Обработки.РеестрПоставщиковМетаданныхТаблиц.СоздатьДляОбработкиЗапроса(
		Контекст.ОписанияВременныхТаблиц,
		Контекст.ИндексЗапросаПакета);
КонецПроцедуры
...
Процедура ОбработатьОператор(Оператор, Контекст, РассчитываемыеСвойства = Неопределено)
	...
	Для Каждого Источник Из Оператор.Источники.Элементы Цикл
		...
		ИначеЕсли Источник.Источник.Тип = "ИсточникДанныхВременнаяТаблица" Тогда
			ОписаниеТаблицыПоставщика = Контекст.РеестрПоставщиковМетаданныхТаблиц.ОписаниеТаблицы(Источник.Источник.ИмяТаблицы);
			Если ОписаниеТаблицыПоставщика = Неопределено Тогда
				ОписаниеВТ = Неопределено;
			Иначе
				ОписаниеВТ = ОписаниеТаблицыПоставщика.ИсходноеОписание;
			КонецЕсли;
			Если ОписаниеВТ = Неопределено Тогда
