...
Функция ЭтоПолеТипа(ТипПоля, ЦелевойТип, УчитыватьПроизвольныйТипКакЦелевой = Истина) Экспорт
	Если ТипПоля.Типы().Количество() > 1 Тогда
		Возврат Ложь;
	ИначеЕсли (УчитыватьПроизвольныйТипКакЦелевой И ТипПоля.СодержитТип(ПроизвольныйТип()))
...
Функция ТипВыраженияУсловияКорректен(ТипЗначения) Экспорт  
	Если ТипЗначения.Типы().Количество() = 2 
		И ТипЗначения.СодержитТип(Тип("Булево"))
		И СодержитПроизвольныйТип(ТипЗначения) Тогда
			
		Возврат Истина;
	ИначеЕсли ТипЗначения.Типы().Количество() = 1 
		И (ТипЗначения.СодержитТип(Тип("Булево"))
		Или СодержитПроизвольныйТип(ТипЗначения)) Тогда
...
Процедура ПроверитьТипЗначенияВыраженияПоля(ТипЗначения, ТекстВыражения = Неопределено) Экспорт
	Если ТипЗначения <> Неопределено И ТипЗначения.СодержитТип(Тип("СписокЗначений")) Тогда
		...
		Если ТекстВыражения <> Неопределено Тогда
			...
			|" + ТекстВыражения;
		Иначе
			ТекстИсключения = ТекстВыражения + "
...
Функция СодержитСсылочныйТип(ТипЗначения) Экспорт
	Если СодержитПроизвольныйТип(ТипЗначения) Тогда
		Возврат Истина;
	КонецЕсли;
