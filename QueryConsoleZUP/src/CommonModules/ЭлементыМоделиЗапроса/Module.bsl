...
Функция НовыйЗапросВыбора(ТекущийТокен = Неопределено) Экспорт
	...
	ЭлементМодели.Вставить("ОписаниеВТ");
	ЭлементМодели.Вставить("ИспользуютсяИсточникиВнешнегоЗапроса", Ложь);
...
// * Источники - Массив из см. НовыйИсточник
// * ВыбираемыеПоля - Массив из см. НовыйПолеЗапроса
// * Отбор - Произвольный - до обработки: см. НовыйВыражениеМоделиЗапроса;
//                         после обработки: Массив из см. НовыйВыражениеМоделиЗапроса
...
// * ОтборСгруппированных - Произвольный - до обработки: см. НовыйВыражениеМоделиЗапроса;
//                                      после обработки: Массив из см. НовыйВыражениеМоделиЗапроса
// * ОтборыСКД - Массив из см. НовыйВыбираемыеПоля
Функция НовыйОператорЗапроса(ТекущийТокен = Неопределено) Экспорт
	...
	ЭлементМодели.Вставить("Источники", НовыйИсточникиЗапроса());
	...
	ЭлементМодели.Вставить("ОтборыСКД", НовыйВыбираемыеПоля());
	
	ЭлементМодели.Вставить("__ИспользуютсяИсточникиВнешнегоЗапроса", Ложь);
...
// Новый выбираемые поля.
// 
// Параметры:
//  ТекущийТокен - Неопределено - Текущий токен
// 
// Возвращаемое значение:
//  Массив из см. НовыйПолеЗапроса
Функция НовыйВыбираемыеПоля(ТекущийТокен = Неопределено) Экспорт
	ЭлементМодели = Новый Массив();
	Возврат ЭлементМодели;
КонецФункции

Функция НовыйВыражениеВсеПоля(ТекущийТокен = Неопределено) Экспорт
	ЭлементМодели = НовыйЭлементМоделиЗапроса("ВыражениеВсеПоля", ТекущийТокен);
	Возврат ЭлементМодели;
КонецФункции

Функция НовыйВыражениеВсеПоляИсточника(ТекущийТокен = Неопределено) Экспорт
	ЭлементМодели = НовыйЭлементМоделиЗапроса("ВыражениеВсеПоляИсточника", ТекущийТокен);
	Возврат ЭлементМодели;
КонецФункции
...
Функция НовыйПолеЗапроса(ТекущийТокен = Неопределено) Экспорт
	...
	ЭлементМодели.Вставить("ТипЗначения");   
	ЭлементМодели.Вставить("СодержитСимволВсеПоля", Ложь);
	Возврат ЭлементМодели;
КонецФункции

// Новый массив корневых источников запроса.
// 
// Параметры:
//  ТекущийТокен - Неопределено - токен parser constructor ABI; для массива не используется
// 
// Возвращаемое значение:
//  Массив из см. НовыйИсточник
Функция НовыйИсточникиЗапроса(ТекущийТокен = Неопределено) Экспорт
...
Функция НовыйИсточник(ТекущийТокен = Неопределено) Экспорт
	ЭлементМодели = НовыйЭлементМоделиЗапроса("Источник", ТекущийТокен);
...
// * ИмяТаблицы - Строка
// * Псевдоним - Строка
// * Параметры - Неопределено, Массив из см. НовыйВыражениеМоделиЗапроса
...
Функция НовыйСписокСоединений(ТекущийТокен = Неопределено) Экспорт
	ЭлементМодели = Новый Массив();
	Возврат ЭлементМодели;
КонецФункции
...
Функция НовыйЭлементПорядка(ТекущийТокен = Неопределено) Экспорт
	...
	ЭлементМодели.Вставить("Выражение");
	ЭлементМодели.Вставить("Направление", НовыйНаправлениеВозрастание());
	ЭлементМодели.Вставить("Иерархия", Ложь);
	Возврат ЭлементМодели;
КонецФункции
...
// Новый бинарная операция.
// 
// Параметры:
//  ТекущийТокен - Неопределено - Текущий токен
// 
// Возвращаемое значение:
//  Структура - Новый бинарная операция:
// * Тип - Строка - 
// * Операция  - Строка
// * ПраваяЧасть - Произвольный
// * ЛеваяЧасть - Произвольный
...
Функция НовыйФункцияЧастьПериодаЧислом(ИмяФункции, ТекущийТокен) Экспорт
...
Функция НовыйФункцияПредставлениеСсылки(ТекущийТокен = Неопределено) Экспорт
...
// * Тип - Строка - 
// * ИмяТаблицы - Строка
// * ВТФильтр - см. НовыйПараметрИсполняемогоПредставленияВТФильтр
// * Параметры - Массив из см. НовыйПараметрИсполняемогоПредставленияКонстанта 
...
// Новый параметр исполянего представления ВТФильтр.
// 
// Параметры:
//  ТекущийТокен - Неопределено - Текущий токен
// 
// Возвращаемое значение:
//  Структура - Новый параметр исполняемого представления ВТФильтр:
// * Тип - Строка - 
// * Поля - Массив из Строка - 
// * ЗапросВыбора - см. НовыйЗапросВыбора 
Функция НовыйПараметрИсполняемогоПредставленияВТФильтр(ТекущийТокен = Неопределено) Экспорт
	ЭлементМодели = НовыйЭлементМоделиЗапроса("ПараметрИсполняемогоПредставленияВТФильтр", ТекущийТокен); 
...
// Новый Параметр исполянего представления константа.
// 
// Параметры:
//  ТекущийТокен - Неопределено - Текущий токен
// 
// Возвращаемое значение:
//  Структура - Новый Параметр исполянего представления константа:
// * Тип - Строка -
// * Имя - Строка - 
// * Значение - Произвольный  
// * ЭтоПараметрЗапроса - Булево - 
Функция НовыйПараметрИсполняемогоПредставленияКонстанта(ТекущийТокен = Неопределено) Экспорт
	ЭлементМодели = НовыйЭлементМоделиЗапроса("ПараметрИсполняемогоПредставленияКонстанта", ТекущийТокен); 
	ЭлементМодели.Вставить("Имя", "");
	ЭлементМодели.Вставить("Значение");
...
Функция НовыйОтборИсполняемогоПредставления(ТекущийТокен = Неопределено) Экспорт
	ЭлементМодели = НовыйЭлементМоделиЗапроса("ОтборИсполняемогоПредставления", ТекущийТокен); 
	ЭлементМодели.Вставить("Выражение");
