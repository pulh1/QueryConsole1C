...
Процедура ОбработатьИсточникЗапроса(Источник, Контекст) Экспорт 
	...
	Если Контекст.Свойство("РеестрПоставщиковМетаданныхТаблиц", Реестр) Тогда
		...
		ОписаниеПредставления = СыроеОписаниеИсполняемогоПредставленияПоИмениТаблицы(
			Источник.Источник.ИмяТаблицы, Реестр, ОписаниеТаблицы);
	...
	Иначе
		Если Не СтрНачинаетсяС(
			ВРег(Источник.Источник.ИмяТаблицы), ВРег("ИсполняемоеПредставление.")) Тогда
			Возврат;
		КонецЕсли;
		ОписаниеПредставления = ПоставщикИсполняемыхПредставлений
			.ОписаниеПредставленияПоИмениТаблицы(Источник.Источник.ИмяТаблицы);
		Если ОписаниеПредставления = Неопределено Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого Параметр Из Параметры Цикл
		...
		ИначеЕсли ЭтоПараметрОтбор(Параметр.Значение) Тогда 
			Отбор = ОтборИсполняемогоПредставленияИзВыражения(ИсполняемоеПредставление, Параметр);
	...
	КонецЦикла;	
 	...
 	УстановитьПараметрыИсполняемогоПредставления(
		...
		Контекст.ОписанияВременныхТаблиц,
		Реестр);
	 
КонецПроцедуры	

Процедура УстановитьПараметрыИсполняемогоПредставления(
	ИсточникИсполняемоеПредставление,
	ОписаниеПредставления,
	Параметры,
	ОписаниеДоступныхВТ,
	РеестрПоставщиков = Неопределено) Экспорт
	ИсполняемоеПредставление = ИсточникИсполняемоеПредставление.Источник;
	...
	Для Каждого ТекущийПараметр Из Параметры Цикл
		...
		ИначеЕсли ТекущийПараметр.Тип = "ОтборИсполняемогоПредставления" Тогда 
			ОбработатьОтбор(
				ТекущийПараметр,
				ИсточникИсполняемоеПредставление,
				ОписаниеПредставления,
				ОписаниеДоступныхВТ,
				РеестрПоставщиков);
			ИсполняемоеПредставление.Отбор = ТекущийПараметр;
		КонецЕсли;
	КонецЦикла;
...
// Устанавливает параметры по имени таблицы, не передавая raw payload вызывающему коду.
Процедура УстановитьПараметрыИсполняемогоПредставленияПоИмениТаблицы(
	ИсточникИсполняемоеПредставление,
	ИмяТаблицы,
	Параметры,
	ОписаниеДоступныхВТ,
	РеестрПоставщиков) Экспорт

	ОписаниеПредставления = СыроеОписаниеИсполняемогоПредставленияПоИмениТаблицы(
		ИмяТаблицы, РеестрПоставщиков);
	Если ОписаниеПредставления = Неопределено Тогда
...
Функция ОбработатьВТФильтр(ИсполняемоеПредставление, ОписаниеПредставления, Параметр, Источник, Контекст)
...
Функция ОтборИсполняемогоПредставленияИзВыражения(ИсполняемоеПредставление, Параметр)
	ПроверитьВыраженийПараметрОтбор(
		ИсполняемоеПредставление, 
		Параметр.Значение);	
	...
	Отбор.Выражение.Значение = Параметр.Значение.ПраваяЧасть.Элементы[0];
...
Процедура ПроверитьОперандДляПараметраВТФильтр(ИсполняемоеПредставление, Псевдоним, ПараметрВТФильтр)
	...
	Если ПараметрВТФильтр.Операнд.Тип <> "СписокВыражений" Тогда
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;	
	
	Для Каждого ЭлементСписка Из ПараметрВТФильтр.Операнд.Элементы Цикл
...
Функция РазобратьСписокПараметров(Операнд)
	...
	Если Операнд.Тип <> "СписокВыражений" Тогда
...
//	КонецЕсли;
	...
	Для Каждого ЭлементСписка Из Операнд.Элементы Цикл
		Если ЭлементСписка.Тип = "Разыменование" 
...
Процедура ПроверитьВыраженийПараметрОтбор(ИсполняемоеПредставление, ПараметрОтбор)		
	Если ПараметрОтбор.Тип = "БинарнаяОперация"	Тогда
		Если ПараметрОтбор.Операция <> "=" Тогда
			ВызватьИсключение "Не корректное выражения для параметра Отбор представления " 
				+ ИсполняемоеПредставление.ИмяТаблицы + ". 
				| Операция " + ПараметрОтбор.Операция + " не поддерживается";	
		КонецЕсли;
		Если ПараметрОтбор.ПраваяЧасть.Тип <> "СписокВыражений" 
			Или ПараметрОтбор.ПраваяЧасть.Элементы.Количество() <> 1 Тогда
			ВызватьИсключение "Не корректное выражения для параметра Отбор представления " 
				+ ИсполняемоеПредставление.ИмяТаблицы + ". 
				| Не задано выражение отбора";	
		КонецЕсли;
		
	Иначе
		ВызватьИсключение "Ее корректное выражения для параметра Отбор представления " 
			+ ИсполняемоеПредставление.ИмяТаблицы + ".";	
	КонецЕсли;	
КонецПроцедуры 
...
Процедура УстановитьПараметрКонстанта(ПараметрыПредставление, Параметр, ОписаниеПараметровПоИменам, ИмяПредставления) Экспорт 		
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ОписаниеПараметра.ДоступныеЗначения)
			И ОписаниеПараметра.ДоступныеЗначения.Найти(Параметр.Значение) = Неопределено Тогда
			
			ВызватьИсключение "Не корректное выражения для параметра " + ОписаниеПараметра.Имя + " представления " 
					+ ИмяПредставления + ". 
					|Указано не допустимое значение";	
		КонецЕсли;					
	КонецЕсли;		
...
Процедура ОбработатьОтбор(
	Отбор,
	ИсточникИсполняемоеПредставление,
	ОписаниеПредставления,
	ОписаниеДоступныхВТ,
	РеестрПоставщиков = Неопределено)
	Если ОписаниеПредставления.ОписаниеОтбора = Неопределено
		Или Не ОписаниеПредставления.ОписаниеОтбора.ДоступенВПараметрах Тогда
			
		ВызватьИсключение "Для исполняемого представления " + ОписаниеПредставления.Имя + "
			|не поддерживается указание отбора в параметрах."; 
	КонецЕсли;
	
	КонтекстОбработкиОтбора = КонтекстОбработкиОтбора(
		ИсточникИсполняемоеПредставление,
		ОписаниеДоступныхВТ,
		РеестрПоставщиков);
	ОбработкаМоделиЗапроса.ОбработатьУсловие(Отбор.Выражение, КонтекстОбработкиОтбора);
	
	ВалидаторОтбора = Обработки.ПроверкаПрименимостиОтбораПосетитель.СоздатьВалидаторОтбора(
		ИсточникИсполняемоеПредставление.ИдентификаторИсточника,
		ОписаниеПредставления.Имя,
		ОписаниеПредставления.ОписаниеОтбора);
	
	ОбходМоделиЯзыкаВыражений.ОбойтиДерево(Отбор.Выражение.Значение, ВалидаторОтбора);
КонецПроцедуры 
...
// Возвращает модель исполняемого представления по нейтральному описанию таблицы.
//
// Параметры:
//  ИмяТаблицы - Строка - Полное имя таблицы.
//  РеестрПоставщиков - ОбработкаОбъект.РеестрПоставщиковМетаданныхТаблиц -
//   Реестр поставщиков таблиц.
//
// Возвращаемое значение:
//  Структура - Модель исполняемого представления:
// * Тип - Строка
// * ИмяТаблицы - Строка
// * ВТФильтр - см. ЭлементыМоделиЗапроса.НовыйПараметрИсполняемогоПредставленияВТФильтр
// * Параметры - Массив из см. ЭлементыМоделиЗапроса.НовыйПараметрИсполняемогоПредставленияКонстанта
// * Отбор - см. ЭлементыМоделиЗапроса.НовыйОтборИсполняемогоПредставления
// * Псевдоним - Строка
// * ИмяОбработчика - Строка
//  Неопределено - Если таблица не найдена или имеет другой вид источника.
...
Функция СыроеОписаниеИсполняемогоПредставленияПоИмениТаблицы(
	ИмяТаблицы, РеестрПоставщиков, ОписаниеТаблицы = Неопределено)

	Если РеестрПоставщиков = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Возврат ОписаниеТаблицы.ДанныеПоставщика;
...
Функция ИсполняемоеПредставлениеПоОписанию(ОписаниеПредставления) Экспорт 
	...
	ИсполняемоеПредставление.ИмяТаблицы = ОписаниеПредставления.ИмяТаблицы; 
...
Функция ЭтоПараметрОтбор(Параметр)
	Возврат Параметр.Тип = "БинарнаяОперация"
		И Параметр.Операция = "="
		И Параметр.ЛеваяЧасть.Тип = "Разыменование"
		И Параметр.ЛеваяЧасть.Элементы.Количество() = 1
		И ВРег(Параметр.ЛеваяЧасть.Элементы[0]) = "ОТБОР";
КонецФункции
	
Функция КонтекстОбработкиОтбора(
	ИсточникИсполняемоеПредставление,
	ОписаниеДоступныхВТ,
	РеестрПоставщиков = Неопределено) Экспорт
	...
	Контекст.ИспользуемыеТаблицы.Вставить(ВРег(ИсточникИсполняемоеПредставление.Источник.Псевдоним), ИсточникИсполняемоеПредставление);
	Контекст.ИсточникиПоИдентификаторам.Вставить(ИсточникИсполняемоеПредставление.ИдентификаторИсточника, ИсточникИсполняемоеПредставление);
	Контекст.ОписанияВременныхТаблиц = ОписаниеДоступныхВТ;
