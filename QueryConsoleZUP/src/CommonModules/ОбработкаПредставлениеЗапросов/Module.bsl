...
Процедура ОбработатьИсточникЗапроса(Источник, Контекст) Экспорт 
	...
	ОписаниеПредставления = ОписаниеПредставленияИсточника(Источник.Источник.ИмяТаблицы, Контекст);
...
Функция ОписаниеПредставленияИсточника(ИмяТаблицы, Контекст)
	Если Контекст.Свойство("РеестрПоставщиковМетаданныхТаблиц") Тогда
		ОписаниеТаблицыПоставщика = Контекст.РеестрПоставщиковМетаданныхТаблиц.ОписаниеТаблицы(ИмяТаблицы);
		Если ОписаниеТаблицыПоставщика <> Неопределено
			И ОписаниеТаблицыПоставщика.ВидПоставщика = "ИсполняемыеПредставления" Тогда
			
			Возврат ОписаниеТаблицыПоставщика.ИсходноеОписание;
		КонецЕсли;
	КонецЕсли;
	
	Возврат ПоставщикИсполняемыхПредставлений.ОписаниеПредставленияПоИмениТаблицы(ИмяТаблицы);
КонецФункции

Процедура УстановитьПараметрыИсполняемогоПредставления(ИсточникИсполняемоеПредставление, ОписаниеПредставления, Параметры, ОписаниеДоступныхВТ) Экспорт
	...
	Для Каждого ТекущийПараметр Из Параметры Цикл
		...
		ИначеЕсли ТекущийПараметр.Тип = "ОтборИсполняемогоПредставления" Тогда 
			ОбработатьОтбор(ТекущийПараметр, ИсточникИсполняемоеПредставление, ОписаниеПредставления, ОписаниеДоступныхВТ);
...
Процедура ОбработатьОтбор(Отбор, ИсточникИсполняемоеПредставление, ОписаниеПредставления, ОписаниеДоступныхВТ) 		
	...
	КонтекстОбработкиОтбора = КонтекстОбработкиОтбора(ИсточникИсполняемоеПредставление, ОписаниеДоступныхВТ);		
...
Функция КонтекстОбработкиОтбора(ИсточникИсполняемоеПредставление, ОписаниеДоступныхВТ) Экспорт  
