...
Перем МассивНачальныхСимволовИдентификаторов;
Перем МассивСимволовЦифр;

Перем КлючевыеСлова;
Перем Лексемы;
Перем КлючевыеСловаОпределямыеСкобкой;
...
Функция Инициализировать() Экспорт 
	...
	ЗаполнитьКлючевыеСловаОпределямыеСкобкой();
	ЗаполнитьЛексемы();
КонецФункции	
...
Процедура ЗаполнитьКлючевыеСловаОпределямыеСкобкой()
	КлючевыеСловаОпределямыеСкобкой = Новый Соответствие;

	КлючевыеСловаОпределямыеСкобкой.Вставить("ВЫРАЗИТЬ", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("ПОДСТРОКА", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("СУММА", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("КВАРТАЛ", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("МИНУТА", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("ДЕНЬГОДА", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("ПРЕДСТАВЛЕНИЕ", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("СЕКУНДА", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("ДЕНЬНЕДЕЛИ", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("МЕСЯЦ", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("ДОБАВИТЬКДАТЕ", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("ТИПЗНАЧЕНИЯ", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("ДЕНЬ", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("ГОД", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("НАЧАЛОПЕРИОДА", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("ЧАС", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("МИНИМУМ", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("СРЕДНЕЕ", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("ЕСТЬNULL", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("ПРЕДСТАВЛЕНИЕССЫЛКИ", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("КОНЕЦПЕРИОДА", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("НЕДЕЛЯ", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("КОЛИЧЕСТВО", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("МАКСИМУМ", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("ЗНАЧЕНИЕ", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("ТИП", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("РАЗНОСТЬДАТ", Истина);
	КлючевыеСловаОпределямыеСкобкой.Вставить("ДАТАВРЕМЯ", Истина);
КонецПроцедуры
...
Функция ЭтоНачалоИдентификатора() 
	...
	Рез = СтрНайти(НачальныеСимволыИдентификаторов, НРег(ОбрабатываемыйСимвол)) > 0;
...
Функция ЭтоНачалоИдентификатораCРешеткой() 
	Начало = ТекущаяУниверсальнаяДатаВМиллисекундах();
	...
	Рез = ОбрабатываемыйСимвол = "#" И СимволБуфера <> "" И СтрНайти(ДопустимыеСимволыИдентификаторов, НРег(СимволБуфера)) > 0;	
...
Функция СледующийТокен() Экспорт
	...
	Если ЭтоНачалоИдентификатора() Тогда
		Идекнтификатор = ИдентификаторИзСтроки();
		Токен = ТокенСлово(Идекнтификатор);
	ИначеЕсли ЭтоНачалоИдентификатораCРешеткой() Тогда
		Идекнтификатор = ИдентификаторСРешеткойИзСтроки();
...
МассивНачальныхСимволовИдентификаторов = МассивКодовСимволов(НачальныеСимволыИдентификаторов);
