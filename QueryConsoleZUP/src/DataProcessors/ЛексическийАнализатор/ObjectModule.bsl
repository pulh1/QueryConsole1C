...
Процедура УстановитьОбрабатываемыйТекст(Текст) Экспорт
	...
	СостояниеПозицииEOF = НовоеСостояниеПозиции();
...
Функция СформироватьШаблонТокенизации()
	...
	СимволИдентификатора = "[A-Za-zА-Яа-яЁё_0-9]";
	ИдентификаторСРешеткой = "#" + СимволИдентификатора + "+";
	Идентификатор = НачалоИдентификатора + СимволИдентификатора + "*";
...
Функция ЭтоНезначащееСовпадение(Совпадение)
	...
	Возврат ПервыйСимвол = "/"
		...
		И Сред(
			ИсходныйТекст, Совпадение.НачальнаяПозиция + 1, 1) = "/";
КонецФункции

Функция ТокенИзСовпадения(Совпадение, Позиция)
	ПервыйСимвол = Сред(
		ИсходныйТекст, Совпадение.НачальнаяПозиция, 1);
	Лексема = Сред(
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ВызватьЛексическуюОшибку(Совпадение.НачальнаяПозиция);
	КонецПопытки;

	Возврат Токен;
КонецФункции

Процедура ПродвинутьПослеСовпадения(Совпадение)
	ОжидаемаяПозиция =
		Совпадение.НачальнаяПозиция + Совпадение.Длина;
	ИндексСледующегоСовпадения = ИндексСледующегоСовпадения + 1;
КонецПроцедуры

Процедура ВызватьЛексическуюОшибку(НачальнаяПозиция)
	Позиция = ПозицияИсточника(
		НачальнаяПозиция, ПереводыСтрок, СостояниеПозиции);
	...
	ВызватьИсключение ПолноеСообщение;
КонецПроцедуры
