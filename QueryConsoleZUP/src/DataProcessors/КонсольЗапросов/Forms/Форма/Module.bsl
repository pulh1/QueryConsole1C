...
&НаКлиенте
Процедура ПолучитьИсполняемыйКод(Команда)
	Код = ПолучитьИсполняемыйКодНаСервере(ТекстЗапроса.ПолучитьТекст());
	ТекстовыДокумент = Новый ТекстовыйДокумент();
	ТекстовыДокумент.УстановитьТекст(Код);
	
	ТекстовыДокумент.Показать();
КонецПроцедуры

&НаКлиенте
Процедура ПолучитьКодЗапросаДляСКД(Команда)
	Код = ПолучитьКодЗапросаСКДНаСервере(ТекстЗапроса.ПолучитьТекст());
	ТекстовыДокумент = Новый ТекстовыйДокумент();
	ТекстовыДокумент.УстановитьТекст(Код);
...
Процедура ИнициализацияТипаИЗначенияПараметра(ТекущийПараметр, ТекущийТип)
	...
	Если ТипСтрока = "dateTime" Тогда
		ТекущийПараметр.Тип  = "Дата";
	КонецЕсли;	
...
Процедура ПослеЗакрытияРасширенногоКонструктораЗапроса(Результат, СтруктураПараметров) Экспорт
	...
	Текст = ТекстОтредактированногоЗапроса(Результат.АдресМоделиЗапроса);
	ПослеЗакрытияКонструктораЗапроса(Текст, СтруктураПараметров);
КонецПроцедуры
...
&НаСервере
Функция ПолучитьИсполняемыйКодНаСервере(Знач ТекстЗапросаВФорме)
	ПараметрыЗапроса = Новый Структура();
	Для каждого ЭлементПараметр Из Объект.Параметры Цикл
	...
	Возврат ИсполнительПредставлений.ПолучитьИсполняемыйКод(ТекстЗапросаВФорме, ПараметрыЗапроса);
КонецФункции

&НаСервере
Функция ПолучитьКодЗапросаСКДНаСервере(Знач ТекстЗапросаВФорме)
	ПараметрыЗапроса = Новый Структура();
	Для каждого ЭлементПараметр Из Объект.Параметры Цикл
		СтрокаЗначение = ЭлементПараметр.Значение;
		Если ПустаяСтрока(СтрокаЗначение) Тогда
	...
	Возврат ИсполнительПредставлений.ПолучитьТекстЗапросаДляСКД(ТекстЗапросаВФорме, ПараметрыЗапроса);
КонецФункции
