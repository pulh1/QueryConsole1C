...
Процедура РазыменованиеПриВыходе(Выражение) Экспорт
	...
	СемантическийАнализВыраженийУтилиты.ОбработатьОбращениеКПолюИсточника(
		Выражение.Элементы, Контекст);
	...
	Для Индекс = 0 По Выражение.Элементы.ВГраница() Цикл
		...
		ИначеЕсли ТипЗнч(ЭлементРазыменования) = Тип("Строка") Тогда
			Если ОписаниеПоля <> Неопределено
				И ОписаниеПоля.ВидПоля = МетаданныеТаблицЗапроса.ВидПоляВложеннаяТаблица() Тогда

				ОписаниеПоля = МетаданныеТаблицЗапроса.НайтиПоле(
					ОписаниеПоля.Поля, ЭлементРазыменования);
				Если ОписаниеПоля = Неопределено Тогда
					ВызватьИсключение "Поле не найдено " + ЭлементРазыменования;
				КонецЕсли;
				Если ОписаниеПоля.ВидПоля = МетаданныеТаблицЗапроса.ВидПоляОбычное() Тогда
					ТекущийТип = ОписаниеПоля.ТипЗначения;
					ОписаниеПоля = Неопределено;
				Иначе
					ТекущийТип = Неопределено;
				КонецЕсли;
			Иначе
				Если ТекущийТип = Неопределено Тогда
					ВызватьИсключение "Поле не найдено " + ЭлементРазыменования;
				КонецЕсли;
				ТекущийТип = СемантическийАнализВыраженийУтилиты
					.ТипЭлементаРазыменованияИзОписанияТипов(
						ЭлементРазыменования, ТекущийТип, Контекст);
			КонецЕсли;
		...
		ИначеЕсли ЭлементРазыменования.Тип = "ПоляВложеннойТаблицы" Тогда
			Если ОписаниеПоля = Неопределено
				Или ОписаниеПоля.ВидПоля <> МетаданныеТаблицЗапроса.ВидПоляВложеннаяТаблица() Тогда

				ВызватьИсключение "Список полей не найден";
			КонецЕсли;
			СтэкТипов.Добавить(Новый ОписаниеТипов("ТаблицаЗначений"));
			Возврат;
		ИначеЕсли ЭлементРазыменования.Тип = "ПолеИсточника" Тогда
			Источник = Контекст.ИсточникиПоИдентификаторам.Получить(
				ЭлементРазыменования.ИдентификаторИсточника);
			ОписаниеПоля = СемантическийАнализВыраженийУтилиты.НайтиПолеВИсточнике(
				ЭлементРазыменования.ИмяПоля, Источник.Источник, Контекст);
			Если ОписаниеПоля = Неопределено Тогда
				ВызватьИсключение "Поле " + Источник.Источник.Псевдоним + "."
					+ ЭлементРазыменования.ИмяПоля + " не найдено";
			КонецЕсли;
			Если ОписаниеПоля.ВидПоля = МетаданныеТаблицЗапроса.ВидПоляОбычное() Тогда
				ТекущийТип = ОписаниеПоля.ТипЗначения;
				ОписаниеПоля = Неопределено;
			...
			РассчитываемыеСвойства.ПоляИсточников.Вставить(
				Строка(ЭлементРазыменования.ИдентификаторИсточника)
					+ "." + ВРег(ЭлементРазыменования.ИмяПоля),
				ЭлементРазыменования.ИмяПоля);
			
			Если Контекст.ИдентификаторыИсточниковВнешнегоЗапроса.Получить(
				ЭлементРазыменования.ИдентификаторИсточника) <> Неопределено Тогда

				РассчитываемыеСвойства.ИспользуютсяИсточникиВнешнегоЗапроса = Истина;
			КонецЕсли;
		ИначеЕсли ЭлементРазыменования.Тип = "ВыражениеВсеПоляИсточника" Тогда
			Продолжить;
		Иначе
	...
	Если ОписаниеПоля <> Неопределено
		...
		ТекущийТип = Новый ОписаниеТипов("ТаблицаЗначений");
	КонецЕсли;
	Если ТекущийТип = Неопределено Тогда
		ВызватьИсключение "Поле "
			+ ГенерацияТекстовЗапросов.ВыражениеВСтроку(Выражение, , Новый Массив())
			+ " не найдено";
	КонецЕсли;
