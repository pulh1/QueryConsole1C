...
Перем ЗапросВыбора; 
Перем МассивСтрок;
Перем ГенераторТекстовВыражений; //см. Обработка.ГенераторТекстовВыражений
...
Процедура ЗапросВыбораВМассивСтрок(Знач Табуляции = "") Экспорт
	...
	Если ЗапросВыбора.Порядок.Количество() > 0 Тогда
		...
		СписокВыраженийУпорядочиванияВМассивСтрок(ЗапросВыбора.Порядок, ТекущиеТабуляции);
	КонецЕсли;
...
//	МассивСтрок.Добавить(Табуляции);
	...
	ГенераторТекстовВыражений.УстановитьИсточники(
	...
	Если ЗначениеЗаполнено(ОператорЗапроса.Отбор) Тогда
		...
		Если ТипЗнч(ОператорЗапроса.Отбор) = Тип("Массив") Тогда
			...
			ВыражениеВМассивСтрок(Условие, ТекущиеТабуляции);
		Иначе
			ВыражениеВМассивСтрок(ОператорЗапроса.Отбор, ТекущиеТабуляции);	
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ОператорЗапроса.ОтборыСКД) Тогда
		ОтборСКДВМассивСтрок(ОператорЗапроса.ОтборыСКД, Табуляции)
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОператорЗапроса.ОтборСгруппированных) Тогда
		...
		Если ТипЗнч(ОператорЗапроса.ОтборСгруппированных) = Тип("Массив") Тогда
			...
			ВыражениеВМассивСтрок(Условие, ТекущиеТабуляции);
		Иначе
			ВыражениеВМассивСтрок(ОператорЗапроса.ОтборСгруппированных, ТекущиеТабуляции);	
		КонецЕсли;
	КонецЕсли;				
КонецПроцедуры	

// Отбор СКДВМассив строк.
// 
// Параметры:
// 	ОтборыСКД - см. ЭлементыМоделиЗапроса.НовыйВыбираемыеПоля
//  Табуляции - Строка
Процедура ОтборСКДВМассивСтрок(ОтборыСКД, Знач Табуляции)
	МассивСтрок.Добавить(Символы.ПС);
	МассивСтрок.Добавить(Табуляции);
	МассивСтрок.Добавить("{ГДЕ");	
	МассивСтрок.Добавить(Символы.ПС);
	...
	Для Каждого Отбор Из ОтборыСКД Цикл
		...
		КонецЕсли;	
		МассивСтрок.Добавить(ТекущиеТабуляции);	
		ВыражениеВМассивСтрок(Отбор.Выражение, ТекущиеТабуляции);
		
		Если ЗначениеЗаполнено(Отбор.Псевдоним) Тогда
			МассивСтрок.Добавить(" КАК ");
			МассивСтрок.Добавить(Отбор.Псевдоним);
		КонецЕсли;
	КонецЦикла;
	МассивСтрок.Добавить("}")
КонецПроцедуры	
...
Процедура ПоляОператораВМассивСтрок(ИндексОператора, Табуляции)
	...
	Для Каждого Поле Из ЗапросВыбора.Операторы[ИндексОператора].ВыбираемыеПоля Цикл
		Если Не Поле.СодержитСимволВсеПоля Тогда
			Продолжить;
		КонецЕсли;
		Если ЭтоПервоеПоле Тогда
			ЭтоПервоеПоле = Ложь;
		Иначе
			...
			МассивСтрок.Добавить(Символы.ПС);
		КонецЕсли;		
		Если Поле.Выражение.Значение.Тип = "ВыражениеВсеПоля" Тогда 
			МассивСтрок.Добавить("*");	
		Иначе
			ВыражениеВМассивСтрок(Поле.Выражение, ТекущиеТабуляции);
		КонецЕсли;
...
Процедура ИтогиВМассивСтрок(Табуляции)
	...
	Для Каждого КонтрольнаяТочка Из ЗапросВыбора.КонтрольныеТочкиИтогов Цикл
		...
		МассивСтрок.Добавить(Символы.ПС);
		МассивСтрок.Добавить(ТекущиеТабуляции);
		ВыражениеВМассивСтрок(КонтрольнаяТочка.Выражение, ТекущиеТабуляции);
		...
		Если ЗначениеЗаполнено(КонтрольнаяТочка.ТипДополненияПериодами) Тогда
			...
			Если КонтрольнаяТочка.НачалоПериодаДополнения <> Неопределено Тогда
				ВыражениеВМассивСтрок(КонтрольнаяТочка.НачалоПериодаДополнения, ТекущиеТабуляции);
			КонецЕсли;	
			МассивСтрок.Добавить(", ");
			Если КонтрольнаяТочка.КонецПериодаДополнения <> Неопределено Тогда
				ВыражениеВМассивСтрок(КонтрольнаяТочка.КонецПериодаДополнения, ТекущиеТабуляции);
			КонецЕсли;	
...
Процедура ИсточникиОператораЗапросВМассивСтрок(Источники, Знач Табуляции)
	...
	ВсеИсточники = МодельЗапросаУтилиты.ВсеИсточники(Источники);
	ИсточникиПоИдентификаторам = МодельЗапросаУтилиты.МассивСтруктурВСоответствие(
		ВсеИсточники, "ИдентификаторИсточника");
	
	ОбработанныеИсточники = Новый Соответствие();
	Для Каждого ТекущийИсточник Из МодельЗапросаУтилиты.СортироватьИсточникиПоИерархии(Источники) Цикл
		...
		Если ОбработанныеИсточники.Количество() <> ВсеИсточники.Количество() Тогда
			МассивСтрок.Добавить(",");
			МассивСтрок.Добавить(Символы.ПС);
			МассивСтрок.Добавить(Табуляции);
		КонецЕсли;
...
//	МассивСтрок.Добавить(ТекущиеТабуляции);
	...
	ИначеЕсли Источник.Источник.Тип = "ИсточникДанныхТаблица" Тогда 
		...
		Если Источник.Источник.Параметры <> Неопределено Тогда
			...
			ГенераторТекстовВыражений.МассивВыраженийВМассивСтрок(Источник.Источник.Параметры, Табуляции); 
			ГенераторТекстовВыражений.УстановитьВыводПсевдонимовИсточников(Истина);
			МассивСтрок.Добавить(")");
		КонецЕсли;		
	...
	Если ЗначениеЗаполнено(Источник.Источник.Псевдоним) Тогда
		МассивСтрок.Добавить(" КАК ");
...
// Параметры:
//  МассивСтрок - Массив из Строка
//  Параметры - Массив из см. ЭлементыМоделиЗапроса.НовыйПараметрИсполняемогоПредставленияВТФильтр, см. НовыйПараметрИсполняемогоПредставленияОтбор -
//  Источники - Соответствие из УникальныйИдентификатор, Строка
//  Табуляции - Строка
Процедура ПараметрыИсполняемогоПредставленияВМассивСтрок(Параметры, ВТФильтр, Отбор, Знач Табуляции)
	ТекущиеТабуляции = Табуляции + Символы.Таб;
	
	Если ВТФильтр <> Неопределено Тогда
		МассивСтрок.Добавить(Символы.ПС);
		МассивСтрок.Добавить(ТекущиеТабуляции);
		Операнд = ЭлементыМоделиЗапроса.НовыйСписокВыражений();
		Для Каждого Поле Из ВТФильтр.Поля Цикл
			...
			Операнд.Элементы.Добавить(Разыменование); 
		КонецЦикла;
		ВыражениеВМассивСтрок(Операнд, ТекущиеТабуляции);
	...
	Для Каждого Параметр Из Параметры Цикл
		...
		КонецЕсли;	
		МассивСтрок.Добавить(ТекущиеТабуляции);
		ВыражениеВМассивСтрок(Выражение, ТекущиеТабуляции);
	...
	Если Отбор <> Неопределено Тогда
		Если Параметры.Количество() > 0
			Или ВТФильтр <> Неопределено Тогда
		...
		Выражение = ЭлементыМоделиЗапроса.НовыйБинарнаяОперация();
		Выражение.ЛеваяЧасть = ЭлементыМоделиЗапроса.НовыйРазыменование();
		Выражение.ЛеваяЧасть.Элементы.Добавить("Отбор");
		Выражение.Операция = "=";
		Выражение.ПраваяЧасть = ЭлементыМоделиЗапроса.НовыйСписокВыражений();
		Выражение.ПраваяЧасть.Элементы.Добавить(Отбор.Выражение);
		
		ГенераторТекстовВыражений.УстановитьВыводПсевдонимовИсточников(Ложь);
		ГенераторТекстовВыражений.ВыражениеВМассивСтрок(Выражение, ТекущиеТабуляции + Символы.Таб);
		ГенераторТекстовВыражений.УстановитьВыводПсевдонимовИсточников(Истина);
	КонецЕсли;
...
Процедура СоединенияИсточникаВМассивСтрок(Соединения, ИсточникиПоИдентификаторам, ОбработанныеИсточники, Табуляции)
	...
	Для Каждого ТекущееСоединение Из Соединения Цикл
		...
		Источник = ТекущееСоединение.Источник;
		...
		МассивСтрок.Добавить(ТекущиеТабуляции);
		МассивСтрок.Добавить("ПО ");
		ВыражениеВМассивСтрок(ТекущееСоединение.Условие, ТекущиеТабуляции);
...
Процедура ВыражениеВМассивСтрок(Выражение, Знач ТекущиеТабуляции)
