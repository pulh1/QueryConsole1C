...
&НаКлиенте
Процедура СгенерироватьFeatureФайлыПроверкиГенерациияКода(Команда)
	...
	Для Каждого Файл Из ФайлыЗапросов Цикл
		СгенерироватьFeatureФайлПроверкиГенерациияКодаНаКлиенте(Файл.ПолноеИмя);
	КонецЦикла;	
КонецПроцедуры

&НаКлиенте
Процедура СгенерироватьFeatureФайлПроверкиГенерациияКода(Команда)
	Файл = Новый Файл(ПутьКФайлу);
	ИмяФайлаИсходника = Файл.ИмяБезРасширения;
	
	СгенерироватьFeatureФайлПроверкиГенерациияКодаНаКлиенте(ПутьКФайлу);
КонецПроцедуры
...
&НаКлиенте
Процедура СгенерироватьFeatureФайлПроверкиГенерациияКодаНаКлиенте(ПолноеИмяФайла)
	Файл = Новый Файл(ПолноеИмяФайла);
	ИмяФайлаИсходника = Файл.ИмяБезРасширения;
	
	ДвоичныеДанные = Новый ДвоичныеДанные(ПолноеИмяФайла);
	
	КодЭталон = РезультатГенерацииКода(ДвоичныеДанные);
	
	ИмяФайлаЭталона = "files\" + ИмяФайлаИсходника + "КодЭталон.txt";
	ИмяФайлаЗапроса = "files\" + ИмяФайлаИсходника + "Запрос.q1c";
	
	ТекстовыйДокумент = Новый ТекстовыйДокумент();
	ТекстовыйДокумент.УстановитьТекст(КодЭталон);
	ТекстовыйДокумент.Записать(ПутьККаталогуТестов + "\" + ИмяФайлаЭталона);
	ДвоичныеДанные.Записать(ПутьККаталогуТестов + "\" + ИмяФайлаЗапроса);
	
	FeatureФайл = FeatureФайлПроверкиГенерацииКода(ИмяФайлаИсходника, ИмяФайлаЗапроса, ИмяФайлаЭталона);
	FeatureФайл.Записать(ПутьККаталогуТестов + "\" + ИмяФайлаИсходника + ".feature");	
КонецПроцедуры
...
&НаСервере
Функция РезультатГенерацииКода(Знач ДвоичныеДанные)
	ТекстЗапроса = ТекстЗапросаИзФайлаНаСервере(ДвоичныеДанные);
	ПараметрыЗапроса = ПараметрыЗапросаИзФайлаНаСервере(ДвоичныеДанные);
	
	Возврат ИсполнительПредставлений.ПолучитьИсполняемыйКод(ТекстЗапроса, ПараметрыЗапроса);
КонецФункции
...
&НаКлиенте
Функция FeatureФайлПроверкиГенерацииКода(ИмяЗапроса, ИмяФайлаЗапроса, ИмяФайлаЭталона)
	Текст = "Функционал: Выполнение запроса """ + ИмяЗапроса + """ в консоли
	|
	|Как Администратор я хочу
	|выполнить запрос в конструкторе
	...
	|	Тогда Сгенерированный код соответствует эталону """ + ИмяФайлаЭталона + """";
