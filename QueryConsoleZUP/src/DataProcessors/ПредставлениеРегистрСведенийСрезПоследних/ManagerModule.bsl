...
Функция Описание(ИмяРегистра) Экспорт
	...
	Описание.Подсказка = "Обеспечивает доступ к методу ""ЗарплатаКадрыПериодическиеРегистры.СоздатьВТИмяРегистраСрезПоследних"".
...
Функция Исполнить(ПараметрыВыполнения, Запрос) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыВыполнения.ИмяВТРезультат) Тогда
		...
		Возврат Неопределено;
	Иначе
		Запрос = ЗарплатаКадрыПериодическиеРегистры.ЗапросВТИмяРегистраСрез(
			ИмяРегистра,
			ПараметрыВыполнения.ТолькоРазрешенные, 
			ОписаниеВТФильтр, 
			ПараметрыПостроения, 
