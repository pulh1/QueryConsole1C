...
&НаКлиенте
Процедура ЗавершитьРедактированиеОтбораСКД(РезультатРедактирования, ДополнительынеПараметры) Экспорт
	Если РезультатРедактирования = Неопределено
		Или Не РезультатРедактирования.Модифицированность Тогда
	...
	Иначе
		...
		Строка = ОтборКомпоновкиДанных.НайтиСтроки(
			Новый Структура("Индекс", ДополнительынеПараметры.ИндексУсловия))[0];			
	КонецЕсли;
