...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонтекстМетаданныхUI =
		КонструкторЗапросовФормы.НовыйКонтекстМетаданныхUI();
	ЗаполнитьСписокДоступныхТаблиц(КонтекстМетаданныхUI);
...
Процедура ОператорЗапросаВДанные(ОператорЗапроса)    	
	...
	ДобавитьТаблицыЗапроса(ОператорЗапроса.Источники);
...
&НаСервере
Процедура ЗаполнитьСписокДоступныхТаблиц(КонтекстМетаданныхUI)

	КонструкторЗапросовФормы.ЗаполнитьКорневыеСтрокиКаталога(
		ДоступныеТаблицы,
		КонтекстМетаданныхUI);
...
Процедура ЗаполнитьСписокДоступныхВременныхТаблиц(ОписаниеДоступныхВТ)  
	...
	Если ОписаниеДоступныхВТ.Количество() > 0 Тогда
		КонтекстМетаданныхUI =
			КонструкторЗапросовФормы.НовыйКонтекстМетаданныхUI();
		КонструкторЗапросовФормы.ДобавитьЭлементВДеревоТаблиц(
			ДоступныеТаблицы,
			НовыйГруппаДостпныхВременныхТаблиц(),
			КонтекстМетаданныхUI);
	КонецЕсли;		
КонецПроцедуры	
...
Процедура ЗаполнитьРазделГруппировка(ОператорЗапроса)
	...
	КонструкторЗапросовФормы.ДобавитьТаблицыЗапроса(
		Узел,
		ОператорЗапроса.Источники,
		КонтекстМетаданныхUI);
КонецПроцедуры	
...
Процедура ЗаполнитьРазделИтоги(ЗапросВыбора) 
	...
	КонтекстМетаданныхUI =
		КонструкторЗапросовФормы.НовыйКонтекстМетаданныхUI();
	Узел = КонструкторЗапросовФормы.ДобавитьЭлементВДеревоТаблиц(
		ПоляДоступныеДляИтогов,
		ГруппаВсеПоля,
		КонтекстМетаданныхUI);

	КонструкторЗапросовФормы.ДобавитьТаблицыЗапроса(
		Узел,
		ЗапросВыбора.Операторы[0].Источники,
		КонтекстМетаданныхUI);
КонецПроцедуры	
...
Процедура ЗаполнитьПоляДоступныеДляОтбора(ОператорЗапроса)
	...
	КонтекстМетаданныхUI =
		КонструкторЗапросовФормы.НовыйКонтекстМетаданныхUI();
	КонструкторЗапросовФормы.ДобавитьТаблицыЗапроса(
		ПоляДоступныеДляОтбора,
		ОператорЗапроса.Источники,
		КонтекстМетаданныхUI);
КонецПроцедуры

&НаСервере
Процедура ДобавитьТаблицыЗапроса(ИсточникиДанных)
...
Процедура УдалитьИсточник(ИндексИсточника)
	...
	Модель = ЗавершитьРедактированиеМодели(Построитель);
	ДобавитьТаблицыЗапроса(ТекущийОператорЗапроса(Модель).Источники);
...
Процедура ПриДобавленииИсточника(МодельЗапроса)
	...
	СтрокиДереваПоПсевданимам = Новый Соответствие;
	УзелИсточника = КонструкторЗапросовФормы.ДобавитьТаблицу(Источники, ИстчоникОператора, КонтекстМетаданныхUI);
