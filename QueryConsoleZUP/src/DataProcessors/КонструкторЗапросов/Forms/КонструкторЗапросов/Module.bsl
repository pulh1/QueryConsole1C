...
Процедура ИзменитьИсточникЗапроса(Команда)
	...
	ИначеЕсли Элементы.Источники.ТекущиеДанные.ТипЗначения = "ИсточникДанныхВременнаяТаблица" Тогда 
		ПараметрыРедактора = ПараметрыОткрытияРедактораОписанияВТ(Элементы.Источники.ТекущиеДанные.Индекс);
		
		Оповещение = Новый ОписаниеОповещения("ЗавершитьРедактированиеОписанияВТ", 
			ЭтотОбъект, Новый Структура("ИндексИсточника", Элементы.Источники.ТекущиеДанные.Индекс));
...
Функция ПараметрыОткрытияРедактораОписанияВТ(ИндексИсточника = Неопределено)
	ПараметрыРедактора = Новый Структура("АдресСпискаПолей, ИмяТаблицы");
	...
	ИдентификаторИсточника = ТекущийОператор.Источники.Элементы[ИндексИсточника].ИдентификаторИсточника;
	Если Источник.Тип <> "ИсточникДанныхВременнаяТаблица" Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ПоляВТ = Новый Массив();
	Если Источник.ОписаниеВТ = Неопределено Тогда
		ПоляИсточника = МодельЗапросаУтилиты.ИспользуемыеПоляИсточников(ТекущийЗапрос, ИндексОператораЗапроса)
			.Получить(ИдентификаторИсточника);
			
		Если ПоляИсточника <> Неопределено Тогда
			Для Каждого КлючЗначение Из ПоляИсточника Цикл
				Поле = Новый Структура("Имя, ТипЗначения");
				Поле.Имя = КлючЗначение.Значение;
				Поле.ТипЗначения = МодельЗапросаТипы.ОписаниеПроивзольногоТипа();
				
				ПоляВТ.Добавить(ПоляВТ);	
			КонецЦикла;
		КонецЕсли;
		ИмяВТ = Источник.ИмяТаблицы;	
	Иначе
