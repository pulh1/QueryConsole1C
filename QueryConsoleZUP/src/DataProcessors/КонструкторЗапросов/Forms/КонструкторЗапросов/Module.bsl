...
&НаКлиенте
Процедура ПолучатьОбщиеИтогиПриИзменении(Элемент)
	ПолучатьОбщиеИтогиПриИзмененииНаСервере();
КонецПроцедуры
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыГруппируемыеПоля

&НаКлиенте
Процедура ГруппируемыеПоляПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, ЭтоГруппа, Параметр)
	Отказ = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ГруппируемыеПоляПередУдалением(Элемент, Отказ)
	Отказ = Истина;
	УдалитьВыбранныеГруппировкиНаСервере();
КонецПроцедуры
...
Процедура УсловияПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, ЭтоГруппа, Параметр)
	...
	Если Копирование Тогда
		ПараметрыРедактораВыражений.ТекстВыражения = Элементы.Условия.ТекущиеДанные.Условие;
	КонецЕсли;
...
&НаКлиенте
Процедура АгрегируемыеПоляПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, ЭтоГруппа, Параметр)
	Отказ = Истина;
КонецПроцедуры

&НаКлиенте
Процедура АгрегируемыеПоляПередУдалением(Элемент, Отказ)
	Отказ = Истина;
	УдалитьАгрегированиеДляВыбранныхПолейНаСервере();
КонецПроцедуры
...
Процедура ПоляДляУпорядочиванияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	 ДобавитьУпорядочиваниеНаСервере(ВыбраннаяСтрока);
КонецПроцедуры
...
&НаКлиенте
Процедура КонтрольныеТочкиИтоговДополнениеПериодаПриИзменении(Элемент)
	ИзменитьСвойстваКонтрольнойТочкиИтогов(Элементы.КонтрольныеТочкиИтогов.ТекущаяСтрока);
КонецПроцедуры
...
&НаКлиенте
Процедура ДобавитьВсеГруппировки(Команда)
	ДобавитьВсеГруппировкиНаСервере();
КонецПроцедуры
...
&НаКлиенте
Процедура ДобавитьАгрегированиеДляВсехПолей(Команда)
	ДобавитьАгрегированиеДляВсехПолейНаСервере();
КонецПроцедуры
...
Процедура УдалитьАгрегированиеДляВсехПолей(Команда)
	Если АгрегируемыеПоля.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура СдвинутьИндексВверх(Команда)
	СдвинутьИндекс(-1)
КонецПроцедуры
...
&НаКлиенте
Процедура СдвинутьЭлементПорядкаВниз(Команда)
	СдвинутьЭлементПорядка(1);
КонецПроцедуры
...
Процедура ЗаполнитьРазделОбъединенияПсевдонимы(ЗапросВыбора)
	...
	Для Каждого Оператор Из ЗапросВыбора.Операторы Цикл  
		...
		ГенераторТекстов.УстановитьИсточники(МодельЗапросаУтилиты.ВсеИсточникиОператора(Оператор));
...
Процедура ЗаполнитьРазделГруппировка(ОператорЗапроса)
	...
	ГенераторТекстов = ГенерацияТекстовЗапросов.СоздатьГенераторТекстовВыражений();
	ГенераторТекстов.УстановитьИсточники(МодельЗапросаУтилиты.ВсеИсточникиОператора(ОператорЗапроса));
...
Процедура ЗаполнитьРазделИтоги(ЗапросВыбора) 
	...
	ГенераторТекстов = ГенерацияТекстовЗапросов.СоздатьГенераторТекстовВыражений();
	ГенераторТекстов.УстановитьИсточники(МодельЗапросаУтилиты.ВсеИсточникиОператора(ЗапросВыбора.Операторы[0]));
	...
	Для Каждого КонтрольнаяТочка Из ЗапросВыбора.КонтрольныеТочкиИтогов Цикл 
		...
		СтрокаТаблицы.ДополнениеПериода = КонтрольнаяТочка.ТипДополненияПериодами;
	...
	ПолучатьОбщиеИтоги = ЗапросВыбора.ОбщиеИтоги;
	...
	Если ЗапросВыбора.Операторы[0].Источники.Количество() > 0 Тогда
		ГруппаВсеПоля.Пустой = Ложь;
	КонецЕсли;	 
...
Процедура ЗаполнитьСоединения(ИсточникиДанных)
	...
	ГенераторТекстов = ГенерацияТекстовЗапросов.СоздатьГенераторТекстовВыражений();
	ВсеИсточники = МодельЗапросаУтилиты.ВсеИсточники(ИсточникиДанных);
	ГенераторТекстов.УстановитьИсточники(ВсеИсточники);
	
	ОбработанныеИсточники = Новый Соответствие;		
	Для Каждого Источник Из МодельЗапросаУтилиты.СортироватьИсточникиПоИерархии(ИсточникиДанных) Цикл
	...
	Для Каждого Источник Из ВсеИсточники Цикл
		Для Каждого Соединение Из Источник.Соединения Цикл
			Если Соединение.Опционально Тогда
				ОпциональныеИсточники.Вставить(Соединение.Источник.ИдентификаторИсточника, Истина);
			КонецЕсли;	
		КонецЦикла;
	КонецЦикла;
...
Процедура ДобавитьСоединение(УзелПриемник, ОписаниеСоединения, СтрокиДереваПоПсевданимам, 
	ОбработанныеИсточники, ГенераторТекстов)
	
	Источник = ОписаниеСоединения.Источник;
...
Процедура УстановитьУсловиеСвязи(ПсевдонимВладельцаСвязи, ПсевдонимПрисоединяемойТаблицы, ТекстВыражения);
	...
	Соединение = МодельЗапросаУтилиты.НайтиСоединение(
		Оператор, ПсевдонимПрисоединяемойТаблицы, ПсевдонимВладельцаСвязи);
...
&НаСервере
Процедура ДобавитьУпорядочиваниеНаСервере(ИдентификаторВыбраннойСтроки)
...
&НаСервере
Процедура ПолучатьОбщиеИтогиПриИзмененииНаСервере()
	Построитель = ПостроительМоделиЗапроса().УстановитьПолучениеОбщихИтогов(ПолучатьОбщиеИтоги);
	ЗавершитьРедактированиеМодели(Построитель);
КонецПроцедуры
...
Процедура ИзменитьСвойстваКонтрольнойТочкиИтогов(ИдентификаторСтроки)
	...
	ВариантДополненияПериодами = СтрокаТаблицы.ДополнениеПериода;
...
Процедура ДобавитьГруппировкуНаСервере(ИдентификаторВыбраннойСтроки)
	...
	Если КонструкторЗапросовКлиентСерверФормы.СодержитТип(СтрокаТаблицы.ТипЗначения, "ГруппаВсеПоля") Тогда
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура ДобавитьВсеГруппировкиНаСервере()
	ВыраженияДляДобавления = Новый Массив();
	Для Каждого СтрокаТаблицы Из ПоляДляГруппировки.ПолучитьЭлементы() Цикл
		Если СтрокаТаблицы.ТипЗначения = "ПолеЗапроса" Тогда
			Выражение = СтрокаТаблицы.Представление;
			ВыраженияДляДобавления.Добавить(Выражение);
		КонецЕсли;
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(ВыраженияДляДобавления) Тогда
		Возврат;
	КонецЕсли;
	
	Построитель = ПостроительМоделиЗапроса();
	Для Каждого Выражение Из ВыраженияДляДобавления Цикл
		Построитель.ДобавитьГруппировку(Выражение);
	КонецЦикла;    
	Модель = ЗавершитьРедактированиеМодели(Построитель);
...
&НаСервере
Процедура ДобавитьАгрегированиеДляВсехПолейНаСервере()
	СтрокиДляДобавления = Новый Массив();
	Для Каждого СтрокаТаблицы Из ПоляДляГруппировки.ПолучитьЭлементы() Цикл
		Если СтрокаТаблицы.ТипЗначения = "ПолеЗапроса"
			И СтрокаТаблицы.ТипЗначенияПоля.СодержитТип(Тип("Число"))
			И СтрокаТаблицы.ТипЗначенияПоля.Типы().Количество() = 1 Тогда
			СтрокиДляДобавления.Добавить(СтрокаТаблицы);
		КонецЕсли;
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(СтрокиДляДобавления) Тогда
		Возврат;
	КонецЕсли;
	
	Построитель = ПостроительМоделиЗапроса();
	
	Для Каждого СтрокаТаблицы Из СтрокиДляДобавления Цикл
		Выражение = СтрокаТаблицы.Представление; 
		ИндексПоля = СтрокаТаблицы.ИндексПоля;
...
&НаСервере
Процедура УдалитьАгрегированиеДляВыбранныхПолейНаСервере()
	ИндексыПолей = Новый СписокЗначений();
	
	Для Каждого ИдентификаторСтроки Из Элементы.АгрегируемыеПоля.ВыделенныеСтроки Цикл
		СтрокаТаблицы = АгрегируемыеПоля.НайтиПоИдентификатору(ИдентификаторСтроки);
		ИндексыПолей.Добавить(СтрокаТаблицы.ИндексПоля);
	КонецЦикла;
		
	Если Не ЗначениеЗаполнено(ИндексыПолей) Тогда
		Возврат;
	КонецЕсли;
	
	Построитель = ПостроительМоделиЗапроса();
	
	ИндексыПолей.СортироватьПоЗначению(НаправлениеСортировки.Убыв);
	Для Каждого ЭлементСписка Из ИндексыПолей Цикл
		ИндексПоля = ЭлементСписка.Значение;
		Построитель.УдалитьАгрегированиеДляПоля(ИндексПоля);	
	КонецЦикла;
...
&НаСервере
Процедура УдалитьВыбранныеГруппировкиНаСервере()
	ИндексыСтрок = Новый СписокЗначений();
	
	Для Каждого ИдентификаторСтроки Из Элементы.ГруппируемыеПоля.ВыделенныеСтроки Цикл
		СтрокаТаблицы = ГруппируемыеПоля.НайтиПоИдентификатору(ИдентификаторСтроки);
		Индекс = ГруппируемыеПоля.Индекс(СтрокаТаблицы);
		ИндексыСтрок.Добавить(Индекс);
	КонецЦикла;
		
	Если Не ЗначениеЗаполнено(ИндексыСтрок) Тогда
		Возврат;
	КонецЕсли;
	
	Построитель = ПостроительМоделиЗапроса();
	
	ИндексыСтрок.СортироватьПоЗначению(НаправлениеСортировки.Убыв);
	Для Каждого ЭлементСписка Из ИндексыСтрок Цикл
		Индекс = ЭлементСписка.Значение;
		Построитель.УдалитьГруппировку(Индекс);	
	КонецЦикла;
...
Функция ПараметрыОткрытияВложенногоЗапроса(ИдентификаторИсточника = Неопределено)
	...
	Оператор = ТекущийОператорЗапроса(МодельЗапроса);
	ИсточникиЗапроса = МодельЗапросаУтилиты.ИсточникиОператораПоИдентификаторам(Оператор);
...
Функция ПараметрыОткрытияРедактораОписанияВТ(ИндексИсточника = Неопределено)
	...
	Источник = МодельЗапросаУтилиты.ВсеИсточникиОператора(ТекущийОператор)[ИндексИсточника].Источник;
...
Процедура ПриДобавленииИсточника(МодельЗапроса)
	...
	Оператор = ТекущийОператорЗапроса(МодельЗапроса); 
	ВсеИсточники = МодельЗапросаУтилиты.ВсеИсточникиОператора(Оператор);
	ИстчоникОператора = ВсеИсточники[ВсеИсточники.ВГраница()];
	
	СтрокиДереваПоПсевданимам = Новый Соответствие;
	УзелИсточника = КонструкторЗапросовФормы.ДобавитьТаблицу(Источники, ИстчоникОператора, ДоступныеТаблицыИБ);
	УзелИсточника.ИндексИсточника = ВсеИсточники.ВГраница();
