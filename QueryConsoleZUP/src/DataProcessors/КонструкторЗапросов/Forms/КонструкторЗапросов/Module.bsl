...
Процедура ОтборКомпоновкиДанныхВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПараметрыОповещения = Новый Структура();
	ПараметрыОповещения.Вставить("ИндексУсловия", СтрокаТаблицы.Индекс);
	Оповещение = Новый ОписаниеОповещения("ЗавершитьРедатированиеОтбораСКД", ЭтотОбъект, ПараметрыОповещения);
...
&НаКлиенте
Процедура ЗавершитьРедатированиеОтбораСКД(РезультатРедактирования, ДополнительынеПараметры) Экспорт
	Если РезультатРедактирования = Неопределено
		Или Не РезультатРедактирования.Модифицированность Тогда
