...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе	
		МодельЗапроса = МодельЗапросаУтилиты.СоздатьПакетЗапрсов();
	КонецЕсли;	    
	...
	Если Не ЭтоВложенныйЗапрос Тогда 
		ЗаполнитьСписокЗапрсов(МодельЗапроса);
	КонецЕсли;	
...
Процедура ДоступныеТаблицыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекущаяСтрока.ЭтоИсполняемоеПредставление Тогда
		ИмяПредставления = ТекущаяСтрока.Имя;
		ОткрытьРедакторПараметровПредставления(ИмяПредставления);
	Иначе	
...
Процедура ДоступныеТаблицыПередРазворачиванием(Элемент, Строка, Отказ)
	ЗапонитьДочерниеЭлементыДереваДоступныхТаблиц(Строка, "ДоступныеТаблицы");
КонецПроцедуры  
...
Процедура ИсточникиПередРазворачиванием(Элемент, Строка, Отказ)
	ЗапонитьДочерниеЭлементыДереваДоступныхТаблиц(Строка, "Источники");
КонецПроцедуры
...
Процедура ИсточникиПередУдалением(Элемент, Отказ)
	Если Элементы.Источники.ТекущиеДанные.Индекс = Неопределено Тогда
		Возврат;
	КонецЕсли;
	УдалитьИсточник(Элементы.Источники.ТекущиеДанные.Индекс);
КонецПроцедуры
...
Процедура ИсточникиПредставлениеОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	...
	ИзменитьПсведонимИсточника(Элементы.Источники.ТекущаяСтрока, Текст);
...
Процедура ПоляДляГруппировкиПередРазворачиванием(Элемент, Строка, Отказ)
	...
	Если КонструкторЗапросовКлиентСерверФормы.СодержаитТип(ТипЭлемента, "ГруппаВсеПоля") Тогда
		Возврат;
	КонецЕсли;	
	ЗапонитьДочерниеЭлементыДереваДоступныхПолейГруппировки(Строка, "ПоляДляГруппировки");

КонецПроцедуры  

&НаКлиенте
Процедура ПоляДляГруппировкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	ДобавитьГруппировку(ВыбраннаяСтрока);
КонецПроцедуры     
...
Процедура Подключаемый_ПсевдонимыЗапросОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, ВыборДобавлением, СтандартнаяОбработка)
	...
	ИндексОператора = Число(СтрЗаменить(Элемент.Имя, "ПсевдонимыЗапрос", "")) - 1;
	ИдентифкаторСтроки = Элементы.Псевдонимы.ТекущаяСтрока;
	ИндексКолонки = Псевдонимы.Индекс(Псевдонимы.НайтиПоИдентификатору(ИдентифкаторСтроки));
	
	СвязатьПолеОператораСКолонкойЗапроса(ИндексОператора, ИндексКолонки, ВыбранноеЗначение);
	
	Элементы.Псевдонимы.ТекущаяСтрока = ИдентифкаторСтроки;
...
Процедура ПоляДоступныеДляОтбораПередРазворачиванием(Элемент, Строка, Отказ)
	ЗапонитьДочерниеЭлементыДереваДостпныхДляУсловийПолей(Строка, "ПоляДоступныеДляОтбора");
КонецПроцедуры
...
Процедура АгрегируемыеПоляПредставлениеОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, ВыборДобавлением, СтандартнаяОбработка)
	...
	ИзменитьАррегатнуюФункциюДляПоля(Элементы.АгрегируемыеПоля.ТекущиеДанные.ИндексПоля, ВыбранноеЗначение);
КонецПроцедуры
...
&НаСервере
Процедура ЗаполнитьСписокЗапрсов(МодельЗапроса)
	Номер = 1;
...
Процедура СоздатьЭлементыОтбораженияПсевдонимовПолей(ТекущееКоличествоОператоров, НовоеКоличествоОператоров)
	...
	Для Сч = НовоеКоличествоОператоров + 1 По ТекущееКоличествоОператоров Цикл
		...
		УдаляемыйЭлемент = Элементы.Найти("Псевдонимы.Запрос" + Сч);
...
Процедура ЗаполнитьРазделИтоги(ЗапросВыбора) 
	...
	КолонкиПоИдентифкаторам = МодельЗапросаУтилиты.МассивСтруктурВСоответствие(ЗапросВыбора.Колонки, "Идентификатор");
	...
	Для Каждого ВыражениеИтогов Из ЗапросВыбора.ВыраженияИтогов Цикл
		...
		Колонка = КолонкиПоИдентифкаторам[ВыражениеИтогов.Выражение.Значение.Аргумент.Идентификатор];
...
Процедура ЗаполнитьРазделИндексы(ЗапросВыбора) 
	...
	Для Каждого ИндексируемоеПоле Из ЗапросВыбора.Индекс.Элементы Цикл
		...
		СтрокаТаблицы.ИндексПоля = Индекс;
		СтрокаТаблицы.Картинка = 30;
		Колонка = КолонкиПоИдентифкаторам[ИндексируемоеПоле.Идентификатор];
		СтрокаТаблицы.ИмяПоля = Колонка.Имя;
...
Процедура ЗаполнитьРазделПорядок(ЗапросВыбора)
	...
	Для Каждого ЭлементПорядка Из ЗапросВыбора.Порядок Цикл 
		...
		СтрокаГруппировки.Направление = ЭлементПорядка.Направеление;
		СтрокаГруппировки.Индекс = Индекс;
		
		Если ЭлементПорядка.Иерахия
			И ЭлементПорядка.Направеление = ЭлементыМоделиЗапроса.НовыйНаправлениеВозрастание() Тогда
			
			СтрокаГруппировки.Направление = "ВозрастаниеИерархии";
		ИначеЕсли ЭлементПорядка.Иерахия 
			И ЭлементПорядка.Направеление = ЭлементыМоделиЗапроса.НовыйНаправлениеУбывание() Тогда 
				
			СтрокаГруппировки.Направление = "УбываниеИерархии";
		ИначеЕсли ЭлементПорядка.Направеление = ЭлементыМоделиЗапроса.НовыйНаправлениеУбывание() Тогда 
...
&НаСервере
Процедура ЗапонитьДочерниеЭлементыДереваДоступныхТаблиц(Строка, ИмяПриемника)
	КонструкторЗапросовФормы.ЗапонитьДочерниеЭлементыДереваДоступныхТаблиц(
		Строка, 
		ЭтотОбъект[ИмяПриемника],    
		КонструкторЗапросовФормы.ТекущийОператорЗапроса(ЭтотОбъект),
		ДоступныеТаблицыИБ(), 
		ОписаниеДоступнхыВТИзВременногоХранилища());	
КонецПроцедуры	

&НаСервере
Процедура ЗапонитьДочерниеЭлементыДереваДостпныхДляУсловийПолей(Строка, ИмяПриемника)
	КонструкторЗапросовФормы.ЗапонитьДочерниеЭлементыДереваДоступныхТаблиц(
		Строка, 
		ЭтотОбъект[ИмяПриемника],    
		КонструкторЗапросовФормы.ТекущийОператорЗапроса(ЭтотОбъект),
...
&НаСервере
Функция ОписаниеДоступнхыВТИзВременногоХранилища(ДляИзменения = Ложь)
	ОписаниеДоступнхыВТ = ПолучитьИзВременногоХранилища(АдресСпискаДоступныхВТ);
	Если Не ДляИзменения Тогда
		ОписаниеДоступнхыВТВоВременноеХранилище(ОписаниеДоступнхыВТ);	
	КонецЕсли;
	
	Возврат ОписаниеДоступнхыВТ;
КонецФункции	   

&НаСервере
Процедура ОписаниеДоступнхыВТВоВременноеХранилище(ОписаниеДоступнхыВТ)
	АдресСпискаДоступныхВТ = ПоместитьВоВременноеХранилище(ОписаниеДоступнхыВТ, УникальныйИдентификатор);
КонецПроцедуры	
...
&НаСервере
Процедура ДоступныеТаблицыВыборНаСервере(ИдентифкаторСтроки)
	ВыбраннаяСтрока = ДоступныеТаблицы.НайтиПоИдентификатору(ИдентифкаторСтроки);
...
Функция ЭтоСтрокаДоступнойТаблицы(ВыбраннаяСтрока)
	...
	ИначеЕсли ВыбраннаяСтрока.ЭтоИсполняемоеПредставление
		...
		Или ВыбраннаяСтрока.ТипЗначения.СодержитТип(Тип("ДоступнаяВложеннаяТаблицаСхемыЗапроса"))  
...
Процедура ДобавитьИсточник(ИмяИсточника, АдресПараметровПредставления = Неопределено)
	...
	УзелИсточника.Индекс = Оператор.Источники.Элементы.ВГраница();
...
&НаСервере
Процедура УстановитьПараметрыИсполняемогоПредставления(ИмяПредставления, ПсевдонимПредставления, АдресПараметров) 
...
Процедура ВыбиратьРазрешенныеПриИзмененииНаСервере()
	Построитель = ПостроительМоделиЗапроса().УстановитьПолучениеРазрешенныхЗаспией(ВыбиратьРазрешенные);
	ЗавершитьРедактированиеМодели(Построитель);
КонецПроцедуры
...
&НаСервере
Процедура ДобавитьУпорялочиваниеНаСервере(ИдентифкаторВыбраннойСтроки)
	СтрокаТаблицы = ПоляДляУпорядочивания.НайтиПоИдентификатору(ИдентифкаторВыбраннойСтроки);
...
&НаСервере
Процедура ДобавитьКонтрольнуюТочкуИтоговНаСервере(ИдентифкаторВыбраннойСтроки)
	СтрокаТаблицы = ПоляДоступныеДляИтогов.НайтиПоИдентификатору(ИдентифкаторВыбраннойСтроки);
	Выражение = СтрокаТаблицы.Представление;
...
&НаСервере
Процедура ДобавитьГруппировку(ИдентифкаторВыбраннойСтроки)
	СтрокаТаблицы = ПоляДляГруппировки.НайтиПоИдентификатору(ИдентифкаторВыбраннойСтроки);
...
&НаСервере
Процедура ДобавитьАгрегируемоеПолеНаСервере(ИдентифкаторВыбраннойСтроки)
	СтрокаТаблицы = ПоляДляГруппировки.НайтиПоИдентификатору(ИдентифкаторВыбраннойСтроки);
...
&НаСервере
Процедура ИзменитьАррегатнуюФункциюДляПоля(ИндексПоля, ИмяФункцции)
	Построитель = ПостроительМоделиЗапроса();
	Построитель.ИзменитьАррегатнуюФункциюДляПоля(ИндексПоля, ИмяФункцции);
	Модель = ЗавершитьРедактированиеМодели(Построитель);
...
&НаСервере
Процедура ИзменитьПсевдонимПоля(ИдентфикаторСтроки, НовыйПсевдоним)
	СтрокаТаблицы = Псевдонимы.НайтиПоИдентификатору(ИдентфикаторСтроки);
...
&НаСервере
Процедура ИзменитьПсведонимИсточника(ИдентфикаторСтроки, НовыйПсевдоним)
	СтрокаТаблицы = Источники.НайтиПоИдентификатору(ИдентфикаторСтроки);
	
	ИндексИсточника = СтрокаТаблицы.Индекс;
...
&НаСервере
Процедура ДобавитьПолеИтоговНаСервере(ИдентифкаторВыбраннойСтроки)
	СтрокаТаблицы = ПоляДоступныеДляИтогов.НайтиПоИдентификатору(ИдентифкаторВыбраннойСтроки);
	Если СтрокаТаблицы.ТипЗначения = "ПолеЗапроса" Тогда
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура ДобавитьИндексНаСервере(ИдентифкаторВыбраннойСтроки)
	СтрокаТаблицы = ПоляДоступныеДляИндексации.НайтиПоИдентификатору(ИдентифкаторВыбраннойСтроки);
	Если СтрокаТаблицы.ТипЗначения = "ПолеЗапроса" Тогда
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура УдалитьИндексНаСервере(ИдентифкаторВыбраннойСтроки)
	СтрокаТаблицы = Индексы.НайтиПоИдентификатору(ИдентифкаторВыбраннойСтроки);	
	Построитель = ПостроительМоделиЗапроса();
...
Процедура ПакетЗапросовВыборНаСервере()
	ИндексЗапроса = ПакетЗапросов.Индекс(ПакетЗапросов.НайтиПоИдентификатору(Элементы.ПакетЗапросов.ТекущаяСтрока));
	ИндексОператораЗапроса = 0;
...
&НаКлиенте
Процедура ОткрытьРедакторПараметровПредставления(ИмяПредставления, ПсевдонимПредставления = Неопределено)
	...
	ПараметрыОткрытия.ИмяПредставления = ИмяПредставления;
	
	ПараметрыОповещения = Новый Структура("ИмяПредставления, ПсевдонимПредставления", ИмяПредставления, ПсевдонимПредставления);
	Оповещение = Новый ОписаниеОповещения("ЗавершитьРедактированиеПараметровПредставления", ЭтотОбъект, ПараметрыОповещения);
	
	ОткрытьФорму(
		"Обработка.КонструкторЗапросов.Форма.ПараметрыИсполняемогоПредстапвления", 
		ПараметрыОткрытия,
		ЭтотОбъект,,,,
		Оповещение,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);		
...
Процедура ЗавершитьРедактированиеПараметровПредставления(РезультатРедактирования, ДополнительныеПараметры) Экспорт    
	...
	ЗавершитьРедактированиеПараметровПредставленияНаСервере(   
		ДополнительныеПараметры.ИмяПредставления,
		ДополнительныеПараметры.ПсевдонимПредставления,
		РезультатРедактирования.АдресПараметровПредставления);
КонецПроцедуры

Процедура ЗавершитьРедактированиеПараметровПредставленияНаСервере(ИмяПредставления, ПсевдонимПредставления, АдресПараметровПредставления)    
	Если ПсевдонимПредставления = Неопределено Тогда
		ДобавитьИсточник(ИмяПредставления, АдресПараметровПредставления);	
	Иначе
...
Функция СписокДоступныхАгрегатныхФункций(ИндексПоля)
	...
	Если ЭтоВложенныйЗапрос Тогда
		СписокДоступныхВТ = ОписаниеДоступнхыВТИзВременногоХранилища();
	Иначе	
...
Функция ПараметрыРедактораВыражений()
	...
	ПараметрыРедактора.Вставить("ЭтоВложенныйЗапрос", ЭтоВложенныйЗапрос);
	ПараметрыРедактора.Вставить("АдресСпискаДоступныхВТ");
	Если ЭтоВложенныйЗапрос Тогда
		ПараметрыРедактора.АдресСпискаДоступныхВТ = ПоместитьВоВременноеХранилище(ОписаниеДоступнхыВТИзВременногоХранилища());
	КонецЕсли;	
...
Функция ПараметрыРедактораПараметровПредставления()
	...
	ПараметрыРедактора.Вставить("ИмяПредставления", "");
...
&НаСервере
Процедура ЗапонитьДочерниеЭлементыДереваДоступныхПолейГруппировки(Строка, ИмяПриемника)   
	ДоступныеТаблицыИБ = ДоступныеТаблицыИБ();   
		
	МодельЗапроса = КонструкторЗапросовФормы.МодельЗапросаИзВременногоХранилища(ЭтотОбъект);
	
	Если ЭтоВложенныйЗапрос Тогда
		ОписаниеДоступныхВТ = ОписаниеДоступнхыВТИзВременногоХранилища();
	Иначе	
		ОписаниеДоступныхВТ = МодельЗапросаУтилиты.ОписаниеДоступныхВременныхТаблиц(МодельЗапроса, ИндексЗапроса);
	КонецЕсли;		
...
Процедура ПоляДоступныеДляОтбораВыборНаСервере(ИдентификаторСтроки)
	...
	ЧастиИмениПоля = СтрРазделить(Выражение, ".");
	ИмяПараметра = ЧастиИмениПоля[ЧастиИмениПоля.ВГраница()];
	Выражение = Выражение + " = &" + ИмяПараметра;
	
	ДобавитьОтбор(Выражение);
КонецПроцедуры
...
&НаСервере
Процедура ЗапонитьДочерниеЭлементыДереваДоступныхПолейУпорядочивания(Строка)   
	ДоступныеТаблицыИБ = ДоступныеТаблицыИБ();   
...
&НаСервере
Процедура ЗапонитьДочерниеЭлементыДереваДоступныхПолейИтогов(Строка)   
	ДоступныеТаблицыИБ = ДоступныеТаблицыИБ();   
...
Процедура ПоляДоступныеДляИтоговПередРазворачиванием(Элемент, Строка, Отказ)
	...
	Если КонструкторЗапросовКлиентСерверФормы.СодержаитТип(ТипЭлемента, "ГруппаВсеПоля") Тогда
		Возврат;
	КонецЕсли;	
	ЗапонитьДочерниеЭлементыДереваДоступныхПолейИтогов(Строка);
КонецПроцедуры
