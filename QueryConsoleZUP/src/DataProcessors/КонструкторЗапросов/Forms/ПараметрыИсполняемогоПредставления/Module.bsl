...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяПредставления = Параметры.ИмяПредставления;  
	ПсевдонимПредставления = Параметры.ПсевдонимПредставления;  
	
	ОписаниеПредставления = ПоставщикИсполняемыхПредставлений.ОписаниеПредставленияПоИмени(ИмяПредставления);	
	...
	ПараметрыПредставленияВДанные(ПараметрыПредставления, ВТФильтр);
КонецПроцедуры   

Процедура ПараметрыПредставленияВДанные(Знач ПараметрыПредставления, Знач ВТФильтр = Неопределено)
...
Функция ПараметрыИзДанныхФормыВоВременноеХранилище()
	...
	Если ОписаниеПредставления.ОписаниеВТФильтр <> Неопределено Тогда
		...
		Если ЗапросВТФильтр <> Неопределено Тогда
			Параметр = ПараметрВТФильтрИзДанныхФормы(ОписаниеПредставления.ОписаниеВТФильтр, ЗапросВТФильтр);
			ПараметрыПредставления.Добавить(Параметр);
		КонецЕсли;
	КонецЕсли;
...
Функция ПараметрВТФильтрИзДанныхФормы(ОписаниеПараметра, ЗапросВТФильтр)
	ПараметрВТФильтр = ЭлементыМоделиЗапроса.НовыйПарараметрИсполянегоПредставленияВТФильтр();	
...
Функция ПараметрКонтсантаИзДанныхФормы(ОписаниеПараметра)
	...
	ПараметрПредставления = ОбработкаПредставлениеЗапросов.ПарараметрКонстантаПоОписанию(ОписаниеПараметра);
...
Процедура ДобавитьОтборажниеПараметраКонстанта(ОписаниеПараметра, Родитель)
	...
	Если ОписаниеПараметра.ДопустимПараметрЗапроса Тогда			
		Если ДопустимВводЗначения Тогда
			...
			Страница = Элементы.Добавить("СтраницыКонстантаЗначение" + ИмяПараметра, Тип("ГруппаФормы"), СтраницыГруппа);      
			Страница.Вид = ВидГруппыФормы.Страница;
			Страница.ВажностьПриОтображении = ВажностьПриОтображении.Обычная;
		
			Страницы.Добавить(Страница);
		
			Страница =  Элементы.Добавить("СтраницыГруппаПараметрЗапроса" + ИмяПараметра, Тип("ГруппаФормы"), СтраницыГруппа);      
			Страница.Вид = ВидГруппыФормы.Страница;
			Страница.ВажностьПриОтображении = ВажностьПриОтображении.Обычная;
			
			Страницы.Добавить(Страница); 
			
			СтраницыГруппа.ТекущаяСтраница = Страницы[0];  
			...
			ТипПараметра.УстановитьДействие("ПриИзменении", "ТипКонстантыПриИзменении");
			
			ЭтотОбъект["ТипКонстанты" + ИмяПараметра] = "Константа";
			
			РодительЭлементаВводаПараметра = Страницы[1];   
			РодительЭлементаВводаЗначения = Страницы[0];
		Иначе      
			ЭтотОбъект["ТипКонстанты" + ИмяПараметра] = "ПараметрЗапроса";
			РодительЭлементаВводаПараметра = ПараметрГруппа;	
		КонецЕсли;	
		 
		ПараметрЗапроса = Элементы.Добавить("ПараметрЗапроса" + ИмяПараметра, Тип("ПолеФормы"), РодительЭлементаВводаПараметра);      
		...
		//ПараметрЗапроса.Заголовок = ИмяПараметра;  
	...
	Иначе
		РодительЭлементаВводаЗначения = ПараметрГруппа; 		
	КонецЕсли;	
	
	Если ДопустимВводЗначения Тогда
		ЗначениеКонстанты = Элементы.Добавить("ЗначениеКонстанты" + ИмяПараметра, Тип("ПолеФормы"), РодительЭлементаВводаЗначения);       
		ЗначениеКонстанты.Вид = ВидПоляФормы.ПолеВвода;
		ЗначениеКонстанты.ПутьКДанным = "ЗначениеКонстанты" + ИмяПараметра;   
		//ЗначениеКонстанты.Заголовок = ИмяПараметра;         
...
&НаКлиенте
Процедура ТипКонстантыПриИзменении(Элемент)
