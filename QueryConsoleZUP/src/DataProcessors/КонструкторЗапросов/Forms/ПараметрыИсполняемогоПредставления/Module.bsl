...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонтекстМетаданныхUI = КонструкторЗапросовФормы.НовыйКонтекстМетаданныхUI();
	ОписаниеТаблицы = КонструкторЗапросовФормы.ОписаниеТаблицыДляРедактораПараметров(
		ИмяТаблицыПредставления, КонтекстМетаданныхUI);
	Если ОписаниеТаблицы = Неопределено Тогда
		ВызватьИсключение "Таблица " + ИмяТаблицыПредставления + " не найдена";
	КонецЕсли;
	ДобавитьЭлементыОтображенияПараметров(ОписаниеТаблицы.Параметры);
...
Функция ПараметрыИзДанныхФормыВоВременноеХранилище()
	...
	Если ОписаниеТаблицы = Неопределено Тогда
		ВызватьИсключение "Таблица " + ИмяТаблицыПредставления + " не найдена";
	КонецЕсли;
	ОписанияВТФильтра = КонструкторЗапросовФормы.ПараметрыПоВиду(
		ОписаниеТаблицы.Параметры, "ОписаниеВТФильтра");
	ОписанияКонстант = КонструкторЗапросовФормы.ПараметрыПоВиду(
		ОписаниеТаблицы.Параметры, "ОписаниеПараметраКонстанта");
	ОписанияОтбора = КонструкторЗапросовФормы.ПараметрыПоВиду(
		ОписаниеТаблицы.Параметры, "ОписаниеОтбора");
	...
	Если ОписанияВТФильтра.Количество() > 0 Тогда
		ЗапросВТФильтр = ЗапросВТФильтрИзВременногоХранилища();
		Если ЗапросВТФильтр <> Неопределено Тогда
			ПараметрыПредставления.ОписаниеВТФильтр = ПараметрВТФильтрИзДанныхФормы(
				ОписанияВТФильтра[0], ЗапросВТФильтр);
		КонецЕсли;
	КонецЕсли;
	
	Для Каждого ОписаниеПараметра Из ОписанияКонстант Цикл
	...
	Если ОписанияОтбора.Количество() > 0 Тогда
...
Процедура ДобавитьЭлементыОтображенияПараметров(Параметры)
	ОписанияВТФильтра = КонструкторЗапросовФормы.ПараметрыПоВиду(
		Параметры, "ОписаниеВТФильтра");
	Если ОписанияВТФильтра.Количество() > 0 Тогда
		ДобавитьОтборажениеПараметраВТФильтр(ОписанияВТФильтра[0]);
	КонецЕсли;

	ОписанияКонстант = КонструкторЗапросовФормы.ПараметрыПоВиду(
		Параметры, "ОписаниеПараметраКонстанта");
	ДобавитьЭлементыОтображенияПараметровКонстант(ОписанияКонстант);

	ОписанияОтбора = КонструкторЗапросовФормы.ПараметрыПоВиду(
		Параметры, "ОписаниеОтбора");
	Если ОписанияОтбора.Количество() > 0 Тогда
		ДобавитьОтборажениеПараметраОтбор(ОписанияОтбора[0]);
	КонецЕсли;
КонецПроцедуры	
...
Процедура ДобавитьОтборажениеПараметраВТФильтр(ОписаниеВТФильтр)
	...
	Для Каждого ОписаниеПараметраФильтра Из ОписаниеВТФильтр.Поля Цикл
		Если ОписаниеПараметраФильтра.Устарело Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ДобавитьОтборажниеПараметраКонстанта(ОписаниеПараметра, Родитель)
	...
	ДопустимВводЗначения = ДопустимВводЗначенияКонстанты(ОписаниеПараметра.ТипЗначения);
	
	Реквизит = Новый РеквизитФормы(
		"ЗначениеКонстанты" + ИмяПараметра,
		ОписаниеПараметра.ТипЗначения,
		"",
		,
		Истина);
	ДобавляемыеРеквизиты.Добавить(Реквизит);   
	...
	Если ОписаниеПараметра.Варианты.Количество() > 0 Тогда
		ЗначениеКонстанты.КнопкаВыпадающегоСписка = Истина;
		Для Каждого Значение Из ОписаниеПараметра.Варианты Цикл
...
&НаКлиенте
Процедура ЗавершитьРедактированиеОтбора(РезультатРедактирования, ДополнительныеПараметры = Неопределено) Экспорт 
...
Функция ПараметрыОткрытияРедактораВыражений()
	...
	МодельДляРедактора.Операторы[0].Источники.Добавить(Источник);
	Источник.Источник =
		ОбработкаПредставлениеЗапросов.ИсполняемоеПредставлениеПоИмениТаблицы(
			ИмяТаблицыПредставления, КонтекстМетаданныхUI.Реестр);
	...
	Если ОписанияОтбора.Количество() > 0 Тогда
		Для Каждого Поле Из ОписанияОтбора[0].Поля Цикл
			Постоитель.ДобавитьПолеВЗапрос(Поле.Имя, Поле.Имя);
		КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Процедура ЗавершитьРедактированиеЗапросаФильтра(РезультатРедактирования, ДополнительныеПараметры = Неопределено) Экспорт 
	Если РезультатРедактирования = Неопределено
		Или Не РезультатРедактирования.Модифицированность Тогда
