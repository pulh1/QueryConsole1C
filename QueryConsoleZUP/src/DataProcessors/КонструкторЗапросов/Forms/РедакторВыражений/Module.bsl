...
Процедура ЗаполнитьДоступныеФункции()
	...
	ПоследнийЭлемет = Неопределено;
	Уровень = Макет.Область("R" + Строка + КолонкаУровня).Текст;
	Пока Уровень <> "-1" Цикл 
		Если Уровень = "" Тогда 
			Уровень = ПоследнийЭлемет.Уровень + 1;
			Картинка = -1;
			ЭтоФункция = Ложь;
		...
		Если ЭтоФункция Тогда
			ПоследнийЭлемет = НовыйЭлемент;
		КонецЕсли;;
	КонецЦикла;
КонецПроцедуры

Процедура ЗаполнитьДостпныеПоля()
...
Процедура ПоляПередРазворачиванием(Элемент, Строка, Отказ) 
	...
	Если КонструкторЗапросовКлиентСерверФормы.СодержаитТип(ТипЭлемента, "ГруппаВсеПоля") Тогда
		Возврат;
	КонецЕсли;	
	ЗапонитьДочерниеЭлементыДереваДоступныхПолей(Строка, "Поля");
КонецПроцедуры  

Процедура ЗапонитьДочерниеЭлементыДереваДоступныхПолей(Строка, ИмяПриемника)   
	Схема = Новый СхемаЗапроса;
	ДоступныеТаблицыИБ = Схема.ПакетЗапросов.Добавить(Тип("ЗапросВыбораСхемыЗапроса")).ДоступныеТаблицы;   
