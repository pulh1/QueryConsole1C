...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоступныПоляВсехИсточников = Параметры.ДоступныПоляВсехИсточников;
	ИспользоватьПсевдонимИсточника = Параметры.ИспользоватьПсевдонимИсточника;
...
Процедура ЗаполнитьДоступныеПоля()
	...
	КонструкторЗапросовФормы.ЗаполнитьВыбранныеПоляЗапроса(Поля, ТекущийОператор.ВыбираемыеПоля, ТекущийОператор, ИспользоватьПсевдонимИсточника); 
	
	Если ДоступныПоляВсехИсточников Тогда
		ГруппаВсеПоля = КонструкторЗапросовФормы.НовыйГруппаВсеПоля();
		
		Если ТекущийОператор.Источники.Количество() > 0 Тогда
