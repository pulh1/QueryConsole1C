...
Процедура ПосетитьВложенныйЗапрос(УзелЗапроса) Экспорт
	ВложенныйКонтекст = УниверсальныйОтчетРасширенный.КонтекстОбработкиРазыменованийВложенногоЗапроса(Контекст);
	УниверсальныйОтчетРасширенный.ОбработатьРазыменованияВЗапросе(УзелЗапроса, -1, , ВложенныйКонтекст);
КонецПроцедуры
...
Процедура РазыменованиеПриВыходе(Выражение) Экспорт
	...
	Для Индекс = 0 По Выражение.Элементы.ВГраница() Цикл  
		...
		Если ТипЗнч(ЭлементРазыменования) = Тип("Строка") Тогда
			ТипВыраженияИсточника = ТипВыражения(ПредыдущийЭлемент);
			Выражение.Элементы[Индекс - 1] = ВыражениеСПриведениемТипа(ПредыдущийЭлемент, ТипВыраженияИсточника);
			Прервать;	  		
		ИначеЕсли ЭлементРазыменования.Тип = "ПриведениеТипа" Тогда
			// Значит тип уже определен, дальнейшая обработка не нужна
			Прервать;
		...
		КонецЕсли;	
		ПредыдущийЭлемент = ЭлементРазыменования;
	КонецЦикла;	
КонецПроцедуры
...
Функция ТипВыражения(Выражение)
	ОбрабатываемоеВыражение = ЭлементыМоделиЗапроса.НовыйВыражениеМоделиЗапроса();
	ОбрабатываемоеВыражение.Значение = ЭлементыМоделиЗапроса.НовыйРазыменование();
	ОбрабатываемоеВыражение.Значение.Элементы.Добавить(Выражение);
	
	ОбработкаМоделиЗапроса.ОбработатьВыражение(ОбрабатываемоеВыражение.Значение, Контекст, ОбрабатываемоеВыражение);
	Возврат ОбрабатываемоеВыражение.ТипЗначения;
КонецФункции

Функция ВыражениеСПриведениемТипа(Выражение, ТипЗначения)
	Если МодельЗапросаТипы.СодержитПроизвольныйТип(ТипЗначения) Тогда
		Возврат Выражение;
	КонецЕсли;
	
	Если ТипЗначения.Типы().Количество() = 1 Тогда
		НовоеВыражение = ЭлементыМоделиЗапроса.НовыйПриведениеТипа();
		НовоеВыражение.Выражение = Выражение;
		НовоеВыражение.ОписаниеТипа = ВыражениеТипСсылочногоПоля(ТипЗначения.Типы()[0]);
		
		Возврат НовоеВыражение;
	КонецЕсли;
	
	НовоеВыражение = ЭлементыМоделиЗапроса.НовыйСписокВыражений();
	Выбор = ЭлементыМоделиЗапроса.НовыйВыбор();
	НовоеВыражение.Элементы.Добавить(Выбор);
	
	Для Каждого Тип Из ТипЗначения.Типы() Цикл
		Альтернатива = ЭлементыМоделиЗапроса.НовыйАльтернативаВыбора();	
		Выбор.АльтернативыВыбора.Добавить(Альтернатива);
		ВыражениеТип = ВыражениеТипСсылочногоПоля(Тип);
		Альтернатива.Условие = ЭлементыМоделиЗапроса.НовыйБинарнаяОперация();
		Альтернатива.Условие.Операция = "=";
		Альтернатива.Условие.ЛеваяЧасть = ЭлементыМоделиЗапроса.НовыйФункцияТипЗначения();
		Альтернатива.Условие.ЛеваяЧасть.Аргумент = Выражение;
		Альтернатива.Условие.ПраваяЧасть = ЭлементыМоделиЗапроса.НовыйФункцияТип();	
		Альтернатива.Условие.ПраваяЧасть.Аргумент = ВыражениеТип;
		
		Альтернатива.Действие = ЭлементыМоделиЗапроса.НовыйПриведениеТипа();
		Альтернатива.Действие.Выражение = Выражение;
		Альтернатива.Действие.ОписаниеТипа = ВыражениеТип;
	КонецЦикла;
	
	Возврат НовоеВыражение;
КонецФункции

Функция ВыражениеТипСсылочногоПоля(ТипСсылочногоПоля)
	МетаданныеТипа = Метаданные.НайтиПоТипу(ТипСсылочногоПоля);
	ТипСсылочногоПоля = ЭлементыМоделиЗапроса.НовыйТипСсылочногоПоля(); 
	
	Если Метаданные.Справочники.Содержит(МетаданныеТипа) Тогда
		ТипСсылочногоПоля.Группа = "Справочник";
	ИначеЕсли Метаданные.Документы.Содержит(МетаданныеТипа) Тогда
		ТипСсылочногоПоля.Группа = "Документ";
	ИначеЕсли Метаданные.Перечисления.Содержит(МетаданныеТипа) Тогда
		ТипСсылочногоПоля.Группа = "Перечисление";
	ИначеЕсли Метаданные.ПланыОбмена.Содержит(МетаданныеТипа) Тогда
		ТипСсылочногоПоля.Группа = "ПланОбмена";
	ИначеЕсли Метаданные.ПланыСчетов.Содержит(МетаданныеТипа) Тогда
		ТипСсылочногоПоля.Группа = "ПланСчетов";
	ИначеЕсли Метаданные.ПланыВидовРасчета.Содержит(МетаданныеТипа) Тогда
		ТипСсылочногоПоля.Группа = "ПланВидовРасчета";
	ИначеЕсли Метаданные.ПланыВидовХарактеристик.Содержит(МетаданныеТипа) Тогда
		ТипСсылочногоПоля.Группа = "ПланВидовХарактеристик";	
	ИначеЕсли Метаданные.БизнесПроцессы.Содержит(МетаданныеТипа) Тогда
		ТипСсылочногоПоля.Группа = "БизнесПроцесс";
	ИначеЕсли Метаданные.Задачи.Содержит(МетаданныеТипа) Тогда
		ТипСсылочногоПоля.Группа = "Задача";
	Иначе
		Возврат Неопределено;
	КонецЕсли;	
			
	ТипСсылочногоПоля.Таблица = МетаданныеТипа.Имя;
	Возврат ТипСсылочногоПоля;
КонецФункции
