...
Функция ЭтоВариантПродукции(Продукция, НомерВарианта)
	СтруктураПоиска = Новый Структура("Продукция, НомерВарианта, КоличествоЭлементов", Продукция, НомерВарианта);
	
	Для Сч = 1 По КоличествоПросматриваемыхСимволов Цикл
		СтруктураПоиска.КоличествоЭлементов = Сч;
...
Функция НовыйУзел()
	Узел = Новый Структура("Листья, Тип, Значение, Лексема, НомерСтроки", Новый Массив);
	Возврат Узел;
КонецФункции	
...
Функция НеТерминалРазрешенныеОпционально(Родитель = Неопределено, ЛевыйЭлемент = Неопределено)
	ЭтотУзел = "ПУСТО";
	НомерВариантаПродукции = НомерВариантаПродукции("РазрешенныеОпцианально");
	Если НомерВариантаПродукции = 1 Тогда
		ЭтотУзел = Неопределено;
		ТекущийЭлемент = Неопределено;
		ЭтотУзел = Родитель;
