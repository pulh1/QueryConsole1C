...
Функция МожноДелегироватьВесьОтбор() Экспорт
	...
	Возврат ИзвлечьРезультатИзСтека();
КонецФункции
...
Процедура ПосетитьВложенныйЗапрос(УзелЗапроса) Экспорт
	...
	// TODO проверить использование истчоников внешнего запроса
	// это нужно проверять на этапе семкнтического анализа, а тут будет проверять нужное булево свойство
...
Процедура БинарнаяОперацияПриВыходе(Выражение) Экспорт
	...
	МожноДелегировать = РезультатПравойЧасти И РезультатЛевойЧасти;
...
Процедура ОператорПроверкиТипаПриВыходе(Выражение) Экспорт
	...
	ПоместитьРезультат(Выражение, МожноДелегировать, "ССЫЛКА");
КонецПроцедуры
...
Процедура СписокВыраженийПриВыходе(Выражение) Экспорт
	...
	Если Выражение.Элементы.Количество() > 1 Тогда
		МожноДелегировать = МожноДелегировать И ОписаниеОтбора.ПоддерживаемыеОперации.СписокВыражений;
	КонецЕсли;	
	
	ПоместитьРезультат(Выражение, МожноДелегировать, "список выражений");
КонецПроцедуры
	...
	Для Каждого ЭлементРазыменования Из Выражение.Элементы Цикл
		...
		ИначеЕсли ЭлементРазыменования.Тип = "ПолеИсточника" Тогда
			МожноДелегировать =  ЭлементРазыменования.ИдентификаторИсточника = ИдентификаторИсточникаПредставления
				И ИменаДоступныхПолей.Получить(ВРег(ЭлементРазыменования.ИмяПоля));
...
Процедура ПриведениеТипаПриВыходе(Выражение) Экспорт
	...
	ПоместитьРезультат(Выражение, МожноДелегировать, "ВЫРАЗИТЬ");
КонецПроцедуры
...
Процедура ФункцияЧастьПериодаЧисломПриВыходе(Выражение) Экспорт
	...
	ПоместитьРезультат(Выражение, МожноДелегировать, "ФункцияЧастьПериодаЧислом");
КонецПроцедуры
...
Процедура ФункцияРазностьДатПриВыходе(Выражение) Экспорт
	...
	МожноДелегировать = МожноДелегироватьАргумент1 И МожноДелегироватьАргумент2 И ОписаниеОтбора.ПоддерживаемыеОперации.ЯзыкВыражений;	
	ПоместитьРезультат(Выражение, МожноДелегировать, " ФункцияРазностьДат");
КонецПроцедуры
