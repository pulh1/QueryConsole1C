...
Перем РеестрПоставщиковМетаданныхТаблиц;
...
Функция Инициализировать(МодельЗапросаПараметр, ИндексЗапросаПараметр = Неопределено, ИндексОператораПараметр = 0, СписокДоступныхВТПараметр = Неопределено) Экспорт
	...
	Если РеестрПоставщиковМетаданныхТаблиц = Неопределено Тогда
		РеестрПоставщиковМетаданныхТаблиц =
...
Функция ВставитьЗапросВыбора(Индекс) Экспорт 
	ЗапросВыбора = ЭлементыМоделиЗапроса.НовыйЗапросВыбора(); 
	
	МодельЗапроса.Элементы.Вставить(Индекс, ЗапросВыбора);
	ИндексЗапроса = Индекс;
	ИндексОператора = -1;
	
	Для ИндексТекущего = Индекс + 1 По МодельЗапроса.Элементы.ВГраница() Цикл
		Элемент = МодельЗапроса.Элементы[ИндексТекущего];
		Если Элемент.ОписаниеВТ = Неопределено Тогда
			Продолжить;
		ИначеЕсли Элемент.Тип = "ЗапросВыбора" Тогда
			Если Элемент.ОписаниеВТ.ИндексЗапросаСоздания >= Индекс Тогда
				Элемент.ОписаниеВТ.ИндексЗапросаСоздания = Элемент.ОписаниеВТ.ИндексЗапросаСоздания + 1;
			КонецЕсли;
		Иначе
			Если Элемент.ОписаниеВТ.ИндексЗапросаУничтожения >= Индекс Тогда
				Элемент.ОписаниеВТ.ИндексЗапросаУничтожения = Элемент.ОписаниеВТ.ИндексЗапросаУничтожения + 1;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция СдвинутьЗапрос(ТекущийИндекс, Направление) Экспорт
	...
	ТекущийЗапрос = МодельЗапроса.Элементы[ТекущийИндекс];
	...
	Если НовыйЗапрос.ОписаниеВТ <> Неопределено Тогда
		Если НовыйЗапрос.Тип = "ЗапросВыбора" Тогда	
			НовыйЗапрос.ОписаниеВТ.ИндексЗапросаСоздания = ТекущийИндекс;
		Иначе
			НовыйЗапрос.ОписаниеВТ.ИндексЗапросаУничтожения = ТекущийИндекс;
		КонецЕсли;
	КонецЕсли;
	
	МодельЗапроса.Элементы[ТекущийИндекс] = НовыйЗапрос;
...
Функция ДобавитьОператорЗапроса() Экспорт
	...
	Если ЗапросВыбора.Операторы.Количество() > 1 Тогда
		...
		УдалитьКонтрольныеТочкиИтоговТипаВыражения(ЗапросВыбора);
	...
	Если Запрос.Тип = "ЗапросВыбора" Тогда
		Если ЗначениеЗаполнено(Запрос.ТаблицаДляПомещения) Тогда
			...
			Иначе
				...
				Запрос.ТаблицаДляПомещения = Неопределено;
				Запрос.Индекс.Элементы.Очистить();
			КонецЕсли;	
...
Функция УстановитьПолучениеРазрешенныхЗаписей(ВыбиратьРазрешенные) Экспорт
...
Функция ДобавитьИсточник(ИмяИсточника, Псевдоним = Неопределено, ПараметрыПредставления = Неопределено) Экспорт 
	...
	Если ЧастиИмени.Количество() = 1 Тогда
		...
		Иначе
			...
			Источник.ИмяТаблицы = ЧастиИмени[0];
			Источник.ОписаниеВТ = МодельЗапросаУтилиты.НайтиОписаниеДоступнойВТ(
				СписокДоступныхВТ(), Источник.ИмяТаблицы, ИндексЗапроса);
		КонецЕсли;
	Иначе
		ОписаниеТаблицы =
			РеестрПоставщиковМетаданныхТаблиц.НайтиТаблицу(ИмяИсточника);
		Если ОписаниеТаблицы = Неопределено Тогда
			ВызватьИсключение "Таблица " + ИмяИсточника + " не найдена";
		ИначеЕсли ОписаниеТаблицы.ВидИсточника = "СтандартнаяТаблица" Тогда
			Источник = ЭлементыМоделиЗапроса.НовыйИсточникДанныхТаблица();
			Источник.ИмяТаблицы = ИмяИсточника;
		ИначеЕсли ОписаниеТаблицы.ВидИсточника = "ИсполняемоеПредставление" Тогда
			Источник = ОбработкаПредставлениеЗапросов
				.ИсполняемоеПредставлениеПоИмениТаблицы(
					ИмяИсточника, РеестрПоставщиковМетаданныхТаблиц);
	...
	Иначе
		Источник.Псевдоним = МодельЗапросаУтилиты.СгенерироватьПсевдонимИсточника(
			ИмяИсточника, ОператорЗапроса);
	КонецЕсли;
	Если ЭтоИсполняемоеПредставление
		И ПараметрыПредставления <> Неопределено Тогда

		УстанавливаемыеПараметры =
			ПараметрыИсполняемогоПредставленияВМассив(ПараметрыПредставления);
		ОбработкаПредставлениеЗапросов
			.УстановитьПараметрыИсполняемогоПредставленияПоИмениТаблицы(
				ИсточникОператора,
				ИмяИсточника,
				УстанавливаемыеПараметры,
				ОписаниеДоступныхВТПоИменам(),
				РеестрПоставщиковМетаданныхТаблиц);
	КонецЕсли;	
	 
	ОператорЗапроса.Источники.Добавить(ИсточникОператора);
...
Функция УдалитьИсточник(ИндексИсточника) Экспорт
	...
	УдаляемыйИсточник = МодельЗапросаУтилиты.ВсеИсточникиОператора(Оператор)[ИндексИсточника];
	ИдентификаторИсточника = УдаляемыйИсточник.ИдентификаторИсточника;
	Контекст = КонтекстОбработкиВыражения();
	
	ПсевдонимыУдаленныхПолей = Новый Массив();
	
	ОбработатьВыбираемыеПоляПриУдаленииИсточника(Оператор, ИдентификаторИсточника, ПсевдонимыУдаленныхПолей, Контекст);
	...
	ОбработатьОтборСКДПриУдаленииИсточника(Оператор, ИдентификаторИсточника, Контекст);
	...
	Для Каждого ИмяПоля Из ПсевдонимыУдаленныхПолей Цикл
		...
		Если Не ЕстьПоляСвязанныеСКолонкой(Колонка, ПоляОператоров) Тогда
	...
	Для Каждого Соединение Из УдаляемыйИсточник.Соединения Цикл
		Оператор.Источники.Добавить(Соединение.Источник);
	КонецЦикла;
	МодельЗапросаУтилиты.УдалитьИсточникОператора(Оператор, УдаляемыйИсточник);
	
	Возврат ЭтотОбъект;
КонецФункции

Функция ДобавитьВложенныйЗапрос(МодельВложенногоЗапрос, Псевдоним = Неопределено) Экспорт 
	ОператорЗапроса = ТекущийОператор();
	
	Если Псевдоним = Неопределено Тогда
		ПсевдонимИсточника = МодельЗапросаУтилиты.СгенерироватьПсевдонимИсточника("ВложенныйЗапрос", ОператорЗапроса);
	Иначе
		ПроверитьУникальностьПсевдонимаИсточника(ОператорЗапроса, Псевдоним);
		ПсевдонимИсточника = Псевдоним;
	КонецЕсли;
	...
	ОператорЗапроса.Источники.Добавить(Источник);
	
	Возврат ЭтотОбъект;
КонецФункции

Функция ИзменитьВложенныйЗапрос(ИдентификаторИсточника, МодельВложенногоЗапрос) Экспорт 
	ТекущийЗапрос = ТекущийЗапрос();
	Оператор = ТекущийОператор();
	
	ИсточникиПоИдентификаторам = МодельЗапросаУтилиты.ИсточникиОператораПоИдентификаторам(Оператор);
...
Функция ДобавитьОписаниеВременнойТаблицы(ОписаниеВТ) Экспорт
	...
	ИзменитьОписаниеВременнойТаблицы(
		МодельЗапросаУтилиты.ВсеИсточникиОператора(ТекущийОператор()).ВГраница(),
		ОписаниеВТ);
	Возврат ЭтотОбъект;
КонецФункции
...
Функция ИзменитьОписаниеВременнойТаблицы(ИндексИсточника, НовоеОписание) Экспорт
	Оператор = ТекущийОператор();
	ИсточникОператора = МодельЗапросаУтилиты.ВсеИсточникиОператора(Оператор)[ИндексИсточника];
	Источник = ИсточникОператора.Источник;
	ИдентификаторИсточникаСтрокой = Строка(ИсточникОператора.ИдентификаторИсточника);
...
Функция ДобавитьПолеВЗапрос(Знач Выражение, Знач Псевдоним = Неопределено) Экспорт 
	...
	Если Колонка = Неопределено Тогда
		...
		Если ОписаниеВТ <> Неопределено Тогда
			МодельЗапросаУтилиты.ДобавитьКолнкуВОписаниеВТ(ОписаниеВТ, Колонка);	
		КонецЕсли;
	КонецЕсли;
...
Функция УдалитьПолеОператора(ИндексПоля) Экспорт
	...
	Если ЕстьПоляСвязанныеСКолонкой(Колонка, ПоляОператоров) Тогда
...
Функция УдалитьКолонкуЗапроса(ИндексКолонки) Экспорт
	Запрос = ТекущийЗапрос();
...
Функция СдвинутьКолонкуЗапроса(ТекущийИндекс, Направление) Экспорт
	НовыйИндекс = ТекущийИндекс + Направление;
	
	Запрос = ТекущийЗапрос();
	
	Если НовыйИндекс < 0 Или НовыйИндекс > Запрос.Колонки.ВГраница() Тогда
		Возврат ЭтотОбъект;
	КонецЕсли;
	
	ТекущмйЭлемент = Запрос.Колонки[ТекущийИндекс];
	НовыйЭлемент = Запрос.Колонки[НовыйИндекс];
	
	Запрос.Колонки[ТекущийИндекс] = НовыйЭлемент;
	Запрос.Колонки[НовыйИндекс] = ТекущмйЭлемент;
...
Функция ДобавитьСоединение(ПсевдонимПрисоединяемойТаблицы, ПсевдонимВладельцаСвязи, Знач ТипСоединения) Экспорт 
	...
	ОператорЗапроса = ТекущийОператор(); 
	Источник = МодельЗапросаУтилиты.ИсточникЗапросаПоПсевдониму(ТекущийОператор(), ПсевдонимВладельцаСвязи); 	 
	ПрисоединяемыйИсточник = МодельЗапросаУтилиты.ИсточникЗапросаПоПсевдониму(
		ОператорЗапроса, 
		ПсевдонимПрисоединяемойТаблицы);
	КорниПоддерева = Новый Массив();
	КорниПоддерева.Добавить(ПрисоединяемыйИсточник);
	Для Каждого ИсточникПоддерева Из МодельЗапросаУтилиты.ВсеИсточники(КорниПоддерева) Цикл
		Если ИсточникПоддерева.ИдентификаторИсточника = Источник.ИдентификаторИсточника Тогда
			ВызватьИсключение "Соединение не должно образовывать цикл источников";
		КонецЕсли;
	КонецЦикла;

	МодельЗапросаУтилиты.УдалитьИсточникОператора(ОператорЗапроса, ПрисоединяемыйИсточник);

	Соединение = ЭлементыМоделиЗапроса.НовыйСоединениеИсточника();
	Соединение.Источник = ПрисоединяемыйИсточник;
...
Функция ДобавитьОтборСКД(Знач ТекстВыражения, ВсеПоля = Истина, Псевдоним = Неопределено) Экспорт
	ОператорЗапроса = ТекущийОператор(); 
	Если ЗначениеЗаполнено(Псевдоним) Тогда
		ПроверитьУникальностьПсевдонимаОтбораСКД(ОператорЗапроса, Псевдоним);	
	КонецЕсли;
		
	ЭлементОтбораСКД = ЭлементыМоделиЗапроса.НовыйПолеЗапроса();
	УстановитьВыражениеЭлементаОтбораСКДСлужебный(ЭлементОтбораСКД, ТекстВыражения, ВсеПоля);
	
	Если ЗначениеЗаполнено(Псевдоним) Тогда
		ЭлементОтбораСКД.Псевдоним = Псевдоним;
	КонецЕсли;

	ОператорЗапроса.ОтборыСКД.Добавить(ЭлементОтбораСКД);
	
	Возврат ЭтотОбъект;
КонецФункции

Функция ИзменитьОтборСКД(ИндексУсловия, Знач ТекстВыражения, ВсеПоля) Экспорт
	ОператорЗапроса = ТекущийОператор();  
	УстановитьВыражениеЭлементаОтбораСКДСлужебный(ОператорЗапроса.ОтборыСКД[ИндексУсловия], ТекстВыражения, ВсеПоля);
	
	Возврат ЭтотОбъект;
КонецФункции

Функция УдалитьОтборСКД(ИндексУсловия) Экспорт
	ОператорЗапроса = ТекущийОператор(); 
	ОператорЗапроса.ОтборыСКД.Удалить(ИндексУсловия);	
	
	Возврат ЭтотОбъект;	
КонецФункции

Функция УстановитьДоступностьПодчиненныхПолейОтбораСКД(ИндексУсловия, ДоступныПодчиненные) Экспорт
	ОператорЗапроса = ТекущийОператор(); 
		
	ЭлементОтбораСКД = ОператорЗапроса.ОтборыСКД[ИндексУсловия];	
	
	УстановитьДоступностьПодчиненныхПолейДляОтбораСКДСлужебный(ЭлементОтбораСКД, ДоступныПодчиненные);
	
	Возврат ЭтотОбъект;
КонецФункции

Функция УстановитьПсевдонимОтбораСКД(ИндексУсловия, Псевдоним) Экспорт
	ОператорЗапроса = ТекущийОператор(); 
	Если ЗначениеЗаполнено(Псевдоним) Тогда
		ПроверитьУникальностьПсевдонимаОтбораСКД(ОператорЗапроса, Псевдоним);	
	КонецЕсли;
...
Процедура УстановитьВыражениеЭлементаОтбораСКДСлужебный(ЭлементОтбора, ТекстВыражения, Знач ВсеПоля = Истина)
	Контекст = КонтекстОбработкиВыражения(); 		
	...
	ЭлементОтбораВспомогательный = ЭлементыМоделиЗапроса.НовыйПолеЗапроса();
	ЭлементОтбораВспомогательный.Выражение = Выражение;
	
	ОбработкаМоделиЗапроса.ОбработатьЭлементОтбораСКД(ЭлементОтбораВспомогательный, Контекст);
	ЭлементОтбора.Выражение = ЭлементОтбораВспомогательный.Выражение;
	ЭлементОтбора.СодержитСимволВсеПоля = ЭлементОтбораВспомогательный.СодержитСимволВсеПоля;
	
	УстановитьДоступностьПодчиненныхПолейДляОтбораСКДСлужебный(ЭлементОтбора, ВсеПоля);
КонецПроцедуры

Процедура УстановитьДоступностьПодчиненныхПолейДляОтбораСКДСлужебный(ЭлементОтбора, ДоступныПодчиненные)	
	Если Не ДоступныПодчиненные
		И Не ЭлементОтбора.СодержитСимволВсеПоля Тогда
			
		Возврат;
	КонецЕсли; 
	
	Если ДоступныПодчиненные
		И ЭлементОтбора.СодержитСимволВсеПоля Тогда
			
		Возврат;
	КонецЕсли;
	
	Если ДоступныПодчиненные
		И Не МодельЗапросаТипы.СодержитСсылочныйТип(ЭлементОтбора.Выражение.ТипЗначения) Тогда
		
		ЭлементОтбора.СодержитСимволВсеПоля = Ложь;
		Возврат;
	КонецЕсли;	
		
	Если ДоступныПодчиненные Тогда
		ДобавитьДоступностьПодичиненныхПолейСКД(ЭлементОтбора);
	Иначе
		УдалитьДоступностьПодичиненныхПолейСКД(ЭлементОтбора);	
	КонецЕсли;
КонецПроцедуры

Процедура ДобавитьДоступностьПодичиненныхПолейСКД(Элемент)
	Если Элемент.Выражение.Значение.Тип <> "Разыменование" Тогда
		ВыражениеЗначение = Элемент.Выражение.Значение;
		Элемент.Выражение.Значение = ЭлементыМоделиЗапроса.НовыйРазыменование();
		Если ВыражениеЗначение.Тип = "СписокВыражений" Тогда
			Элемент.Выражение.Значение.Элементы.Добавить(ВыражениеЗначение);
		Иначе
			СписокВыражений = ЭлементыМоделиЗапроса.НовыйСписокВыражений();
			СписокВыражений.Элементы.Добавить(ВыражениеЗначение);
			Элемент.Выражение.Значение.Элементы.Добавить(СписокВыражений);
		КонецЕсли;
	КонецЕсли;		
	Элемент.Выражение.Значение.Элементы.Добавить(ЭлементыМоделиЗапроса.НовыйВыражениеВсеПоляИсточника());
	Элемент.СодержитСимволВсеПоля = Истина;
КонецПроцедуры

Процедура УдалитьДоступностьПодичиненныхПолейСКД(Элемент)
	Элемент.Выражение.Значение.Элементы.Удалить(Элемент.Выражение.Значение.Элементы.ВГраница());
	Если Элемент.Выражение.Значение.Элементы.Количество() = 1 
		И Элемент.Выражение.Значение.Элементы[0].Тип <> "ПолеИсточника" Тогда
		Элемент.Выражение.Значение = Элемент.Выражение.Значение.Элементы[0];
	КонецЕсли;	
	
	Если Элемент.Выражение.Значение.Тип =  "СписокВыражений" 
		И Элемент.Выражение.Значение.Элементы.Количество() = 1 Тогда
		
		Элемент.Выражение.Значение = Элемент.Выражение.Значение.Элементы[0];
	КонецЕсли;	
			
	Элемент.СодержитСимволВсеПоля = Ложь;	
КонецПроцедуры

Процедура ПроверитьУникальностьПсевдонимаОтбораСКД(ОператорЗапроса, Псевдоним)
	Для Каждого ОтборСКД Из ОператорЗапроса.ОтборыСКД Цикл
		Если ВРег(ОтборСКД.Псевдоним) = ВРег(Псевдоним) Тогда
			ВызватьИсключение "Не уникальный псевдоним " + Псевдоним + " для отбора СКД";
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Функция ИзменитьАгрегатнуюФункциюДляПоля(ИндексПоля, ИмяФункцции) Экспорт   
	Оператор = ТекущийОператор();
...
Функция УдалитьЭлементПорядка(ИндексЭлемента) Экспорт
	Запрос = ТекущийЗапрос();
	
	Запрос.Порядок.Удалить(ИндексЭлемента);
	
	Возврат ЭтотОбъект;	
КонецФункции

Функция СдвинутьЭлементПорядка(ТекущийИндекс, Направление) Экспорт
	НовыйИндекс = ТекущийИндекс + Направление;
	
	Запрос = ТекущийЗапрос();
	
	Если НовыйИндекс < 0 Или НовыйИндекс > Запрос.Порядок.ВГраница() Тогда
		Возврат ЭтотОбъект;
	КонецЕсли;
	
	ТекущмйЭлемент = Запрос.Порядок[ТекущийИндекс];
	НовыйЭлемент = Запрос.Порядок[НовыйИндекс];
	
	Запрос.Порядок[ТекущийИндекс] = НовыйЭлемент;
	Запрос.Порядок[НовыйИндекс] = ТекущмйЭлемент;
...
Функция ДобавитьКонтрольнуюТочкуИтогов(Знач ТекстВыражения) Экспорт
	...
	Контекст = КонтекстОбработкиВыражения(); 
	Контекст.РазрешеныИтоги = РазрешеныИтоги(ЗапросВыбора);	
...
Функция УстановитьПолучениеОбщихИтогов(Знач ОбщиеИтоги) Экспорт
	ЗапросВыбора = ТекущийЗапрос();
	
	Контекст = КонтекстОбработкиВыражения(); 
	Контекст.РазрешеныИтоги = РазрешеныИтоги(ЗапросВыбора);	
	
	ОбработкаМоделиЗапроса.ОбработатьОбщиеИтоги(ЗапросВыбора, ОбщиеИтоги, Контекст);
	
	ЗапросВыбора.ОбщиеИтоги = ОбщиеИтоги;
	
	Возврат ЭтотОбъект;
КонецФункции

Функция РазрешеныИтоги(ЗапросВыбора)
	ЭтоВложенныйЗапрос = МодельЗапроса.Тип = ЗапросВыбора.Тип;
	Возврат Не ЭтоВложенныйЗапрос И Не ЗначениеЗаполнено(ЗапросВыбора.ТаблицаДляПомещения);
КонецФункции
...
Функция СдвинутьИндекс(ТекущийИндекс, Направление) Экспорт
	НовыйИндекс = ТекущийИндекс + Направление;
	
	Запрос = ТекущийЗапрос();
	
	Если НовыйИндекс < 0 Или НовыйИндекс > Запрос.Индекс.Элементы.ВГраница() Тогда
		Возврат ЭтотОбъект;
	КонецЕсли;
	
	ТекущмйЭлемент = Запрос.Индекс.Элементы[ТекущийИндекс];
	НовыйЭлемент = Запрос.Индекс.Элементы[НовыйИндекс];
	
	Запрос.Индекс.Элементы[ТекущийИндекс] = НовыйЭлемент;
	Запрос.Индекс.Элементы[НовыйИндекс] = ТекущмйЭлемент;
...
Функция СвязатьПолеОператораСКолонкойЗапроса(ИндексОператора, ИндексКолонки, ИндексПоля) Экспорт
	...
	Если Не ЕстьПоляСвязанныеСКолонкой(ТекущаяКолонка, ПоляОператоров) Тогда
...
Функция УдалитьКолонкуЗапросаСлужебный(Запрос, ИндексКолонки)
	...
	Для Индекс = 0 По ВГраница Цикл
		...
		Если ЭлементИндекса.Идентификатор = ИдентификаторКолонки Тогда
...
Функция ИзменитьСвойстваКонтрольнойТочкиИтогов(ИндексЭлемента, ТипКонтрольнойТочки = Неопределено, ИмяКолонки = Неопределено,
	...
	КонтрольнаяТочкаДляОбработки = МодельЗапросаУтилиты.СкопироватьЭлементМоделиЗапроса(КонтрольнаяТочка);
	КонтрольнаяТочкаДляОбработки.ТипКонтрольнойТочки = ТипКонтрольнойТочки;
	КонтрольнаяТочкаДляОбработки.ТипДополненияПериодами = ВРег(ВариантДополненияПериодами);
	КонтрольнаяТочкаДляОбработки.НачалоПериодаДополнения = НачалоПериодаДополнения;
	КонтрольнаяТочкаДляОбработки.КонецПериодаДополнения = КонецПериодаДополнения;
	КонтрольнаяТочкаДляОбработки.ИмяКолонки = ИмяКолонки;
	
	ОбработкаМоделиЗапроса.ОбработатьДополнениеПериодамиКонтрольнойТочкиИтогов(КонтрольнаяТочкаДляОбработки);
	
	КонтрольнаяТочка.ТипКонтрольнойТочки = КонтрольнаяТочкаДляОбработки.ТипКонтрольнойТочки;
	КонтрольнаяТочка.ТипДополненияПериодами = КонтрольнаяТочкаДляОбработки.ТипДополненияПериодами;
	КонтрольнаяТочка.НачалоПериодаДополнения = КонтрольнаяТочкаДляОбработки.НачалоПериодаДополнения;
	КонтрольнаяТочка.КонецПериодаДополнения = КонтрольнаяТочкаДляОбработки.КонецПериодаДополнения;
	КонтрольнаяТочка.ИмяКолонки = КонтрольнаяТочкаДляОбработки.ИмяКолонки;
...
Функция ИзменитьПсевдонимПоля(ИндексПоля, НовыйПсевдоним) Экспорт
	...
	Если ОписаниеВТ <> Неопределено Тогда
		МодельЗапросаУтилиты.ДобавитьКолнкуВОписаниеВТ(ОписаниеВТ, ЗапросВыбора.Колонки[ИндексПоля]);
	КонецЕсли;
...
Функция ИзменитьПсевдонимИсточника(ИндексИсточника, НовыйПсевдоним) Экспорт
	...
	Источник = МодельЗапросаУтилиты.ВсеИсточникиОператора(Оператор)[ИндексИсточника];
	СтарыйПсевдоним = Источник.Источник.Псевдоним;
	Если СтарыйПсевдоним = НовыйПсевдоним Тогда
		Возврат ЭтотОбъект;
	КонецЕсли;
	ПроверитьУникальностьПсевдонимаИсточника(Оператор, НовыйПсевдоним);
	Источник.Источник.Псевдоним = НовыйПсевдоним;
...
Функция УдалитьСоединение(ПсевдонимПрисоединяемойТаблицы, ПсевдонимВладельцаСвязи) Экспорт
	Оператор = ТекущийОператор();
	Источник = МодельЗапросаУтилиты.ИсточникЗапросаПоПсевдониму(Оператор, ПсевдонимВладельцаСвязи);
	ПрисоединяемыйИсточник = МодельЗапросаУтилиты.ИсточникЗапросаПоПсевдониму(
		Оператор,
		ПсевдонимПрисоединяемойТаблицы);
	...
	Оператор.Источники.Добавить(ПрисоединяемыйИсточник);
...
Функция УстановитьПризнакОбязательноТаблицыСКД(ПсевдонимТаблицы, Обязательно) Экспорт
	Оператор = ТекущийОператор();
	Соединение = МодельЗапросаУтилиты.НайтиСредиВсехСоединений(Оператор, ПсевдонимТаблицы);
	
	Если Соединение = Неопределено Тогда
		ВызватьИсключение "Признак Обязательно можно менять только для присоединяемsых таблиц";
	КонецЕсли;
	
	Соединение.Опционально = Не Обязательно;
...
Функция УстановитьПараметрыИсполняемогоПредставления(ПсевдонимПредставления, ПараметрыПредставления) Экспорт 
	ОператорЗапроса = ТекущийОператор();
	Источник = МодельЗапросаУтилиты.ИсточникЗапросаПоПсевдониму(
		ОператорЗапроса, ПсевдонимПредставления);
	Если Источник = Неопределено Тогда
		ВызватьИсключение "Источник с псевдонимом " + ПсевдонимПредставления + " не найден";
	КонецЕсли;
	ИмяТаблицы = Источник.Источник.ИмяТаблицы;
	ОписаниеТаблицы =
		РеестрПоставщиковМетаданныхТаблиц.НайтиТаблицу(ИмяТаблицы);
...
Функция КонтекстОбработкиВыражения()
	Возврат МодельЗапросаУтилиты.КонтекстОбработкиВыражения(
		ТекущийОператор(),
		ИндексЗапроса,
		СписокДоступныхВТ(),
		РеестрПоставщиковМетаданныхТаблиц);
...
Функция ОписаниеДоступныхВТПоИменам()
	ОписаниеДоступныхВТ = Новый Соответствие();
	
	СписокДоступныхВТ = СписокДоступныхВТ();
	Для Каждого ОписаниеВТ Из СписокДоступныхВТ Цикл
		ОписаниеДоступныхВТ.Вставить(ВРег(ОписаниеВТ.Имя), ОписаниеВТ);	
	КонецЦикла;
...
Процедура УдалитьКонтрольныеТочкиИтоговТипаВыражения(ЗапросВыбора)
...
Процедура ОбновитьТипКолонкиЗапроса(Колонка, ПоляОператоровПоПсевдонимам, ОписаниеВТ = Неопределено)
	...
	Для Каждого ПоляОператора Из ПоляОператоровПоПсевдонимам Цикл
		...
		Если ПолеОператора = Неопределено Тогда
			ДобавляемыйТип = Новый ОписаниеТипов("Null");	
		Иначе
			ДобавляемыйТип = ПолеОператора.Выражение.ТипЗначения;	
		КонецЕсли;
		
		Если УстанавливаемыйТип = Неопределено Тогда
			УстанавливаемыйТип = ДобавляемыйТип;
		Иначе
			УстанавливаемыйТип = Новый ОписаниеТипов(УстанавливаемыйТип, ДобавляемыйТип.Типы());
		КонецЕсли;
	КонецЦикла;
...
Функция ЕстьПоляСвязанныеСКолонкой(Колонка, ПоляОператоров) Экспорт
	Для Каждого ПоляОператора Из ПоляОператоров Цикл
...
Процедура ОбработатьИзменениеПолейИсточника(ЗапросВыбора, Оператор, УдаленныеПоля, ПоляСИзмененнымТипом)
	...
	ОбработатьВыбираемыеПоляПриИзмененииИсточника(
		Оператор, 
		УдаленныеПоля, 
...
Процедура УдалитьСоединенияСИсточником(ИдентификаторИсточника)
	...
	Для Каждого Источник Из МодельЗапросаУтилиты.ВсеИсточникиОператора(Оператор) Цикл
		ВГраница = Источник.Соединения.ВГраница();
		Для Индекс = 0 По ВГраница Цикл
			Соединение = Источник.Соединения[ВГраница - Индекс];
			Если Соединение.Источник.ИдентификаторИсточника = ИдентификаторИсточника Тогда
...
Процедура ОбработатьВыбираемыеПоляПриИзмененииИсточника(Оператор, УдаленныеПоля, ПоляСИзмененнымТипом, 
...
Процедура ОбработатьВыбираемыеПоляПриУдаленииИсточника(Оператор, ИдентификаторИсточника, ПсевдонимыУдаленныхПолей, Контекст)
	ВГраница = Оператор.ВыбираемыеПоля.ВГраница();
	Для Индекс = 0 По ВГраница Цикл
		Поле = Оператор.ВыбираемыеПоля[ВГраница - Индекс];
...
Процедура ОбработатьОтборСКДПриУдаленииИсточника(Оператор, ИдентификаторИсточника, Контекст)
	ВГраница = Оператор.ОтборыСКД.ВГраница();
	Для Индекс = 0 По ВГраница Цикл
		ЭлементОтбора = Оператор.ОтборыСКД[ВГраница - Индекс];
		Если МодельЗапросаУтилиты.ВыражениеИспользуетПоляИсточника(ЭлементОтбора.Выражение, ИдентификаторИсточника) Тогда
			Оператор.ОтборыСКД.Удалить(ВГраница - Индекс);		
		КонецЕсли;
	КонецЦикла;	
КонецПроцедуры
...
Процедура ОбработатьПакетЗапросовПриИзмененииКолонокВТ()
	...
	ИндексОператора = ИндексТекущегоОператора;
КонецПроцедуры
	...
	Для Каждого Оператор Из ЗапросВыбра.Операторы Цикл
		...
		Для Каждого Источник Из МодельЗапросаУтилиты.ВсеИсточникиОператора(Оператор) Цикл
			Если Источник.Источник.Тип = "ИсточникДанныхВложенныйЗапрос" Тогда
				ОбработатьВложенныйЗапросПриИзмененииКолонокВТ(
					Источник, 
					УдаленныеПоля, 
					ПоляСИзмененнымТипом,
					ДоступныеВТ);		
...
Процедура ОбработатьВложенныйЗапросПриИзмененииКолонокВТ(Источник, УдаленныеПоля, ПоляСИзмененнымТипом, ДоступныеВТ)
...
Процедура ПроверитьУникальностьПсевдонимаИсточника(ОператорЗапроса, Псевдоним)
	Для Каждого ИсточникОператора Из МодельЗапросаУтилиты.ВсеИсточникиОператора(ОператорЗапроса) Цикл
...
Функция ПараметрыИсполняемогоПредставленияВМассив(ПараметрыПредставления)
	УстанавливаемыеПараметры = Новый Массив();
	Если ПараметрыПредставления.ОписаниеВТФильтр <> Неопределено Тогда
		УстанавливаемыеПараметры.Добавить(ПараметрыПредставления.ОписаниеВТФильтр);	
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПараметрыПредставления.Отбор) Тогда
		Парсер = КонструкторЗапросовПовтИсп.ПарсерЗапросов();
		Выражение = Парсер.РазобратьВыражение(ПараметрыПредставления.Отбор);
		Отбор = ЭлементыМоделиЗапроса.НовыйОтборИсполняемогоПредставления();
		Отбор.Выражение = ЭлементыМоделиЗапроса.НовыйВыражениеМоделиЗапроса();
		Отбор.Выражение.Значение = Выражение;
		
		УстанавливаемыеПараметры.Добавить(Отбор);
	КонецЕсли;
	
	Для Каждого ПараметрКонстанта Из ПараметрыПредставления.ПараметрыКонстанты Цикл
		УстанавливаемыеПараметры.Добавить(ПараметрКонстанта);
	КонецЦикла;	
