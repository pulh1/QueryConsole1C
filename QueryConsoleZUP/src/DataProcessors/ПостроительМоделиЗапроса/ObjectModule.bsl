...
Функция ДобавитьИсточник(ИмяИсточника, Псевдоним = Неопределено, ПараметрыПредставления = Неопределено) Экспорт 
	...
	Иначе 		
		...
		Если ДоступныеТаблицы.Найти(ИмяИсточника) = Неопределено Тогда 
			ОписаниеПредставления = ПоставщикИсполняемыхПредставлений.ОписаниеПредставленияПоИмениТаблицы(ИмяИсточника);
	...
	Иначе
		Источник.Псевдоним = МодельЗапросаУтилиты.СгенерироватьПсевдонимИсточника(ИмяИсточника, ОператорЗапроса);
	КонецЕсли;
	Если ПараметрыПредставления <> Неопределено Тогда
		УстанавливаемыеПараметры = ПараметрыИсполняемогоПредставленияВМассив(ПараметрыПредставления);
...
Функция УстановитьПараметрыИсполняемогоПредставления(ПсевдонимПредставления, ПараметрыПредставления) Экспорт 
	ОператорЗапроса = ТекущийОператор();
	Источник = МодельЗапросаУтилиты.ИсточникЗапросаПоПсевдониму(ОператорЗапроса, ПсевдонимПредставления);
	
	ОписаниеПредставления = ПоставщикИсполняемыхПредставлений.ОписаниеПредставленияПоИмениТаблицы(Источник.Источник.ИмяТаблицы);
