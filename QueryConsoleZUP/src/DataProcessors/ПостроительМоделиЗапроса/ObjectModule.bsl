...
Перем МодельЗапроса; // см. ЭлементыМоделиЗапроса.НовыйПакетЗапрсов
...
Функция ДобавитьОператорЗапроса() Экспорт
	...
	Оператор.Источники = ЭлементыМоделиЗапроса.НовыйИстчоникиЗапроса();
...
Функция УстановитьПолучениеПервыхЗаписей(КоличествоЗаписей) Экспорт
	Если Не МодельЗапросаУтилиты.ВозможноПолучениеПервыхЗаписей(ТекущийЗапрос(), КоличествоЗаписей, ЭтоВложенныйЗапрос) Тогда
		ВызватьИсключение "Получение первых записей при создании временной таблицы или во вложенном запросе, возможно только если задано упорядочивание";
	КонецЕсли;
...
Функция ДобавитьИсточник(ИмяИсточника, Псевдоним = Неопределено, ПараметрыПредставления = Неопределено) Экспорт 
	...
	Иначе 		
		...
		Если ДоступныеТаблицы.Найти(ИмяИсточника) = Неопределено Тогда 
			...
			Источник = ОбработкаПредставлениеЗапрсов.ИсполняемоеПредставлениеПоОписанию(ОписаниеПредставления);	
		Иначе
	...
	Если ПараметрыПредставления <> Неопределено Тогда
		ОбработкаПредставлениеЗапрсов.УстановитьПараметрыИсполняемогоПредтсавления(
			Источник, 
...
Функция ИзменитьОписаниеВременнойТаблицы(ИндексИсточника, НовоеОписание) Экспорт
	...
	Для Каждого Поле Из ТекущееОписаниеВТ Цикл
...
Функция ДобавитьУпорядочивание(Знач ТекстВыражения) Экспорт
	...
	ОбработкаМоделиЗапроса.ОбработатьЭлементыПорядка(ЗапросВыбора, ЭлементыПорядка, Контекст, ЭтоВложенныйЗапрос);
...
Функция ДобавитьКонтрольнуюТочкуИтогов(Знач ТекстВыражения) Экспорт
	...
	ОбработкаМоделиЗапроса.ОбработатьКонтрольныеТочкиИтогов(ЗапросВыбора, КонтрольныеТочки, Контекст, ЭтоВложенныйЗапрос);
...
Функция ДобавитьПолеИтогов(Знач ИмяКолонки) Экспорт
	...
	АгрегатнаяФункция = МодельЗапросаУтилиты.АгрегаттнаяФункцияПоТипуЗначения(Колонка.ТипЗначения);	
	...
	ОбработкаМоделиЗапроса.ОбработатьПоляИтогов(ЗапросВыбора, ПоляИтогов, Контекст, ЭтоВложенныйЗапрос);
...
Функция ИзменитьСвойстваЭлементаУпорядочивания(ИндексЭлемента, УчитыватьИерархию, СортироватьПоВозрастанию) Экспорт
	...
	ЭлементУпорядочивания.Иерахия = УчитыватьИерархию;
	Если СортироватьПоВозрастанию Тогда
		ЭлементУпорядочивания.Направеление = ЭлементыМоделиЗапроса.НовыйНаправлениеВозрастание();
	Иначе
		ЭлементУпорядочивания.Направеление = ЭлементыМоделиЗапроса.НовыйНаправлениеУбывание();
	КонецЕсли;	
...
Функция ИзменитьСвойстваКонтрольнойТочкиИтогов(ИндексЭлемента, ТипКонтрольнойТочки = Неопределено, ИмяКолонки = Неопределено,
	...
	ДостпныеТипыДополнения = Новый Массив();
	ДостпныеТипыДополнения.Добавить("СЕКУНДА");
	ДостпныеТипыДополнения.Добавить("МИНУТА");
	ДостпныеТипыДополнения.Добавить("ЧАС");
	ДостпныеТипыДополнения.Добавить("ДЕНЬ");
	ДостпныеТипыДополнения.Добавить("НЕДЕЛЯ");
	ДостпныеТипыДополнения.Добавить("МЕСЯЦ");
	ДостпныеТипыДополнения.Добавить("ГОД");
	ДостпныеТипыДополнения.Добавить("ДЕКАДА");
	ДостпныеТипыДополнения.Добавить("ПОЛУГОДИЕ");
	...
	Если ЗначениеЗаполнено(ВариантДополненияПериодами) Тогда
		Если ДостпныеТипыДополнения.Найти(ВРег(ВариантДополненияПериодами)) = Неопределено Тогда
...
Функция ИзменитьПсевдонимПоля(ИндексПоля, НовыйПсевдоним) Экспорт
	...
	Если ЗапросВыбора.КолонкиПоПсевдонимам.Получить(ВРег(НовыйПсевдоним)) <> Неопределено Тогда
...
Функция УстановитьУсловиеСоединения(ПсевдонимПрисоединяемойТаблицы, ПсевдонимВладельцаСвязи, ТекстВыражения) Экспорт
	...
	Оператор = МодельЗапроса.Элементы[ИндексЗапроса].Операторы[ИндексОператора];
	ИсточникиПоИдентифкаторам = МодельЗапросаУтилиты.МассивСтруктурВСоответствие(Оператор.Источники.Элементы, "ИдентификаторИсточника");
	
	Соединение = МодельЗапросаУтилиты.НайтиСоединение(
		ТекущийОператор(), 
		ПсевдонимПрисоединяемойТаблицы, 
		ПсевдонимВладельцаСвязи, 
		ИсточникиПоИдентифкаторам);
...
Функция КонтекстОбработкиВыражения()
	Возврат МодельЗапросаУтилиты.КонтекстОбработкиВыражения(ТекущийОператор(), СписокДоступныхВТ(), ИндексЗапроса);	 
КонецФункции
