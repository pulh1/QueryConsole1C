...
Функция ДобавитьВложенныйЗапрос(МодельВложенногоЗапрос, Псевдоним = Неопределено) Экспорт 
	...
	Иначе
		// TODO Проверить уникальность псевдонима
...
Функция ДобавитьОтборСКД(Знач ТекстВыражения, ВсеПоля = Истина, Псевдоник = Неопределено) Экспорт
	ОператорЗапроса = ТекущийОператор(); 
	Если ЗначениеЗаполнено(Псевдоник) Тогда
		ПроверитьУникальностьПсевдоникаОтбораСКД(ОператорЗапроса, Псевдоник);	
	КонецЕсли;
...
Функция УстановитьПсевдонимОтбораСКД(ИндексУсловия, Псевдоник) Экспорт
	ОператорЗапроса = ТекущийОператор(); 
	Если ЗначениеЗаполнено(Псевдоник) Тогда
		ПроверитьУникальностьПсевдоникаОтбораСКД(ОператорЗапроса, Псевдоник);	
	КонецЕсли;
	...
	ЭлементОтбораСКД = ОператорЗапроса.ОтборыСКД[ИндексУсловия];	
	ЭлементОтбораСКД.Псевдоним = Псевдоник;
...
Процедура ПроверитьУникальностьПсевдоникаОтбораСКД(ОператорЗапроса, Псевдоник)
	Для Каждого ОтборСКД Из ОператорЗапроса.ОтборыСКД Цикл
		Если ВРег(ОтборСКД.Псевдоник) = ВРег(Псевдоник) Тогда
			ВызватьИсключение "Не ункальный псевдоним " + Псевдоник + " для отбора СКД";
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Функция УстановитьПризнакОбязательноТаблицыСКД(ПсевдонимТаблицы, Обязательно) Экспорт
	Оператор = ТекущийОператор();
	ИсточникиПоИдентификаторам = МодельЗапросаУтилиты.МассивСтруктурВСоответствие(Оператор.Источники.Элементы, "ИдентификаторИсточника");
	Соединение = МодельЗапросаУтилиты.НайтиСрелиВсехСоединений(Оператор, ПсевдонимТаблицы, ИсточникиПоИдентификаторам);	
	
	Если Соединение = Неопределено Тогда
		ВызватьИсключение "Признак Обязательно можно менять только для присоединяемх таблиц";
	КонецЕсли;
