...
Функция ДобавитьИсточник(ИмяИсточника, Псевдоним = Неопределено, ПараметрыПредставления = Неопределено) Экспорт 
	...
	Если ЧастиИмени.Количество() = 1 Тогда
		...
		КонецЕсли;
	Иначе 		
		РеестрПоставщиков = Обработки.РеестрПоставщиковМетаданныхТаблиц.СоздатьДляОбработкиЗапроса(СписокДоступныхВТ(), ИндексЗапроса);
		ОписаниеТаблицыПоставщика = РеестрПоставщиков.ОписаниеТаблицы(ИмяИсточника);
		Если ОписаниеТаблицыПоставщика = Неопределено Тогда
			ВызватьИсключение "Таблица " + ИмяИсточника + " не найдена";
		ИначеЕсли ОписаниеТаблицыПоставщика.ВидПоставщика = "ИсполняемыеПредставления" Тогда 
			ОписаниеПредставления = ОписаниеТаблицыПоставщика.ИсходноеОписание;
...
Функция ДобавитьСоединение(ПсевдонимПрисоединяемойТаблицы, ПсевдонимВладельцаСвязи, Знач ТипСоединения) Экспорт 
	...
	Соединение.Источник = МодельЗапросаУтилиты.ИсточникЗапросаПоПсевдониму(
		ОператорЗапроса, 
...
Функция УстановитьПризнакОбязательноТаблицыСКД(ПсевдонимТаблицы, Обязательно) Экспорт
	Оператор = ТекущийОператор();
	ИсточникиПоИдентификаторам = МодельЗапросаУтилиты.МассивСтруктурВСоответствие(Оператор.Источники.Элементы, "ИдентификаторИсточника");
	Соединение = МодельЗапросаУтилиты.НайтиСредиВсехСоединений(Оператор, ПсевдонимТаблицы, ИсточникиПоИдентификаторам);	
