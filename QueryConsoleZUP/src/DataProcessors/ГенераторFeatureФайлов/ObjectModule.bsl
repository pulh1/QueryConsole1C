...
Процедура СценарийСозданияПакетаЗапросаВТекДок(ТекстЗапроса, ТекушиеТабуляции = "") Экспорт
	...
	Иначе
		...
		ТекстовыйДокумент.ДобавитьСтроку(ТекушиеТабуляции + Сценарий);
		СценарийСозданияЗапросаУничтоженияВТВТекДок(МодельПакета.Элементы[0], ТекушиеТабуляции);	
	КонецЕсли;
	...
	Для Индекс = 1 По МодельПакета.Элементы.ВГраница() Цикл
		Если МодельПакета.Элементы[Индекс].Тип = "ЗапросВыбора" Тогда
			...
			ТекстовыйДокумент.ДобавитьСтроку(ТекушиеТабуляции + Символы.Таб + Шаг);
			СценарийСозданияЗапросаВыбораВТекДок(МодельПакета.Элементы[Индекс], ТекушиеТабуляции + Символы.Таб);
		Иначе
			Сценарий = "* Уничтожение временной таблицы " + МодельПакета.Элементы[Индекс].ИмяТаблицы;		
			ТекстовыйДокумент.ДобавитьСтроку(ТекушиеТабуляции + Сценарий);
			СценарийСозданияЗапросаУничтоженияВТВТекДок(МодельПакета.Элементы[Индекс], ТекушиеТабуляции + Символы.Таб);	
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура СценарийСозданияЗапросаВыбораВТекДок(МодельЗапроса, ТекушиеТабуляции = "") Экспорт
	...
	Иначе
		...
		Для Индекс = 1 По МодельЗапроса.Операторы.ВГраница() Цикл
			...
			Если МодельЗапроса.Операторы[Индекс].ТипОбъединения = ЭлементыМоделиЗапроса.НовыйТипОбъединение() Тогда
				ТекстовыйДокумент.ДобавитьСтроку(ТекушиеТабуляции + Символы.Таб + "И Я добавляю объединяемый запрос без дублирования записей");
			Иначе
				ТекстовыйДокумент.ДобавитьСтроку(ТекушиеТабуляции + Символы.Таб + "И Я добавляю объединяемый запрос");
			КонецЕсли;	
	...
	Если ЗначениеЗаполнено(МодельЗапроса.ТаблицаДляПомещения) Тогда
		ДобавитьУстановкуИмениВременнойТаблицыВТекДок(МодельЗапроса.ТаблицаДляПомещения, ТекушиеТабуляции);	
	КонецЕсли;
...
Процедура СценарийСозданияЗапросаУничтоженияВТВТекДок(МодельЗапроса, ТекушиеТабуляции = "") Экспорт
...
Процедура СценарийДобавленияВложенногоЗапросаВМассивСтрок(Источник, Знач ТекушиеТабуляции = "")
	ТекстовыйДокумент.ДобавитьСтроку(ТекушиеТабуляции + "* Вложенный запрос");	
	
	ТекстовыйДокумент.ДобавитьСтроку(ТекушиеТабуляции + Символы.Таб + "И Я добавляю вложенный запрос");
	
	Генератор = СоздатьГенераторДляВложенногоЗапроса();
	Генератор.СценарийСозданияЗапросаВыбораВТекДок(Источник.ЗапросВыбора, ТекушиеТабуляции + Символы.Таб);
	Шаг = "И Я завершаю редактирование текушего запроса в конструкторе";
	ТекстовыйДокумент.ДобавитьСтроку(ТекушиеТабуляции + Символы.Таб + Шаг);
...
Процедура ДобавитьУстановкуИмениВременнойТаблицыВТекДок(ИмяВТ, ТекушиеТабуляции)
	Шаг = "И Я устаналиваю имя создаваемой временной таблицы """ + ИмяВТ + """";
	ТекстовыйДокумент.ДобавитьСтроку(ТекушиеТабуляции + Шаг);
КонецПроцедуры
...
Процедура ПрисвоениеМногострочновоВыраженияПеременнойВТекДок(Выражение, ИмяПеременнной, Знач ТекушиеТабуляции)
	...
	ТекстовыйДокумент.ДобавитьСтроку(ТекушиеТабуляции + Шаг);
	ТекстовыйДокумент.ДобавитьСтроку(ТекушиеТабуляции + """""""");
	Для НомерСтроки = 1 По СтрЧислоСтрок(Выражение) Цикл
//		Строка = "|'" + СтрПолучитьСтроку(Выражение, НомерСтроки) + "'|";
		ТекстовыйДокумент.ДобавитьСтроку(ТекушиеТабуляции + СтрПолучитьСтроку(Выражение, НомерСтроки));
	КонецЦикла;
	ТекстовыйДокумент.ДобавитьСтроку(ТекушиеТабуляции + """""""");
КонецПроцедуры
