...
Функция Описание(ИмяРегистра) Экспорт
	...
	Описание.ОписаниеОтбора = ОписаниеИсполняемыхПредставленийШаблоны.ОписаниеБазовогоОтбора();
	...
	Для Каждого Реквизит Из МетаданныеРегистра.Реквизиты Цикл
		...
		Описание.ОписаниеОтбора.ДоступныеПоля.Добавить(Реквизит.Имя);
	КонецЦикла;
...
Функция Справка() Экспорт
	...
	Справка.Имя = "ИсполняемоеПредставление.РегистрРасчета.ИмяРегистра.База";
	Справка.Описание = "Позволяет получить информацию расчетной базе для регистров расчета Начисления и Удержания включая подробности. 
	|Для получения используются те же методы типовой конфигурации. которые используются при расчете зарплаты.";
	
	Возврат Справка;
КонецФункции

Функция ИмяПредставления() Экспорт
	Возврат "РегистрРасчета.База";
КонецФункции
...
Функция Исполнить(ПараметрыВыполнения, Запрос) Экспорт
	...
	Для Каждого Выоажение Из ПараметрыВыполнения.Отборы Цикл
		Отбор = ИсполнительПредставленийУтилиты.ЭлементБазовогоОтбораИзВыражения(Выоажение);
	...
	Если ИмяРегистра = "Начисления" Тогда
		СуммируемыеПоля = "РезультатБаза";
	Иначе
		СуммируемыеПоля = "РезультатБаза, РезультатБазаВЦеломЗаМесяц";
	КонецЕсли;
	ПоляГруппировкиСтрокой = СтрСоединить(ПоляГруппировки, ",");
	РасчетнаяБаза.Свернуть(ПоляГруппировкиСтрокой, СуммируемыеПоля);
...
Функция ИсполняемыйКод(ПараметрыВыполнения, Знач ТекущиеТабуляции) Экспорт
	...
	КонецЕсли;
	ПоляГруппировкиСтрокой = СтрСоединить(ПоляГруппировки, ",");
	Строка = "РасчетнаяБаза.Свернуть(""" + ПоляГруппировкиСтрокой + """,""" + СуммируемыеПоля + """);";
	ТекстовыйДокумент.ДобавитьСтроку(ТекущиеТабуляции + Строка);
	ТекстовыйДокумент.ДобавитьСтроку("");
...
Функция ЗапросПолученияДанныхРегистра(ПараметрыВыполнения, ИмяРегистра)
	...
	Для Каждого Выражение Из ПараметрыВыполнения.Отборы Цикл
		Отбор = ИсполнительПредставленийУтилиты.ЭлементБазовогоОтбораИзВыражения(Выражение);
		...
		Выражение = "РегистрРасчета." + Отбор.ПутьКПолю + Отбор.ВидСравнения + ЗначениеСтрокой;
		Оператор.Отбор.Добавить(Выражение);
	КонецЦикла;
	
	ОбязательныеПоляОтбора = ОбязательныеПоляРегистра(ИмяРегистра);
...
Функция ОбязательныеПоляРегистра(ИмяРегистра)
	...
	Для Каждого Колонка Из Таблица.Колонки Цикл
		...
		ОбязательныеПоля.Добавить(Колонка.Имя);
	КонецЦикла;
...
Функция ТекстРезультирующегоЗапрос(ПараметрыВыполнения, ОписаниеПредставления)
	...
	Для Каждого Поле Из ОписаниеПредставления.Поля Цикл
		Если ПараметрыВыполнения.ИспользуемыеПоля.Получить(ВРег(Поле.Имя)) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Псевдоним = ПсевдонимыПолейВРег.Получить(ВРег(Поле.Имя));	
