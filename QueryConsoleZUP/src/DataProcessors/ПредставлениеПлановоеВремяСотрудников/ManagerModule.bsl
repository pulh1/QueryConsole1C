...
Функция Исполнить(ПараметрыВыполнения, Запрос) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;
		Если НачалоМесяца(Выборка.ДатаНачала) <> НачалоМесяца(Выборка.ДатаОкончания) Тогда
...
Функция ИсполняемыйКод(ПараметрыВыполнения, ТекущиеТабуляции) Экспорт
	...
	|	|	#ВТФильтр КАК Фильтр"";
	|КонецЕсли;
	|ТекстЗапросаВТПериоды = СтрЗаменить(ТекстЗапросаВТПериоды, ""#ВТФильтр"", """ + ПараметрыВыполнения.ОписаниеВТФильтр.ИмяВТ + """);
	|
	|ЗапросВспомогательный.Текст = ТекстЗапросаВТПериоды;
	|ЗапросВспомогательный.Выполнить();
	|ПараметрыПолученияДанных.ИмяВТСотрудники = ""ВТФильтОбработанныйИсполняемоеПредсталениеПлановоеВремя"";
	|";
	
	ГенерацияИсполняемогоКодаПредставленийУтилиты.ВывестиМультиСтрокуВТекстовыйДокумент(
		ТекстовыйДокумент, 
		Строка, 
		ТекущиеТабуляции
	);
