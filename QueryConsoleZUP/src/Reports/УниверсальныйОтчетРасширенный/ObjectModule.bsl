...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Команда = Форма.Команды.Добавить("Подключаемая_УстановитьЗапрос");
	Команда.Заголовок = "Установить запрос";
	Команда.Действие = "Подключаемый_Команда";
	
	Кнопка = Форма.Элементы.Добавить(
		"Подключаемая_УстановитьЗапрос", 
		Тип("КнопкаФормы"), 
		Форма.Элементы.ГруппаГлавное);
		
	Кнопка.ИмяКоманды = "Подключаемая_УстановитьЗапрос";
КонецПроцедуры
...
Процедура ПослеЗаполненияПанелиБыстрыхНастроек(Форма, ПараметрыОбновления) Экспорт
	...
	ПараметрКомпоновки = Новый ПараметрКомпоновкиДанных("ТекстЗапросаИсточника");
	Индекс = Неопределено;
	Для Каждого Элемент Из Форма.Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
		Если ТипЗнч(Элемент) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных")
			И Элемент.Параметр = ПараметрКомпоновки Тогда
...
Процедура ПередЗагрузкойВариантаНаСервере(Форма, Настройки, ПередЗагрузкойНастроек = Ложь) Экспорт
	...
	ИначеЕсли Не ПередЗагрузкойНастроек Тогда
		...
		Для Каждого Параметр Из Настройки.ПараметрыДанных.Элементы Цикл
			Если Параметр.Параметр = Новый ПараметрКомпоновкиДанных("ТекстЗапросаИсточника") Тогда
				ФиксированныеПараметры.ТекстЗапросаИсточника = ?(Параметр.Значение = Неопределено, "", Параметр.Значение);
				Прервать;
			КонецЕсли;
		КонецЦикла;
		КлючСхемы = ХешСтроки(ФиксированныеПараметры.ТекстЗапросаИсточника);
...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД) Экспорт
	...
	Если ТекущийКлючСхемы = Неопределено Тогда 
		ТекущийКлючСхемы = ХешСтроки(ФиксированныеПараметры.ТекстЗапросаИсточника);
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	УниверсальныйОтчетРасширенный.ОбработатьРазыменованияВФинальномЗапросе(МодельОптимизированногоЗапроса);	
//	Текст = ГенерацияТекстовЗапросов.ТекстПакетаЗапросов(МодельОптимизированногоЗапроса);
	ИсполнительПредставлений.СформироватьУниверсальныйОтчет(
...
Функция ХешСтроки(Знач Строка)
	Если Строка = Неопределено Тогда
		Строка = "";
	КонецЕсли;
	ХешированиеДанных = Новый ХешированиеДанных(ХешФункция.MD5);
	ХешированиеДанных.Добавить(Строка);
	Возврат СтрЗаменить(ХешированиеДанных.ХешСумма, " ", "") + "_" + Формат(СтрДлина(Строка), "ЧГ=");
КонецФункции
