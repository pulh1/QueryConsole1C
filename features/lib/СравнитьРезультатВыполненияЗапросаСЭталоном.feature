...
Сценарий: Я сравниваю результат выполнения запроса в консоли с эталоном из файла "ОтносительныйПутьКЭталону"
	...
	"""bsl
		...
		Если ТаблицаЭталон.НайтиСтроки(Новый Структура("Флаг", 0)).Количество() <> ТаблицаЭталон.Количество() Тогда
			...
			Колонки.Добавить("Флаг");
			Поля = СтрСоединить(Колонки, ",");
			РазличияТаблиц = Новый Массив;
			Для Каждого СтрокаТаблицы Из ТаблицаЭталон Цикл
				Если СтрокаТаблицы.Флаг <> 0 Тогда
					Структура = Новый Структура(Поля);
					ЗаполнитьЗначенияСвойств(Структура, СтрокаТаблицы);
					РазличияТаблиц.Добавить(Структура);
				КонецЕсли;
			КонецЦикла;
			Контекст.Вставить("РазличияТаблиц", РазличияТаблиц);	
		КонецЕсли;
	"""
