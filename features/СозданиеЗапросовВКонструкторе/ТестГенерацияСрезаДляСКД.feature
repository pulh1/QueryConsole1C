...
Функционал: Создание запроса"ТестГенерацияСрезаДляСКД" в конструкторе
...
Контекст:
	...
	И я закрыл все окна клиентского приложения
Сценарий: создание запроса ТестГенерацияСрезаДляСКД
	...
	* Создание запроса получения данных
		...
		И Я добавляю поле запроса "ВТКадроаяИстория.Организация" как "Организация" в конструкторе запроса
	...
	* Проверка результа создания запроса
		Тогда Текст запроса в консоли соответствует эталону "files/ТестГенерацияСрезаДляСКД.txt"
