...
Функционал: Создание запроса"ТестИндексыИПсевдониымПолей" в конструкторе
...
Контекст:
	...
	И я закрыл все окна клиентского приложения
Сценарий: создание запроса ТестИндексыИПсевдониымПолей
	...
	* Создание временной таблицы ВТКадроаяИстория
		* Добавление ИсполняемоеПредставление.РегистрСведений.КадроваяИсторияСотрудников.СрезПоследних
		...
		И Я добавляю поле запроса "КадроваяИсторияСотрудниковСрезПоследних.Организация" как "ТекущаяОрганизация" в конструкторе запроса
	...
	* Проверка результа создания запроса
		Тогда Текст запроса в консоли соответствует эталону "files/ТестИндексыИПсевдониымПолей.txt"
