...
Функционал: Создание запроса"ТестПараметрыИнтервалыРегистра" в конструкторе
...
Контекст:
	...
	И я закрыл все окна клиентского приложения
Сценарий: создание запроса ТестПараметрыИнтервалыРегистра
	...
	* Создание временной таблицы ВТКадроаяИстория
		* Добавление ИсполняемоеПредставление.РегистрСведений.КадроваяИсторияСотрудников.Периоды
		...
		И Я добавляю поле запроса "КадроваяИсторияСотрудниковЗаписи.Организация" как "ТекущаяОрганизация" в конструкторе запроса
		...
		И Я устаналиваю имя создаваемой временной таблицы "ВТКадроаяИстория"
		И Я добавляю колонку запроса "Сотрудник" в индекс
		И Я добавляю колонку запроса "ТекущаяОрганизация" в индекс
	* Создание запроса получения данных
		...
		И Я добавляю поле запроса "ВТКадроаяИстория.НачалоПериода" как "НачалоПериода" в конструкторе запроса
		И Я добавляю поле запроса "ВТКадроаяИстория.КонецПериода" как "КонецПериода" в конструкторе запроса
		И Я добавляю поле запроса "ВТКадроаяИстория.ТекущаяОрганизация" как "ТекущаяОрганизация" в конструкторе запроса
	И Я завершаю редактирование текушего запроса в конструкторе
	* Проверка результа создания запроса
		Тогда Текст запроса в консоли соответствует эталону "files/ТестПараметрыИнтервалыРегистра.txt"
