...
- [x] **Step 1: Write failing fragment tests**
...
- [x] **Step 3: Extract the shared renderer minimally**
...
- [x] **Step 1: Write failing hybrid behavior tests**
...
- [x] **Step 3: Add default-preserving legacy assembly hooks**
...
- [x] **Step 4: Implement hybrid assembly**
...
- [x] **Step 5: Prove legacy parity and hybrid GREEN**
...
- [x] **Step 2: Write failing CLI route tests**
...
- [x] **Step 4: Implement config parsing and CLI routing**
...
- [x] **Step 1: Document verified boundary**
...
- [x] **Step 3: Run final diff and status checks**
...
- Task 1 RED: three projection calls failed with missing
  `production_names`; duplicate projection separately failed because no error
  was raised. GREEN commit: `28a632d`.
- Task 2 RED: `generate_canonical_functions` was absent. GREEN commit:
  `32f0791`.
- Task 3 RED: `parsergen.hybrid_bsl_codegen` was absent; the first GREEN
  attempt then exposed the real legacy/canonical syntax-error helper name
  collision, fixed at the linker boundary. GREEN commit: `91427d5`.
- Task 4 RED: config rejected `[migration]`, `ParsergenConfig` had no ownership
  field and CLI stayed on legacy generation. GREEN commit: `49b5b10`.
- Focused hybrid/legacy/reference gate used the actual available files
  `test_hybrid_bsl_codegen.py`, `test_bsl_codegen.py`, `test_artifacts.py`,
  `test_follow_select.py`, `test_repository_grammar.py` and
  `test_reference_parser.py`: `101 passed`, `1 skipped`, `768 subtests passed`.
- Final complete Python gate: `404 passed`, `1 skipped`, `4082 subtests
  passed`; the skip is the known Windows symlink privilege limitation.
- Current-source production `validate` and `generate --check` both return `1`
  with the same two `LLK202` at grammar lines 246/254 and 256/257. No artifact
  path was written and `parsergen.toml` remains without migration opt-in.
