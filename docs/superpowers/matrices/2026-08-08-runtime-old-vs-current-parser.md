...
## Контрольный paired-прогон

Этот single-run результат дополнен counterbalanced-серией
`old → current → old → current → current → old`.
Для выводов о производительности следует использовать
`2026-08-08-runtime-paired-old-vs-current-parser.md`: на объединённых
60 samples и с обратной парой большой запрос подтвердил локальное замедление
current parser на 9.3% по median и 6.4% по p95.
