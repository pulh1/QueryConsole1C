...
Clustering evidence воспроизводится так: XML-декодированный первый query text
канонизируется `CRLF/CR → LF`, переводится в Unicode lowercase, все Unicode
whitespace runs заменяются одним ASCII space, края обрезаются. Для multiset всех
перекрывающихся трёхсимвольных substrings считается cosine similarity по частотам.
Byte/text-exact pairs исключаются из near set; near edge имеет score `>= 0.94`,
а группы являются connected components. Exact-duplicate triple
`ЗапросДанныеУчетаВремениСотрудников` / `ТестДанныеУчетаВремени` /
`ТестДанныеУчетаВремениСПараметрами` имеет общий SHA-256
`39e57bf70704d71de0f3ed79266e413702c675a850cec719d8e51b8fa5f3d193`.
Восемь near-duplicate components определены следующими 12 edges (все paths имеют
prefix `QueryExamples/` и suffix `.q1c`):

| Group | Edge | Cosine |
|---|---|---:|
| G1 | `ТестГенерацияСрезаДляСКД` — `ТестИндексыИПсевдониымПолей` | 0.996918904928 |
| G1 | `ТестГенерацияСрезаДляСКД` — `ТестПараметрыСрезаПоследних` | 0.996939468187 |
| G1 | `ТестИндексыИПсевдониымПолей` — `ТестПараметрыСрезаПоследних` | 0.997430996821 |
| G2 | `ТестЗаписиРегистраЗапросБезВТ` — `ТестИнтервалыРегистраЗапросБезВТ` | 0.985964797217 |
| G3 | `ТестЗапросаВТШтатноеРасписание` — `ТестЗапросаВТШтатноеРасписаниеДляСКД` | 0.951946063756 |
| G4 | `ТестЗапросаНачисленияУдержания` — `ТестЗапросаНачисленияУдержанияАвансом` | 0.990230216763 |
| G4 | `ТестЗапросаНачисленияУдержания` — `ТестЗапросаНачисленияУдержания_И_КадроваяИстория` | 0.948549110858 |
| G4 | `ТестЗапросаНачисленияУдержания_И_КадроваяИстория` — `ТестЗапросаНачисленияУдержанияАвансом` | 0.940049394782 |
| G5 | `ТестЗапросаРабочиеМестаСотрудников` — `ТестЗапросаРабочиеМестаСотрудниковБезОтбораПоСотр` | 0.993765176807 |
| G6 | `ТестКадровыеДанныеСПараметрами` — `ТестКадровыеДанныеСПараметрами_БезФормированияДопЗапроса` | 0.998571467682 |
| G7 | `ТестПараметрыЗаписиРегистра` — `ТестПараметрыИнтервалыРегистра` | 0.981880523547 |
| G8 | `ТестСотрудникиОрганизацииБезВТФизлица` — `ТестСотрудникиОрганизацииПоВТФизлица` | 0.944445393370 |
