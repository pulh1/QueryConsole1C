...
При импорте фиксируются 289542 исходных bytes, 5489 строк,
165623 символа до нормализации и 160135 символов после нормализации CRLF в LF,
...
Для этого exact path в `.gitattributes` задан как `-text`, поэтому Git blob
сохраняет исходные CRLF bytes независимо от локального `core.autocrlf`.
