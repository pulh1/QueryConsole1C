...
Turn canonical semantic Parser IR and decision DAGs into a standalone Python
parser returning generated AST classes.
...
`python_semantic_codegen.py` is the only Python parser target. It consumes the
single canonical `build_parser_ir()` result and executes its semantic
operations. The earlier syntax-only `python_codegen.py`, its separate IR
builder and their tests are removed rather than maintained as a second parser
contract.
...
The caller must supply `build_parser_ir(...)`. The result contains
deterministic standalone `module_text` and immutable AST schema metadata. The generated module imports
...
The full parsergen suite must remain green. This milestone does not edit the
runtime BSL grammar or implement notebook lowering; those are separate
